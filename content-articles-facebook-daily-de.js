/**
 * content-articles-facebook-daily-de.js
 * Auto-generated from Spanish source of truth.
 */

window.siteContent = window.siteContent || {};
window.siteContent.de = window.siteContent.de || { global: {}, pages: {}, articles: {} };
window.siteContent.de.articles = window.siteContent.de.articles || {};

Object.assign(window.siteContent.de.articles, {

    "fb-daily-20260420-01-la-campana-de-la-renta-espanola-2025-afecta-a-mas-residentes-en-suiza-de-lo-que-": {
        title: "Die spanische Kampagne „Einkommen 2025“ betrifft mehr Einwohner in der Schweiz, als Sie denken",
        description: "Die Kampagne „Einkommenssteuer 2025“ startete in Spanien am 8. April und endet am 30. Juni 2026. Wenn Sie in der Schweiz wohnen und hier Ihren steuerlichen Wohnsitz haben, sind Sie grundsätzlich kein steuerlicher Wohnsitz…",
        keywords: "Glocke, Einkommen, Espanola, 2025, Auswirkungen, Bewohner, Schweiz, glauben",
        keywordsLocalized: true,
        category: "Steuern",
        hub: "impuestos",
        slug: "actualidad-2026-04-20-1-la-campana-de-la-renta-espanola-2025-afecta-a-mas-residentes-en-suiza-de-lo-que-",
        readingTime: 2,
        dateUpdated: "20 April 2026",
        featuredImage: "media/guides/actualidad-renta-espanola-suiza.jpg",
        imageAlt: "Spanisches Einkommensteuererklärungsformular für Einwohner in der Schweiz",
        summary: "Die Kampagne „Einkommenssteuer 2025“ startete in Spanien am 8. April und endet am 30. Juni 2026. Wenn Sie in der Schweiz wohnen und hier Ihren steuerlichen Wohnsitz haben, gilt grundsätzlich nicht...",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>Die Kampagne „Einkommenssteuer 2025“ startete in Spanien am 8. April und endet am 30. Juni 2026. Wenn Sie in der Schweiz leben und hier Ihren steuerlichen Wohnsitz haben, sind Sie grundsätzlich nicht in Spanien steuerlich ansässig und zahlen dort keine Steuern auf Ihr weltweites Einkommen. Bis dahin ist alles klar. Das Problem besteht darin, dass „im Prinzip“ nicht „auf jeden Fall“ bedeutet und es bestimmte Situationen gibt, die uns dazu zwingen, die spanische Steuerbehörde weiterhin im Auge zu behalten.</p>
<p>Wenn Sie eine Mietwohnung in Spanien, Einkünfte aus spanischem Kapital, eine spanische Sozialversicherungsrente haben oder im Jahr 2025 eine Immobilie verkauft haben, haben Sie weiterhin Verpflichtungen gegenüber dem spanischen Finanzministerium. In einigen Fällen werden Sie in Spanien direkt über die Einkommensteuer für Nichtansässige (Formular 210) besteuert, in anderen Fällen bestimmt das Doppelbesteuerungsabkommen zwischen der Schweiz und Spanien, wo die Zahlung erfolgt und ob im anderen Land Anspruch auf Befreiung oder Steuergutschrift besteht. Diese 1977 unterzeichnete und überarbeitete Vereinbarung bildet den Rahmen, der diese Fälle regelt, und es ist ratsam, sie sorgfältig zu lesen, bevor Sie davon ausgehen, dass „es nichts mehr für Sie bedeutet“.</p>
<p>Wenn Sie im Jahr 2025 in der Schweiz angekommen sind, kann das Steuerjahr auf zwei Länder aufgeteilt werden. Während der Monate, in denen Sie in Spanien wohnten, waren Sie in Spanien ansässig, und daraus ergeben sich Meldepflichten für das Einkommen in diesem Zeitraum. Und wenn Sie außerhalb Spaniens über Vermögenswerte verfügen, die 50.000 Euro übersteigen (Bankkonten, Immobilien, Investmentfonds), bleibt das Modell 720 bestehen, auch wenn die unverhältnismäßigen Sanktionen für europarechtswidrig erklärt wurden.</p>
<p>Das Praktischste: Wenn Sie Zweifel haben, ob Sie in Spanien zur Anmeldung verpflichtet sind, wenden Sie sich an einen Manager oder direkt an die AEAT über deren elektronische Zentrale (sede.agenciatributaria.gob.es). Die Missachtung der Verpflichtung führt nicht dazu, dass sie verschwindet, und die Folgen können Jahre später eintreten, einschließlich Verzugszinsen.</p>
</div>`
    },

    "fb-daily-20260420-02-suiza-negocio-con-trump-y-bajo-los-aranceles-al-15-pero-el-dano-ya-estaba-hecho": {
        title: "Die Schweiz hat mit Trump verhandelt und die Zölle auf 15 % gesenkt, aber der Schaden war bereits angerichtet",
        description: "Anfang 2025 verhängte die Trump-Administration Zölle in Höhe von 39 % auf Schweizer Exporte in die Vereinigten Staaten, einer der höchsten Prozentsätze in ganz Europa. Für einen Pa…",
        keywords: "Schweiz, Geschäft, Trump, Zölle, Schaden, war, erledigt, Prinzipien",
        keywordsLocalized: true,
        category: "Arbeit und Löhne",
        hub: "trabajo",
        slug: "actualidad-2026-04-20-2-suiza-negocio-con-trump-y-bajo-los-aranceles-al-15-pero-el-dano-ya-estaba-hecho",
        readingTime: 2,
        dateUpdated: "20 April 2026",
        featuredImage: "media/guides/actualidad-suiza-trump-aranceles.jpg",
        imageAlt: "Handelsverhandlungen zwischen der Schweiz und den USA über Zölle",
        summary: "Anfang 2025 verhängte die Trump-Regierung Zölle in Höhe von 39 % auf Schweizer Exporte in die Vereinigten Staaten, einer der höchsten Prozentsätze aller Zeiten.",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>Anfang 2025 verhängte die Trump-Administration Zölle in Höhe von 39 % auf Schweizer Exporte in die Vereinigten Staaten, einer der höchsten Prozentsätze in ganz Europa. Für ein Land, dessen Wirtschaft stark vom Export von Arzneimitteln, Uhren, Präzisionsmaschinen und medizinischen Instrumenten abhängt, war der Schlag bemerkenswert. Es liegt eine gewisse Ironie darin, dass die Schweiz, ein neutrales Land, das sich seit Jahrhunderten in keine Kriege verwickelt hat, plötzlich zu einem der beliebtesten „Handelsfeinde“ Washingtons wurde.</p>
<p>Die Schweizer Regierung hat eine Senkung auf 15 % ausgehandelt und erreicht, womit sie auf das gleiche Niveau wie die Europäische Union gebracht wird. Die Einigung ist eine relative Erleichterung: Die am stärksten gefährdeten Sektoren, insbesondere der Pharmasektor mit Roche und Novartis an der Spitze, können etwas aufatmen, aber die ausgesetzten Verträge, lahmgelegten Investitionen und verschobenen Projekte in den Monaten der Unsicherheit werden nicht durch eine Ankündigung wiederhergestellt. Die KOF (Wirtschaftsinstitut der ETH) prognostiziert bis 2026 ein Wachstum von 0,9 %, mit der Möglichkeit, dank der Zollsenkung sogar über 1 % zu steigen. Es ist keine Katastrophe, aber es ist nicht die unverwundbare Schweiz, die sich viele Menschen vorstellen.</p>
<p>Für die Beschäftigten in den betroffenen Branchen bedeutet dies etwas ganz Konkretes: weniger Neueinstellungen, größerer Produktivitätsdruck und teilweise bereits stattfindende Umstrukturierungen. Die Pharmaindustrie in den Kantonen Basel und Zürich beschäftigt eine bedeutende Anzahl Spanier. Auch das Uhrengeschäft im Jurabogen. Wenn Ihre Branche in die Vereinigten Staaten exportiert, sind Sie von den Ereignissen in Washington stärker betroffen, als es aus der Sicht Ihres Unternehmens scheint.</p>
<p>Die Episode bestätigt etwas, das klar sein sollte: Die Schweiz ist eine offene Volkswirtschaft, die stark von ihren Handelsbeziehungen abhängig ist, und die Stabilität, die hier täglich eingeatmet wird, bedeutet nicht, dass sie angesichts globaler Turbulenzen undurchdringlich ist. Es ist ein kleines Land mit einer großen Auslandspräsenz.</p>
</div>`
    },

    "fb-daily-20260420-03-la-declaracion-de-impuestos-en-suiza-quien-esta-obligado-cuando-y-como-pedir-mas": {
        title: "Steuern in der Schweiz einreichen: Wer ist wann verpflichtet und wie kann man mehr Zeit einfordern?",
        description: "Der Frühling ist in der Schweiz auch Steuererklärungszeit. In den meisten Kantonen liegt die ordentliche Frist zwischen Ende März und Ende April, wobei das genaue Datum v…",
        keywords: "Erklärung, Steuern, Schweiz, wer, verpflichtet, fragen, Zeit, Frühling",
        keywordsLocalized: true,
        category: "Steuern",
        hub: "impuestos",
        slug: "actualidad-2026-04-20-3-la-declaracion-de-impuestos-en-suiza-quien-esta-obligado-cuando-y-como-pedir-mas",
        readingTime: 2,
        dateUpdated: "20 April 2026",
        featuredImage: "media/guides/actualidad-declaracion-impuestos-suiza.jpg",
        imageAlt: "Schweizer Steuererklärungsformular",
        summary: "Der Frühling ist in der Schweiz auch Steuererklärungszeit. In den meisten Kantonen liegt die ordentliche Frist zwischen Ende März und Ende April, obwohl…",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>Der Frühling ist in der Schweiz auch Steuererklärungszeit. In den meisten Kantonen liegt die ordentliche Frist zwischen Ende März und Ende April, das genaue Datum variiert jedoch: Zürich legt den 31. März fest, Waadt und Bern den 31. März, Genf den 31. Mai. Fast alle Kantone erlauben die Beantragung einer Verlängerung, in der Regel kostenlos online. Einige erheben eine geringe Gebühr, wenn die Verlängerung lang ist. Wenn Sie die Erklärung nicht abgegeben haben und die Frist bereits abgelaufen ist, ist es ratsam, sie so schnell wie möglich anzufordern: Mahnungen und Bußgelder wegen verspäteter Ankunft lassen nicht lange auf sich warten.</p>
<p>Bei der Frage, wer sich anmelden muss, herrscht unter ausländischen Einwohnern häufig Verwirrung. Inhaber einer C-Bewilligung befinden sich in der gleichen Situation wie Schweizer Bürger: Sie melden sich wie jeder andere Einwohner und haben Zugang zu allen möglichen Abzügen (Transportkosten, Ausbildung, Unterhalt usw.). Anders verhält es sich bei B-Bewilligungsinhabern: Sie werden grundsätzlich über die Quellensteuer besteuert, die der Arbeitgeber direkt vom Lohn einbehält. Es gibt jedoch zwei Situationen, in denen Sie zusätzlich zur ordentlichen Steuererklärung verpflichtet sind: wenn Ihr jährliches Bruttoeinkommen CHF 120'000 übersteigt und wenn Sie über zusätzliche Einkünfte verfügen, die nicht durch die Quellensteuer gedeckt sind (Vermietung, Investitionen, selbständige Erwerbstätigkeit).</p>
<p>Auch wenn Sie nicht dazu verpflichtet sind, können Sie an der Abgabe einer Nachtragserklärung („Antrag auf nachträgliche Ordentliche Veranlagung“ oder deren kantonales Äquivalent) interessiert sein, wenn Sie wichtige Abzüge haben, die der Quellensteuerabzug nicht umfasst. Ausbildungskosten, Alimente, Beiträge in die Säule 3a oder eine Hypothek auf eine Immobilie im Ausland können erhebliche Renditen generieren. Es geschieht nicht automatisch, sondern Sie müssen es aktiv anfordern.</p>
<p>Die Online-Plattformen der einzelnen Kantone sind das übliche Tool: VaudTax, TaxMe Online für Genf, PrivateTax für Zürich. Die meisten ermöglichen den Import von Daten aus dem Vorjahr und sind recht einfach zu handhaben, wenn man die Logik des Systems erst einmal verstanden hat. Wenn es Ihr erstes Mal ist oder Sie sich in einer komplexen Situation befinden, kostet ein lokaler Treuhänder Geld, kann Ihnen aber deutlich mehr Fehler oder nicht geltend gemachte Abzüge ersparen.</p>
</div>`
    },

    "fb-daily-20260422-01-el-referendum-del-14-de-junio-lo-que-dicen-las-encuestas-y-lo-que-deberias-hacer": {
        title: "Die Volksabstimmung vom 14. Juni: Was die Umfragen sagen und was Sie tun sollten, wenn Sie ein Projekt in der Schweiz haben",
        description: "Am 14. Juni stimmt die Schweiz darüber ab, ob eine verfassungsmäßige Obergrenze von 10 Millionen Einwohnern festgelegt werden soll. Die von der UDC geförderte Initiative zwingt die Bundesregierung, die Vereinbarung von… zu brechen.",
        keywords: "Referendum, Juni, sagen sie, Umfragen, Sie sollten, tun, Sie haben, Projekt",
        keywordsLocalized: true,
        category: "Warnungen und Ressourcen",
        hub: "recursos",
        slug: "actualidad-2026-04-22-1-el-referendum-del-14-de-junio-lo-que-dicen-las-encuestas-y-lo-que-deberias-hacer",
        readingTime: 2,
        dateUpdated: "22 April 2026",
        featuredImage: "media/guides/actualidad-referendum-junio-suiza.jpg",
        imageAlt: "Urna de votación en referéndum suizo",
        summary: "Am 14. Juni stimmt die Schweiz darüber ab, ob eine verfassungsmäßige Obergrenze von 10 Millionen Einwohnern festgelegt werden soll. Die von der UDC geförderte Initiative zwingt die Bundesregierung dazu,…",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>Am 14. Juni stimmt die Schweiz darüber ab, ob eine verfassungsmäßige Obergrenze von 10 Millionen Einwohnern festgelegt werden soll. Die von der SVP geförderte Initiative zwingt den Bund, das Freizügigkeitsabkommen mit der EU zu brechen, wenn dieser Schwellenwert vor 2050 überschritten wird. Die Schweiz liegt derzeit bei rund 9,1 Millionen. Der Abstand ist geringer als es scheint.</p>
<p>Besorgniserregend ist nicht die Zahl selbst, sondern der Mechanismus: Sollte die Initiative angenommen werden, würde jede Überschreitung des Schwellenwerts zur Aufhebung der Verträge führen, die es europäischen Bürgern, einschließlich spanischen Bürgern, ermöglichen, sich mit relativ einfacher Bürokratie in der Schweiz niederzulassen und zu arbeiten. Die zuletzt veröffentlichte Umfrage zeigt 48 % dafür und 41 % dagegen, mit einer klaren geografischen Verteilung: Ländliche Gebiete und die Deutschschweiz sind günstiger; die Städte und die römische Schweiz, skeptischer.</p>
<p>Dennoch ist es ratsam, einen kühlen Kopf zu bewahren. Die Schweizer haben bereits mehrere ähnliche Initiativen der SVP abgelehnt, darunter auch diejenige von 2020, die versuchte, die Freizügigkeit direkt anzuprangern. Die „Nein“-Maschinerie greift meist spät, aber stark, und die Schweizer Wirtschaft, die massiv auf europäische Arbeitskräfte angewiesen ist, kämpft seit Monaten aktiv gegen die Initiative. Sollte die Initiative gewinnen, wären die Auswirkungen darüber hinaus nicht unmittelbar spürbar: Es käme zu jahrelangen Verhandlungen und Übergangsprozessen.</p>
<p>Wenn Sie ernsthaft planen, in die Schweiz zu kommen, ist es jetzt sinnvoll, die Reise nicht ohne Grund zu verschieben. Nicht aus Panik, sondern weil die Arbeits- und Genehmigungsverfahren per Definition bereits langsam sind und eine größere regulatorische Unsicherheit nach dem Sommer weitere Hindernisse schaffen könnte. Und wenn Sie schon lange genug hier sind, um eine C-Bewilligung oder sogar die Einbürgerung zu beantragen, ist es ein angemessener Zeitpunkt, sich über diese Prozesse zu informieren. Nicht aus Dringlichkeit, sondern aus Logik.</p>
</div>`
    },

    "fb-daily-20260422-02-la-nacionalidad-suiza-lo-que-hay-que-saber-antes-de-pedirla-y-la-trampa-que-nadi": {
        title: "Schweizer Staatsangehörigkeit: Was Sie wissen müssen, bevor Sie sich bewerben, und die Falle, vor der Sie niemand warnt",
        description: "Es gibt Tausende Spanier in der Schweiz, die seit 10, 15, 20 Jahren hier sind und nie ernsthaft über eine Einbürgerung nachgedacht haben. Manche, weil sie nicht wissen, dass sie darum bitten können, andere, weil...",
        keywords: "Nationalität, Schweiz, wissen, vorher, danach fragen, Falle, niemand, warnen",
        keywordsLocalized: true,
        category: "Formalitäten und Bewilligungen",
        hub: "tramites",
        slug: "actualidad-2026-04-22-2-la-nacionalidad-suiza-lo-que-hay-que-saber-antes-de-pedirla-y-la-trampa-que-nadi",
        readingTime: 2,
        dateUpdated: "22 April 2026",
        featuredImage: "media/guides/actualidad-nacionalidad-suiza-naturalizacion.jpg",
        imageAlt: "Pasaporte suizo y proceso de naturalización",
        summary: "Es gibt Tausende Spanier in der Schweiz, die seit 10, 15, 20 Jahren hier sind und nie ernsthaft über eine Einbürgerung nachgedacht haben. Einige, weil sie nicht wissen, was sie verlangen können ...",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>Es gibt Tausende Spanier in der Schweiz, die seit 10, 15, 20 Jahren hier sind und nie ernsthaft über eine Einbürgerung nachgedacht haben. Einige, weil sie nicht wissen, dass sie darum bitten können, andere, weil sie davon ausgehen, dass sie die Spanier verlieren. Keines der Argumente hält stand, wenn man es im Detail betrachtet.</p>
<p>Die Grundvoraussetzung ist klar: zehn Jahre effektiver Aufenthalt in der Schweiz und Besitz einer C-Bewilligung. Der Prozess beginnt bei der Wohnsitzgemeinde, verläuft über den Kanton und endet beim Bund. Jeder Kanton legt zudem eigene Kriterien für die Mindestdauer der Aufenthaltsdauer auf seinem Gebiet fest, die in der Regel zwischen zwei und fünf Jahren liegt. Einige, wie Genf oder Zürich, sind teuer in den Gebühren und langsam in den Konditionen. Andere, in kleineren Kantonen, können in kürzerer Zeit gelöst werden, wenn die Integrationskriterien gut erfüllt sind. Das erforderliche Sprachniveau ist B1 mündlich und A2 schriftlich in der Landessprache des Kantons: Französisch, Deutsch oder Italienisch. Nichts Ungewöhnliches für jemanden, der hier seit Jahren lebt und arbeitet.</p>
<p>Zur spanischen Staatsangehörigkeit: Spanien erlaubt die doppelte Staatsangehörigkeit, Sie müssen also auf nichts verzichten. Doch es gibt eine Falle, die nur sehr wenige Menschen kennen. Das spanische Zivilgesetzbuch legt fest, dass ein volljähriger spanischer Staatsbürger, der im Ausland wohnt, automatisch seine spanische Staatsangehörigkeit verliert, wenn er eine andere Staatsangehörigkeit erwirbt, es sei denn, er gibt innerhalb einer Frist von maximal drei Jahren nach dem Erwerb eine ausdrückliche Erklärung zur Beibehaltung vor dem Konsulat ab. Es ist nicht kompliziert: Es erfordert die persönliche Unterzeichnung einer Urkunde beim spanischen Konsulat in Bern oder beim Generalkonsulat in Genf. Aber wenn es nicht getan wird, ist es verloren. Und es gibt Menschen, die seit Jahren einen Schweizer Pass besitzen und ihr Spanisch verloren haben, ohne es zu wissen.</p>
<p>Wenn Sie den C-Ausweis schon lange genug in der Tasche haben, informieren Sie sich bei Ihrer Gemeinde. Die Fristen sind lang, die Bürokratie groß, aber das Ergebnis ist solide: ein Reisepass aus einem Land, das nicht auf ein bilaterales Abkommen angewiesen ist, um Ihnen das Bleiberecht zu garantieren.</p>
</div>`
    },

    "fb-daily-20260422-03-suiza-cambia-las-normas-cuando-el-titular-de-la-prensa-espanola-da-mas-miedo-que": {
        title: "„Die Schweiz ändert die Regeln“: Wenn die Schlagzeile in der spanischen Presse beängstigender ist als die Realität",
        description: "Seit Wochen veröffentlichen mehrere spanische Medien Artikel mit Titeln wie „Die Schweiz verschärft Regeln für Spanier“ oder „Die Schweiz fordert Arbeitsvertrag und Wohnsitz…“",
        keywords: "Schweiz, Änderungen, Regeln, Schlagzeile, Presse, Spanisch, Angst, Realität",
        keywordsLocalized: true,
        category: "Formalitäten und Bewilligungen",
        hub: "tramites",
        slug: "actualidad-2026-04-22-3-suiza-cambia-las-normas-cuando-el-titular-de-la-prensa-espanola-da-mas-miedo-que",
        readingTime: 2,
        dateUpdated: "22 April 2026",
        featuredImage: "media/guides/actualidad-prensa-espanola-suiza-normas.jpg",
        imageAlt: "Spanische Presseberichterstattung über regulatorische Änderungen in der Schweiz",
        summary: "Seit Wochen veröffentlichen mehrere spanische Medien Artikel mit Titeln wie „Die Schweiz verschärft Regeln für Spanier“ oder „Die Schweiz fordert Vertrag …“",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>Seit Wochen veröffentlichen mehrere spanische Medien Artikel mit Titeln wie „Die Schweiz verschärft die Regeln für Spanier“ oder „Die Schweiz verlangt einen Arbeitsvertrag und eine Adresse, um im Land zu wohnen.“ Einige dieser Artikel wurden in spanischen Gruppen in der Schweiz weit verbreitet und lösten Fragen und in einigen Fällen echte Besorgnis aus.</p>
<p>Das Problem ist, dass das, was sie beschreiben, nicht neu ist. Das Schweizer Aufenthaltsbewilligungssystem funktioniert seit Jahrzehnten genau so: Um eine B-Bewilligung (fünfjähriger Aufenthalt) zu erhalten, benötigt man einen Arbeitsvertrag mit einer Laufzeit von mindestens einem Jahr. Für eine L-Bewilligung (Kurzaufenthalt) ein befristeter Vertrag von weniger als einem Jahr. Um einen Job zu suchen, ohne zu arbeiten, können Sie bis zu drei Monate ohne Erlaubnis verbringen, unter bestimmten Umständen sogar bis zu sechs. Das ist genau das, was in den Artikeln steht, dargestellt als wäre es eine Gesetzesneuheit des Jahres 2026. Das ist nicht der Fall.</p>
<p>Was passiert, ist, dass es in einem Teil der spanischen Presse eine wiederkehrende Dynamik gibt: Sie übernehmen die aktuellen Bestimmungen von sem.admin.ch, formulieren sie um, als ob es sich um eine kürzliche Änderung handelte, fügen eine alarmierende Schlagzeile hinzu und generieren Klicks. Es ist keine Bosheit, oder nicht immer: Die Schweiz generiert Traffic bei Lesern, die über eine Auswanderung nachdenken, und jede Variation des Titels verkauft sich gut. Die Folge ist, dass in den Gruppen Falschmeldungen kursieren, die als offizielle Informationen getarnt sind, und es gibt Leute, die das Konsulat anrufen oder ein Projekt verschieben, weil sich etwas nicht geändert hat.</p>
<p>Der einzige wirksame Weg, dem entgegenzuwirken, besteht darin, zur Quelle zu gehen: sem.admin.ch für Berechtigungen, admin.ch für allgemeine Vorschriften, ch.ch für barrierefreie Zusammenfassungen. Wenn Sie ein Artikel aus einem spanischen Medium alarmiert, weil sich in der Schweiz angeblich etwas geändert hat, überprüfen Sie dies immer, bevor Sie ihn weiterleiten. Wenn es zu echten Veränderungen kommt, geht das meist eher mit institutionellem als mit journalistischem Lärm einher.</p>
</div>`
    },

    "fb-daily-20260423-01-la-franquicia-del-seguro-medico-va-a-subir-lo-que-cambia-y-lo-que-no": {
        title: "Der Selbstbehalt in der Krankenversicherung wird steigen: Was ändert sich und was nicht",
        description: "Der Bundesrat hat eine Gesetzesänderung zur Vernehmlassung vorgelegt, die eine Erhöhung der Mindestselbstbeteiligung in der Krankenversicherung von 300 auf 400 Franken vorsieht. Es hat sich seit 20 nicht verändert...",
        keywords: "Franchise, Versicherung, Arzt, hochladen, ändern, Rat, Bundesrat, Position",
        keywordsLocalized: true,
        category: "Gesundheit und KVG",
        hub: "salud",
        slug: "actualidad-2026-04-23-1-la-franquicia-del-seguro-medico-va-a-subir-lo-que-cambia-y-lo-que-no",
        readingTime: 2,
        dateUpdated: "23 April 2026",
        featuredImage: "media/guides/actualidad-franquicia-seguro-medico-suiza.jpg",
        imageAlt: "Grundkrankenversicherungs-Franchise in der Schweiz LAMal",
        summary: "Der Bundesrat hat eine Gesetzesänderung zur Vernehmlassung vorgelegt, die eine Erhöhung der Mindestselbstbeteiligung in der Krankenversicherung von 300 auf 400 Franken vorsieht. Nicht…",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>Der Bundesrat hat eine Gesetzesänderung zur Vernehmlassung vorgelegt, die eine Erhöhung der Mindestselbstbeteiligung in der Krankenversicherung von 300 auf 400 Franken vorsieht. Daran hat sich seit 2004 nichts geändert, was schon viel über die Geschwindigkeit aussagt, mit der diese Dinge in der Schweiz aktualisiert werden. Erklärtes Ziel ist es, die Versicherungsnehmer zu einer „selektiveren“ Inanspruchnahme medizinischer Leistungen zu bewegen, was in der Praxis bedeutet, dass jeder, der zum Arzt muss, etwas mehr aus eigener Tasche bezahlen muss, bevor der Fonds ins Spiel kommt.</p>
<p>Bei dem vorgeschlagenen Mechanismus handelt es sich nicht um eine einmalige Erhöhung, sondern vielmehr um ein automatisches Anpassungssystem, das an die Gesundheitskosten gekoppelt ist. Wenn die Kostenbeteiligung des Versicherten einen Schwellenwert von 13,5 % der Bruttoleistung unterschreitet, erhöht sich automatisch der Selbstbehalt. Kinder sind von dieser Maßnahme ausgenommen. Der theoretische Ausgleich besteht in einer leichten Senkung der monatlichen Prämien, wobei etwaige Prämieneinsparungen größtenteils durch die Erhöhung der direkten Kosten bei Nutzung des Systems ausgeglichen werden.</p>
<p>Dieses Projekt ist eine Reaktion auf einen Antrag des Parlaments, der im März 2025 angenommen wurde. Der Prozess befindet sich in der Konsultation, was bedeutet, dass er noch kein Gesetz ist, aber die Richtung des Fortschritts ist ziemlich klar. Für diejenigen, die sich heute für einen Selbstbehalt von 300 Franken entscheiden, weil sie sich keinen höheren leisten können, wird diese Änderung eine echte Wirkung haben. Und für diejenigen, die bereits einen hohen Selbstbehalt hatten, ändert sich sofort wenig. Wie immer in der Schweiz, hängen die Einzelheiten der endgültigen Auswirkungen auch von der Kasse und dem Versicherungsmodell ab, das Sie abgeschlossen haben.</p>
</div>`
    },

    "fb-daily-20260423-02-el-carnet-de-conducir-espanol-en-suiza-valido-al-llegar-pero-con-fecha-de-caduci": {
        title: "Der spanische Führerschein in der Schweiz: gültig bei der Einreise, jedoch mit Ablaufdatum",
        description: "Die meisten Spanier, die in der Schweiz ankommen, wissen, dass sie mit ihrem spanischen Führerschein fahren können. Was sie nicht immer wissen, ist, dass dieses Recht zeitlich begrenzt ist. Es war einmal...",
        keywords: "Führerschein, fahren, Spanisch, Schweiz, gültig, ankommen, Datum, Ablauf",
        keywordsLocalized: true,
        category: "Formalitäten und Bewilligungen",
        hub: "tramites",
        slug: "actualidad-2026-04-23-2-el-carnet-de-conducir-espanol-en-suiza-valido-al-llegar-pero-con-fecha-de-caduci",
        readingTime: 1,
        dateUpdated: "23 April 2026",
        featuredImage: "media/guides/actualidad-carnet-conducir-espanol-suiza.jpg",
        imageAlt: "Carnet de conducir español y permiso suizo",
        summary: "Die meisten Spanier, die in der Schweiz ankommen, wissen, dass sie mit ihrem spanischen Führerschein fahren können. Was sie nicht immer wissen, ist, dass dieses Recht eine Grenze von ... hat.",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>Die meisten Spanier, die in der Schweiz ankommen, wissen, dass sie mit ihrem spanischen Führerschein fahren können. Was sie nicht immer wissen, ist, dass dieses Recht zeitlich begrenzt ist. Mit Feststellung des Wohnsitzes in der Schweiz ist der europäische Führerschein 12 Monate gültig. Nach diesem Zeitraum ist das Fahren mit einem spanischen Führerschein ohne Umtausch gleichbedeutend mit dem Fahren ohne gültigen Führerschein, mit den Folgen, die dies im Falle einer Kontrolle oder eines Unfalls haben kann.</p>
<p>Der Austausch erfolgt beim kantonalen Verkehrsamt (Strassenverkehrsamt in deutschen Kantonen, service des automobiles in französischsprachigen Kantonen). Für EU- oder EWR-Bürger ist das Verfahren administrativ: Es muss keine Fahrprüfung bestanden werden. Die ausländische Bewilligung wird vorgelegt, der Wohnsitz nachgewiesen, eine je nach Kanton unterschiedliche Gebühr entrichtet (in der Regel zwischen 70 und 120 Franken) und die Schweizer Bewilligung erhalten. Der Originalführerschein wird an das Ausstellungsland zurückgegeben, Sie behalten ihn also nicht.</p>
<p>Es muss klar sein, dass die 12-Monats-Frist ab der amtlichen Anmeldung in der Gemeinde gilt, nicht ab dem Grenzübertritt. Wenn die Registrierung Wochen gedauert hat, ist dies der Referenzzeitpunkt. Das Vorgehen kann von Kanton zu Kanton leicht variieren und in einigen Fällen sind zusätzliche Unterlagen erforderlich, beispielsweise ein ärztliches Attest. Es lohnt sich, vor dem Zeitvertreib direkt beim Strassenverkehrsamt des Wohnkantons nachzufragen.</p>
</div>`
    },

    "fb-daily-20260423-03-hacerse-autonomo-en-suiza-siendo-extranjero-mas-pasos-de-los-que-parece": {
        title: "Als Ausländer in der Schweiz selbstständig machen: mehr Schritte als es scheint",
        description: "Für einen Einwohner Spaniens ist es durchaus möglich, in der Schweiz als Selbstständiger zu arbeiten. Aber es gibt ein paar Besonderheiten, die Sie kennen sollten, bevor Sie sich für neun anmelden...",
        keywords: "werden, selbständig, Schweiz, Sein, Ausländer, Schritte, scheint, Arbeit",
        keywordsLocalized: true,
        category: "Arbeit und Löhne",
        hub: "trabajo",
        slug: "actualidad-2026-04-23-3-hacerse-autonomo-en-suiza-siendo-extranjero-mas-pasos-de-los-que-parece",
        readingTime: 2,
        dateUpdated: "23 April 2026",
        featuredImage: "media/guides/actualidad-autonomo-suiza-extranjero.jpg",
        imageAlt: "Selbstständigerwerbender meldet sein Unternehmen in der Schweiz an",
        summary: "Für einen Einwohner Spaniens ist es durchaus möglich, in der Schweiz als Selbstständiger zu arbeiten. Es gibt aber ein paar Besonderheiten, die man vorher kennen sollte…",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>Für einen Einwohner Spaniens ist es durchaus möglich, in der Schweiz als Selbstständiger zu arbeiten. Es gibt jedoch einige Besonderheiten, die Sie kennen sollten, bevor Sie sich irgendwo anmelden, insbesondere wenn Sie aus Spanien kommen, wo das System der Selbständigkeit viel direkter ist. In der Schweiz ist die selbständige Tätigkeit nicht nur an die Steuerbehörde gebunden: Sie umfasst Bewilligungen, Ausgleichsfonds und ein Beitragsmodell, das anders funktioniert.</p>
<p>Der erste Punkt ist die Aufenthaltserlaubnis. Eine Standardbewilligung B berechtigt Sie nicht automatisch zur Ausübung einer selbständigen Erwerbstätigkeit. Die Bewilligung einer selbständigen Tätigkeit muss ausdrücklich beim kantonalen Migrationsamt beantragt werden. Dieses kann einen Geschäftsplan und einen Nachweis der Wirtschaftlichkeit verlangen. Nach der Genehmigung besteht der nächste Schritt darin, der AVS als Selbständiger beizutreten. Hier liegt der grundlegende Unterschied zum Arbeitnehmerregime: Selbstständige zahlen beide Beitragsanteile, den des Arbeitnehmers und den des Arbeitgebers, der rund 9,65 % des Nettoeinkommens ausmacht. Es gibt keinen Arbeitgeber, der die Hälfte übernimmt.</p>
<p>Was der Selbstständige im Vergleich zu einem Arbeitnehmer verliert, ist der Zugang zur Arbeitslosenversicherung. Für den Fall, dass das Geschäft schlecht läuft, gibt es keine Arbeitslosenkasse, auf die man zurückgreifen kann. Das entdecken viele zu spät. Die Krankenversicherung bleibt unabhängig von der beruflichen Tätigkeit weiterhin obligatorisch und individuell. Und obwohl die Besteuerung günstig sein kann, wenn die beruflichen Abzüge gut verwaltet werden, kann es in den ersten Jahren eng werden, wenn die Steuerbelastung nicht mit genügend Spielraum geplant wurde. Es ist kein Luxus, vor dem Schritt einen Treuhänder zu konsultieren, sondern in diesem Zusammenhang geradezu umsichtig.</p>
</div>`
    },

    "fb-daily-20260424-01-el-kurzarbeit-cuando-la-economia-frena-el-seguro-cubre-parte-de-tu-sueldo": {
        title: "Kurzarbeit: Wenn die Wirtschaft nachlässt, übernimmt die Versicherung einen Teil Ihres Gehalts",
        description: "In den letzten Wochen hat der Arbeitgeberverband der Schweizer Maschinen- und Elektronikindustrie (Swissmem) die Regierung gebeten, den Rückgriff auf Kurzarbeit, den Kürzungsmechanismus, zu genehmigen…",
        keywords: "Kurzarbeit, Wirtschaft, Bremsen, Versicherung, Abdeckungen, Teil, Gehalt, Neueste",
        keywordsLocalized: true,
        category: "Arbeit und Löhne",
        hub: "trabajo",
        slug: "actualidad-2026-04-24-1-el-kurzarbeit-cuando-la-economia-frena-el-seguro-cubre-parte-de-tu-sueldo",
        readingTime: 2,
        dateUpdated: "24 April 2026",
        featuredImage: "media/guides/actualidad-kurzarbeit-suiza.jpg",
        imageAlt: "Arbeitnehmer mit reduzierter Arbeitszeit aufgrund von Kurzarbeit in der Schweiz",
        summary: "In den letzten Wochen hat der Arbeitgeberverband der Schweizer Maschinen- und Elektronikindustrie (Swissmem) die Regierung gebeten, den Rückgriff auf die Kurzarbeit zu genehmigen.",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>In den letzten Wochen hat der Arbeitgeberverband der Schweizer Maschinen- und Elektronikindustrie (Swissmem) die Regierung gebeten, den Rückgriff auf Kurzarbeit zu genehmigen, den von der Arbeitslosenversicherung subventionierten Mechanismus zur Arbeitszeitverkürzung. Der direkte Grund sind die US-Zölle, die zwar nach wochenlangen Spannungen von 39 % auf 15 % gesenkt wurden, sich aber weiterhin auf Schweizer Exporte in die USA auswirken, insbesondere in Branchen wie Pharmazeutika, Maschinen und Uhren. Für die Arbeitnehmer ist es wichtig zu verstehen, was das in der Praxis bedeutet.</p>
<p>Kurzarbeit (RHT in französischsprachigen Kantonen, Kurzarbeit in germanischen) ermöglicht einem Unternehmen, die geleistete Arbeitszeit zu reduzieren, ohne jemanden entlassen zu müssen. Die Differenz zwischen Normalstunden und geleisteten Arbeitsstunden wird größtenteils von der Arbeitslosenversicherung übernommen: bis zu 80 % des Lohns für nicht geleistete Stunden. Es handelt sich nicht um eine Schenkung des Unternehmens, sondern um eine Leistung des Sozialversicherungssystems, die aktiviert wird, wenn der Grund für die Kürzung wirtschaftlicher, technischer oder struktureller Natur ist und nicht auf den Arbeitnehmer zurückzuführen ist. Das Unternehmen beantragt es beim kantonalen Arbeitsamt und der Arbeitnehmer muss es schriftlich entweder im Vertrag oder durch eine ausdrückliche Vereinbarung akzeptieren.</p>
<p>Wenn Sie in einem Unternehmen der Industrie- oder Exportbranche arbeiten und in den kommenden Monaten mit Ihnen über Kurzarbeit gesprochen wird, müssen Sie zunächst prüfen, ob das Unternehmen die kantonale Bewilligung korrekt beantragt hat. Zweitens müssen Sie wissen, dass Sie während dieser Zeit weiterhin Beiträge zahlen, weiterhin Anspruch auf den üblichen Urlaub haben und dass die Kurzarbeitszeit auf das Dienstalter angerechnet wird. Was Sie nicht tun können, ist, Überstunden anzurechnen und gleichzeitig die Arbeitszeit aufgrund von Kurzarbeit zu reduzieren: Das wird kontrolliert. Der Mechanismus dient gerade dazu, Massenentlassungen in vorübergehenden Krisenzeiten zu verhindern, und die Schweiz hat ihn im Jahr 2020 während der Pandemie massiv genutzt. In diesem Jahr ist der Kontext ein anderer, aber die Logik ist dieselbe.</p>
</div>`
    },

    "fb-daily-20260424-02-la-cuenta-bancaria-en-suiza-lo-que-nadie-te-explica-bien-cuando-llegas": {
        title: "Das Bankkonto in der Schweiz: Was niemand gut erklärt, wenn man ankommt",
        description: "Eine der ersten praktischen Schwierigkeiten bei der Ankunft in der Schweiz ist die Eröffnung eines Bankkontos. Es scheint ein einfacher Vorgang zu sein, aber es gibt einige Aspekte, die vorher bekannt sein sollten, um…",
        keywords: "Konto, Bankwesen, Schweiz, niemand, erklären Sie, nun, Sie kommen zuerst an",
        keywordsLocalized: true,
        category: "Formalitäten und Bewilligungen",
        hub: "tramites",
        slug: "actualidad-2026-04-24-2-la-cuenta-bancaria-en-suiza-lo-que-nadie-te-explica-bien-cuando-llegas",
        readingTime: 2,
        dateUpdated: "24 April 2026",
        featuredImage: "media/guides/actualidad-cuenta-bancaria-suiza.jpg",
        imageAlt: "Eröffnung eines Bankkontos in der Schweiz für Ausländer",
        summary: "Eine der ersten praktischen Schwierigkeiten bei der Ankunft in der Schweiz ist die Eröffnung eines Bankkontos. Es scheint ein einfacher Vorgang zu sein, aber es gibt einige Aspekte, die passen ...",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>Eine der ersten praktischen Schwierigkeiten bei der Ankunft in der Schweiz ist die Eröffnung eines Bankkontos. Es scheint ein einfacher Vorgang zu sein, aber es gibt einige Aspekte, die man vorher kennen sollte, um nicht wochenlanges Grübeln zu verschwenden. Zunächst benötigen Sie umgehend eine Schweizer IBAN (die mit CH beginnt), da Ihr Arbeitgeber diese für die erste Gehaltszahlung verlangt und viele Vermieter eine solche für die Wohnungskaution verlangen. Die Anreise nur mit einem spanischen Konto oder einer Revolut-Nummer mit europäischer IBAN reicht nicht aus, um in der Schweiz voll zu funktionieren.</p>
<p>Traditionelle Banken (UBS, Kantonalbanken, Raiffeisen) eröffnen Konten für Einwohner mit einer B-Bewilligung ohne allzu große Probleme, aber die Verfahren können langwierig sein: Terminvereinbarung, Formulare und manchmal bis zu zwei Wochen bis zum Erhalt der Karte. PostFinance, die Einheit der Schweizerischen Post, ist historisch gesehen für Einsteiger am besten zugänglich: Sie akzeptiert die vorläufige Bewilligung (die Bescheinigung der Gemeinde), ist in der ganzen Schweiz präsent und bietet angemessene Provisionen. Wenn Sie es schneller mögen, können Sie mit Yuh (dem Fintech von PostFinance und Swissquote) und Neon in wenigen Stunden ein Konto von Ihrem Handy aus eröffnen und eine echte Schweizer IBAN ausstellen, was das unmittelbare Problem löst.</p>
<p>Worüber Sie sich im Klaren sein müssen, ist der Unterschied zwischen einer Schweizer IBAN und einem Auslandskonto oder einem Konto in elektronischer Währung ohne echte Bankunterstützung. Revolut verfügt über eine IBAN mit einem litauischen oder belgischen Präfix: Sie mag für einige Überweisungen funktionieren, aber nicht alle Arbeitgeber oder alle Schweizer Verwaltungen akzeptieren sie, und für das Konto der dritten Säule (3a), das ein zentrales Element des Schweizer Rentensystems ist, benötigen Sie unbedingt eine Bank oder Versicherungsgesellschaft mit einer echten Präsenz im Land. Der praktische Ratschlag lautet: Starten Sie vom ersten Tag an bei PostFinance oder einer Schweizer Neobank und vergleichen Sie in aller Ruhe, wenn Sie bereits installiert sind, die Optionen und eröffnen Sie das Konto, das Sie wirklich benötigen.</p>
</div>`
    },

    "fb-daily-20260424-03-los-festivos-en-suiza-dependen-del-canton-donde-trabajas-no-de-donde-vives": {
        title: "Die Feiertage in der Schweiz hängen vom Kanton ab, in dem Sie arbeiten, nicht von Ihrem Wohnort",
        description: "Die Schweiz hat nur einen offiziellen Nationalfeiertag: den 1. August, die Fête nationale. Alles andere sind kantonale Feiertage, und die Unterschiede zwischen den einzelnen Kantonen können unterschiedlich sein...",
        keywords: "Ferien, Schweiz, abhängig, Kanton, arbeiten, leben, haben, allein",
        keywordsLocalized: true,
        category: "Leben in der Schweiz",
        hub: "vivir-en-suiza",
        slug: "actualidad-2026-04-24-3-los-festivos-en-suiza-dependen-del-canton-donde-trabajas-no-de-donde-vives",
        readingTime: 2,
        dateUpdated: "24 April 2026",
        featuredImage: "media/guides/actualidad-festivos-cantones-suiza.jpg",
        imageAlt: "Feiertagskalender nach Kantonen in der Schweiz",
        summary: "Die Schweiz hat nur einen offiziellen Nationalfeiertag: den 1. August, die Fête nationale. Alles andere sind kantonale Feiertage, und der Unterschied zwischen einem Kanton und oder...",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>Die Schweiz hat nur einen offiziellen Nationalfeiertag: den 1. August, die Fête nationale. Alles andere sind kantonale Feiertage, und der Unterschied zwischen den einzelnen Kantonen kann mehrere Tage im Jahr betragen. Genf zum Beispiel hat 9 bezahlte Feiertage, darunter den Himmelfahrtsdonnerstag, die Wiederherstellung der Republik am 31. Dezember oder das „Jeûne genevois“ im September. Im Kanton Waadt gibt es eine andere Liste. Bern, Zürich, Basel, jedes hat seine eigenen Daten, viele davon sind lokalen religiösen oder historischen Ursprungs. Für jemanden, der aus Spanien anreist, wo nationale Feiertage mit regionalen Varianten ziemlich standardisiert sind, kann dies zunächst verwirrend sein.</p>
<p>Das Wichtigste aus beruflicher Sicht ist, dass die für Sie geltenden Feiertage davon abhängen, wo sich Ihr Arbeitsplatz befindet, und nicht davon, wo Sie wohnen. Wenn Sie im Kanton Waadt wohnen, aber in Genf arbeiten, haben Sie Anspruch auf Genfer Ferien. Wenn Sie als Grenzgänger von Frankreich in den Kanton Genf arbeiten, gilt dasselbe: Die geltenden Feiertage sind die Schweizer Feiertage des Kantons, in dem Sie arbeiten, nicht die französischen auf der Grenzseite. Dies ist nicht immer bekannt und kann zu Missverständnissen mit Arbeitgebern führen, insbesondere in Unternehmen, deren Arbeitnehmer über mehrere Kantone verteilt sind.</p>
<p>Welche Feiertage genau für Sie gelten, erfahren Sie am zuverlässigsten auf der Website des Kantons, in dem Sie arbeiten, oder direkt bei der Personalabteilung. Viele Kollektivverträge (KVG) in der Schweiz sehen über das gesetzliche Minimum hinausgehende Feiertage vor oder legen fest, wie Feiertage, die auf einen Samstag oder Sonntag fallen, behandelt werden. Es kann auch Unterschiede zwischen Gemeinden innerhalb desselben Kantons geben, insbesondere in Orten mit einer starken religiösen Tradition. Nicht davon auszugehen, dass die Feiertage in der Schweiz die gleichen sind wie in Spanien, oder auch nur, dass sie im Nachbarkanton gleich sind, ist der erste Schritt, um eine unangenehme Überraschung zu vermeiden, wenn einem gesagt wird, dass der Montag, den man für frei gehalten hat, ein normaler Arbeitstag war.</p>
</div>`
    },

    "fb-daily-20260425-01-la-nueva-asignacion-de-guarderia-en-suiza-lo-que-cambia-para-familias-trabajador": {
        title: "Das neue Kinderbetreuungsgeld in der Schweiz: Was sich für berufstätige Familien ändert",
        description: "Im Jahr 2026 genehmigte das Schweizer Parlament eine neue Zuteilung für Familien mit Kindern in Kindertagesstätten, die so genannte Allocation de Garde, die in Kraft treten wird, wenn die aktuelle F…",
        keywords: "neu, Aufgabe, Kindertagesstätte, Schweiz, Veränderungen, Familien, Arbeitnehmer, Parlament",
        keywordsLocalized: true,
        category: "Leben in der Schweiz",
        hub: "vivir-en-suiza",
        slug: "actualidad-2026-04-25-1-la-nueva-asignacion-de-guarderia-en-suiza-lo-que-cambia-para-familias-trabajador",
        readingTime: 2,
        dateUpdated: "25 April 2026",
        featuredImage: "media/guides/actualidad-guarderia-asignacion-suiza.jpg",
        imageAlt: "Familie mit Kind in subventionierter Kindertagesstätte in der Schweiz",
        summary: "Im Jahr 2026 genehmigte das Schweizer Parlament eine neue Zuteilung für Familien mit Kindern in Kindertagesstätten, die so genannte Allocation de Garde, die nach Ablauf ihrer Gültigkeit in Kraft treten wird …",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>Im Jahr 2026 genehmigte das Schweizer Parlament eine neue Zuweisung für Familien mit Kindern in Kindertagesstätten, die Allokation de Garde, die in Kraft treten wird, wenn das aktuelle Bundesprogramm zur Unterstützung der ausserfamiliären Betreuung Ende 2026 ausläuft. Die Änderung hat echte Tragweite: Anstatt Pflegestrukturen direkt zu subventionieren, wird das System Familien direkt auf der Grundlage der Zeit finanzieren, die ihre Kinder in einer anerkannten Kindertagesstätte verbringen.</p>
<p>Der Mindestbetrag beträgt 100 Franken pro Monat für einen Betreuungstag pro Woche, mit einer Erhöhung um 50 Franken für jeden weiteren halben Tag. Bei Kindern mit Behinderung vervielfacht sich der Betrag zwischen dem 1,5- und 2-fachen, wenn die tatsächlichen Pflegekosten verhältnismäßig höher sind. Die Regelung gilt bis zum Ende des Monats, in dem das Kind sein achtes Lebensjahr vollendet, also genau dann, wenn in den meisten Kantonen üblicherweise der reguläre Schulunterricht beginnt.</p>
<p>Voraussetzung ist, dass beide Elternteile einer bezahlten Tätigkeit nachgehen oder eine Ausbildung absolvieren. Dies ist für spanische Familien relevant, in denen einer von beiden über die Rückkehr in den Beruf nachdenkt: Die Beihilfe gilt nicht für diejenigen, die aus Bequemlichkeitsgründen ein Kind in der Kindertagesstätte haben, sondern für diejenigen, die arbeiten oder eine Ausbildung machen. Die Berechnung und endgültige Verteilung hängt vom jeweiligen Kanton ab, der wie bereits bei den klassischen Familienzulagen seine eigenen Regeln über dem eidgenössischen Minimum festlegt.</p>
<p>Für diejenigen, die kleine Kinder haben und schon länger die Preise der Schweizer Kitas in Kauf nehmen, die in Städten wie Zürich oder Genf leicht über 2000 Franken pro Monat liegen können, löst diese Massnahme zwar nicht das Grundproblem, ist aber ein konkreter Schritt. Wenn Sie bei Ihrem Arbeitgeber noch keine gewöhnlichen Familienzulagen beantragt haben, beginnen Sie dort. Der Rest wird kommen.</p>
</div>`
    },

    "fb-daily-20260425-02-el-mercado-laboral-suizo-en-2026-que-sectores-contratan-y-cuales-estan-ajustando": {
        title: "Der Schweizer Arbeitsmarkt im Jahr 2026: Welche Branchen stellen ein und welche passen sich an",
        description: "Der Schweizer Arbeitsmarkt ist nicht mehr der nahezu wolkenlose Hafen wie vor drei, vier Jahren. Der Wirtschaftsminister prognostiziert ein BIP-Wachstum von 1,1 % im Jahr 2…",
        keywords: "Markt, Arbeit, Schweiz, 2026, Sektoren, Miete, die sind",
        keywordsLocalized: true,
        category: "Arbeit und Löhne",
        hub: "trabajo",
        slug: "actualidad-2026-04-25-2-el-mercado-laboral-suizo-en-2026-que-sectores-contratan-y-cuales-estan-ajustando",
        readingTime: 2,
        dateUpdated: "25 April 2026",
        featuredImage: "media/guides/actualidad-mercado-laboral-suiza-2026.jpg",
        imageAlt: "Schweizer Arbeitsmarkt im Jahr 2026, Branchen mit Beschäftigung",
        summary: "Der Schweizer Arbeitsmarkt ist nicht mehr der nahezu wolkenlose Hafen wie vor drei, vier Jahren. Der Wirtschaftsminister prognostiziert ein Wachstum von…",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>Der Schweizer Arbeitsmarkt ist nicht mehr der nahezu wolkenlose Hafen wie vor drei, vier Jahren. Der Wirtschaftsminister prognostiziert für 2026 ein BIP-Wachstum von 1,1 %, und die Arbeitslosigkeit wächst seit Monaten stärker als im Rest Europas, was in einem Land, das an Quoten unter 3 % gewöhnt ist, nicht üblich ist. Die Zahl der offenen Stellen, die im Jahr 2022 bei über 130.000 lag, ist auf unter 90.000 gesunken. Es ist keine Krise, aber eine sichtbare Abkühlung.</p>
<p>Die am stärksten von den Kürzungen betroffenen Sektoren sind die Pharmaindustrie, auf die fast 30 % der im Jahr 2025 verzeichneten Entlassungen entfielen, und der Finanzdienstleistungsbereich, der immer noch unter der Integration der Credit Suisse in die UBS leidet. Zwei Sektoren, die einst ein Synonym für stabile und gut bezahlte Beschäftigung waren und die sich heute einer Umstrukturierung unterziehen, die noch nicht abgeschlossen ist. Für jemanden, der mit einem technischen oder administrativen Profil in die Schweiz kommt und auf der Suche nach einer Stelle in diesen Bereichen ist, ist es ratsam, realistisch in Bezug auf Fristen und Wettbewerb zu sein.</p>
<p>Gleichzeitig ist der Arbeitskräftemangel nicht verschwunden, sondern hat sich lediglich verlagert. Das Gesundheitswesen, das Baugewerbe und die Gastronomie sind weiterhin auf der Suche nach Arbeitskräften und angesichts der demografischen Entwicklung der Schweiz wird sich dieser Trend in den kommenden Jahren noch verstärken. Es handelt sich um Branchen mit wichtigen sprachlichen Anforderungen, aber auch mit geschützten kantonalen Mindestlöhnen, die den Unterschied zu den in Spanien geltenden Gehältern ausmachen.</p>
<p>Die praktische Moral ist einfach: Wer heute zum Arbeiten in die Schweiz kommt, muss wissen, in welcher Branche er landet. Die vor zwei Jahren verbreitete Aussage „Es gibt überall Jobs“ trifft die Realität nicht mehr ganz genau. Vorab gut informiert zu sein, mit aktualisierten Daten und ohne vertrauenswürdige Aussagen von vor drei Jahren, ist notwendiger denn je.</p>
</div>`
    },

    "fb-daily-20260425-03-el-convenio-de-doble-imposicion-espana-suiza-por-que-importa-y-cuando-te-afecta": {
        title: "Das Doppelbesteuerungsabkommen zwischen Spanien und der Schweiz: Warum es wichtig ist und wann es Sie betrifft",
        description: "Eines der häufigsten Missverständnisse unter Spaniern, die in der Schweiz leben, besteht darin, zu glauben, dass sie durch ihren Wohnsitz und die Zahlung von Steuern automatisch von jeglicher Steuerpflicht befreit sind...",
        keywords: "Vereinbarung, Doppel, Besteuerung, Spanien, Schweiz, Angelegenheiten, Auswirkungen, Missverständnisse",
        keywordsLocalized: true,
        category: "Steuern",
        hub: "impuestos",
        slug: "actualidad-2026-04-25-3-el-convenio-de-doble-imposicion-espana-suiza-por-que-importa-y-cuando-te-afecta",
        readingTime: 2,
        dateUpdated: "25 April 2026",
        featuredImage: "media/guides/actualidad-doble-imposicion-espana-suiza.jpg",
        imageAlt: "Doppelbesteuerungsabkommen zwischen Spanien und der Schweiz",
        summary: "Eines der häufigsten Missverständnisse unter Spaniern, die in der Schweiz leben, besteht darin, zu glauben, dass sie durch ihren Wohnsitz und die Zahlung von Steuern automatisch von allen Steuern befreit sind.",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>Eines der häufigsten Missverständnisse unter Spaniern, die in der Schweiz leben, besteht darin, zu glauben, dass sie durch ihren Wohnsitz und die Zahlung von Steuern automatisch von jeglicher Steuerpflicht in Spanien befreit sind. Die Realität ist differenzierter, und das zwischen beiden Ländern unterzeichnete Doppelbesteuerungsabkommen, das seit 1966 in Kraft ist und 2013 aktualisiert wurde, ist der Rahmen, der bestimmt, wer das Recht hat, was und unter welchen Bedingungen zu besteuern. Es zu ignorieren kann kostspielig sein.</p>
<p>Das Abkommen legt fest, dass Einkünfte aus Arbeit grundsätzlich in dem Land besteuert werden, in dem die Tätigkeit ausgeübt wird. Wenn Sie in der Schweiz arbeiten, zahlen Sie hier. Aber es gibt Ausnahmen. Renten aus Spanien öffentlicher Herkunft können in Spanien weiterhin besteuert werden, auch wenn Sie sie in der Schweiz beziehen. Die Vermietung einer Immobilie in Spanien wird in Spanien besteuert. Dividenden oder Zinsen von spanischen Konten oder Anlagen können zu Quelleneinbehalten führen. Wenn Sie über Vermögen oder Einkünfte in Spanien verfügen und gleichzeitig in der Schweiz steuerlich ansässig sind, ist es wichtig zu wissen, was Sie wo deklarieren müssen, um nicht doppelt zu zahlen, aber auch, um Ihre Schulden nicht zu versäumen.</p>
<p>Der Schlüsselbegriff ist der der „steuerlichen Ansässigkeit“. Für die Schweiz reicht es aus, hier mit einer Aufenthaltserlaubnis zu leben und zu arbeiten, um als Steuerinländer zu gelten. Wenn Sie sich in Spanien mehr als 183 Tage im Jahr außerhalb des Landes aufhalten, verlieren Sie grundsätzlich Ihre steuerliche Ansässigkeit in Spanien. Allerdings kann das Finanzministerium dagegen Einspruch erheben, wenn es der Ansicht ist, dass Sie Ihren „Mittelpunkt Ihrer Lebensinteressen“ in Spanien haben. Dies kommt häufiger vor, als es den Anschein hat, in Familien, in denen einer der Ehegatten noch in Spanien lebt.</p>
<p>Der sicherste Weg, Überraschungen zu vermeiden, ist die Konsultation eines Steuerberaters, der beide Systeme kennt. Es ist nicht teuer und kann rückwirkende Ansprüche vermeiden. Der Bund existiert, um Sie zu schützen, aber nur, wenn Sie wissen, dass er existiert und verstehen, wie er funktioniert.</p>
</div>`
    },

    "fb-daily-20260426-01-la-e-id-suiza-el-dni-digital-que-tambien-pueden-pedir-los-que-vivimos-aqui-con-p": {
        title: "Die Schweizer E-ID: die digitale DNI, die auch diejenigen von uns beantragen können, die hier mit einer Erlaubnis leben",
        description: "Irgendwann im dritten Quartal 2026 wird die Schweiz ihr nationales digitales Identitätssystem offiziell einführen. Die Anwendung heißt Swiyu, sie existiert bereits in…",
        keywords: "Schweiz, auch digital, man kann fragen, wir leben hier, Erlaubnis",
        keywordsLocalized: true,
        category: "Formalitäten und Bewilligungen",
        hub: "tramites",
        slug: "actualidad-2026-04-26-1-la-e-id-suiza-el-dni-digital-que-tambien-pueden-pedir-los-que-vivimos-aqui-con-p",
        readingTime: 1,
        dateUpdated: "26 April 2026",
        featuredImage: "media/guides/actualidad-eid-identidad-digital-suiza.jpg",
        imageAlt: "Digitale Schweizer E-ID-Identität für Einwohner mit Aufenthaltserlaubnis",
        summary: "Irgendwann im dritten Quartal 2026 wird die Schweiz ihr nationales digitales Identitätssystem offiziell einführen. Die Anwendung heißt Swiyu,…",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>Irgendwann im dritten Quartal 2026 wird die Schweiz ihr nationales digitales Identitätssystem offiziell einführen. Die Anwendung heißt Swiyu, sie existiert bereits in der Beta-Version und was viele nicht wissen: Sie ist nicht nur Schweizer Bürgern vorbehalten. Wer über eine in der Schweiz ausgestellte gültige Aufenthaltsbewilligung verfügt, egal ob Bewilligung B oder C, kann seine digitale Identität genauso beantragen wie ein Schweizer Passinhaber.</p>
<p>Der geplante Ablauf ist einfach: Laden Sie die App auf Ihr Handy herunter, scannen Sie das in der Schweiz ausgestellte Ausweisdokument und fügen Sie ein Foto hinzu. Die digitale Identität wird ausschließlich auf dem Gerät des Nutzers gespeichert, ohne eine zentrale Datenbank. Es wird völlig freiwillig und kostenlos sein, was betont werden sollte, bevor Gerüchte über Gebühren oder komplizierte Verfahren in Umlauf kommen.</p>
<p>Der praktische Nutzen hängt davon ab, wie viele öffentliche und private Dienste es als gültiges Ausweisdokument akzeptieren. In den ersten Monaten wird es insbesondere für Online-Verwaltungsverfahren und den Zugriff auf Plattformen nützlich sein, die bereits eine Identitätsprüfung erfordern. Für internationale Reisen ist weiterhin ein Reisepass oder ein Personalausweis erforderlich.</p>
<p>Wenn Sie über eine Aufenthaltsbewilligung in der Schweiz verfügen und sich fragen, ob diese für Sie das Richtige ist, lautet die Antwort „Ja“. Es gibt keinen Grund zu warten. Wenn das System die Beta-Phase verlässt und offiziell betriebsbereit ist, lohnt es sich, es zu bearbeiten. Es ist ein weiteres Verfahren, das gelöst werden muss.</p>
</div>`
    },

    "fb-daily-20260426-02-el-ees-ya-funciona-en-toda-europa-lo-que-implica-si-tienes-familia-que-viene-de-": {
        title: "Das EES funktioniert bereits in ganz Europa: Was es bedeutet, wenn Ihre Familie von außerhalb des Schengen-Raums kommt",
        description: "Seit dem 10. April 2026 ist das Einreise- und Ausreisesystem der Europäischen Union im gesamten Schengen-Raum, einschließlich der Schweiz, voll funktionsfähig. Es lohnt sich, es von Anfang an zu klären …",
        keywords: "Es funktioniert, alles, Europa, impliziert, Sie, haben, Familie, kommen, raus",
        keywordsLocalized: true,
        category: "Formalitäten und Bewilligungen",
        hub: "tramites",
        slug: "actualidad-2026-04-26-2-el-ees-ya-funciona-en-toda-europa-lo-que-implica-si-tienes-familia-que-viene-de-",
        readingTime: 2,
        dateUpdated: "26 April 2026",
        featuredImage: "media/guides/actualidad-ees-sistema-entrada-europa.jpg",
        imageAlt: "Sistema EES de entrada y salida en fronteras europeas Schengen",
        summary: "Seit dem 10. April 2026 ist das Einreise- und Ausreisesystem der Europäischen Union im gesamten Schengen-Raum, einschließlich der Schweiz, voll funktionsfähig. Es ist praktisch…",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>Seit dem 10. April 2026 ist das Einreise- und Ausreisesystem der Europäischen Union im gesamten Schengen-Raum, einschließlich der Schweiz, voll funktionsfähig. Es ist von vornherein klarzustellen: Von diesem System sind weder Bürger der Europäischen Union noch Personen mit Wohnsitz in der Schweiz mit einer Aufenthaltserlaubnis betroffen. Wenn Sie Spanier sind und hier leben, bleiben Ihre Grenzübergänge genau die gleichen wie zuvor.</p>
<p>Eine wirkliche Änderung gibt es für alle, die von außerhalb des Schengen-Raums mit einem Nicht-EU-Pass einreisen. Das EES erfasst automatisch Ein- und Ausgänge, macht ein Foto und erfasst Fingerabdrücke. Es handelt sich um ein biometrisches System, das das einfache Abstempeln des Reisepasses ersetzt. Das erklärte Ziel besteht darin, die Aufenthaltsdauer zu kontrollieren und diejenigen zu erkennen, die länger als die erlaubten 90 Tage bleiben.</p>
<p>Für diejenigen von uns, die hier leben, ist es die unmittelbarste Auswirkung, wenn Familie oder Freunde, die keine europäischen Staatsbürger sind, uns besuchen kommen. Kommt Ihre Mutter aus Kolumbien, kommt eine Freundin aus Marokko oder haben Sie einen Partner aus einem Land, das nicht zur EU oder EFTA gehört, fallen diese in das EES-System. Beim ersten Grenzübertritt dauert es länger als üblich, da die biometrischen Daten registriert werden müssen. Es ist ratsam, sie im Voraus zu benachrichtigen, damit sie am Flughafen keinen Schrecken bekommen.</p>
<p>Das System selbst ändert weder die Visagebühren noch die Anzahl der erlaubten Aufenthaltstage. Was sich ändert, ist die Steuerung, die jetzt automatisiert und systematisch erfolgt. Von einem rechtmäßigen und korrekten Aufenthalt gibt es nichts zu befürchten. Für diejenigen, die dies nicht tun, wurde die Marge erheblich reduziert.</p>
</div>`
    },

    "fb-daily-20260426-03-comprar-piso-en-suiza-siendo-espanol-lo-que-depende-de-tu-permiso-y-lo-que-no": {
        title: "Als Spanier eine Wohnung in der Schweiz kaufen: Was hängt von Ihrer Bewilligung ab und was nicht",
        description: "Letzte Woche kündigte die Schweizer Regierung an, dass sie die Bedingungen für den Erwerb von Wohneigentum durch Nicht-EU-Ausländer im Land verschärfen wird. Der Besitzer reiste mehrere…",
        keywords: "kaufen, Wohnung, Schweiz, Sein, Spanisch, hängt davon ab, Genehmigung, Woche",
        keywordsLocalized: true,
        category: "Wohnen",
        hub: "vivienda",
        slug: "actualidad-2026-04-26-3-comprar-piso-en-suiza-siendo-espanol-lo-que-depende-de-tu-permiso-y-lo-que-no",
        readingTime: 2,
        dateUpdated: "26 April 2026",
        featuredImage: "media/guides/actualidad-comprar-piso-suiza-espanol.jpg",
        imageAlt: "Kauf einer Wohnung in der Schweiz durch einen spanischen Staatsbürger mit Genehmigung",
        summary: "Letzte Woche kündigte die Schweizer Regierung an, dass sie die Bedingungen für den Erwerb von Wohneigentum durch Nicht-EU-Ausländer im Land verschärfen wird. Der…",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>Letzte Woche kündigte die Schweizer Regierung an, dass sie die Bedingungen für den Erwerb von Wohneigentum durch Nicht-EU-Ausländer im Land verschärfen wird. Die Schlagzeile verbreitete sich dramatischer als nötig durch mehrere spanische Medien. Die Realität für einen Spanier mit einer Aufenthaltserlaubnis in der Schweiz sieht ganz anders aus, als diese Art von Nachrichten vermuten lassen.</p>
<p>Die Schweizer Regelung, die den Erwerb von Immobilien durch Ausländer regelt, heißt Lex Koller und besteht seit Jahrzehnten. Es wird eine klare Unterscheidung je nach Art der Genehmigung vorgenommen. Wenn Sie über eine C-Bewilligung verfügen, haben Sie genau die gleichen Rechte wie ein Schweizer Bürger: Sie können ein Hauptwohnsitz, eine Anlageliegenschaft oder eine beliebige Immobilie frei kaufen, ohne dass eine besondere Genehmigung erforderlich ist. Wenn Sie über eine B-Bewilligung verfügen, ist die Situation eingeschränkter: Sie können ein Haupthaus zur Eigennutzung erwerben, benötigen jedoch eine Bewilligung des Kantons, in dem sich die Immobilie befindet, und die Immobilie darf nicht als Kapitalanlage genutzt oder zur Vermietung angeboten werden.</p>
<p>Die angekündigten neuen Maßnahmen richten sich speziell an Bürger von Ländern, die weder EU noch EFTA sind. Sie wären als Einzige verpflichtet, eine vorherige Genehmigung einzuholen und die Immobilie innerhalb einer Frist von zwei Jahren weiterzuverkaufen, wenn sie das Land verlassen. Für einen Spanier, der EU-Bürger ist und dessen Freizügigkeit durch bilaterale Abkommen garantiert ist, gelten diese Einschränkungen nicht.</p>
<p>Abgesehen davon heißt es nicht, dass es einfach ist, nur weil man es kaufen kann. Der Schweizer Immobilienmarkt ist einer der teuersten und wettbewerbsintensivsten in Europa, die Einstiegspreise in Städten wie Genf oder Zürich sind hoch und die Hypothekenkonditionen erfordern in der Regel eine Eigenbeteiligung von mindestens 20 %. Das Hindernis ist nicht rechtlicher Natur, sondern wirtschaftlicher Natur. Und das löst die Lex Koller nicht.</p>
</div>`
    },

    "fb-daily-20260426-04-empadronarse-en-suiza-el-primer-tramite-que-muchos-dejan-para-despues": {
        title: "Anmeldung in der Schweiz: der erste Vorgang, den viele auf später verschieben",
        description: "In der Schweiz ankommen, sich einleben, mit der Arbeit beginnen, eine Kita für die Kinder suchen... Und die Anmeldung beim Einwohnermeldeamt steht noch aus, „wenn es Zeit ist“. UND…",
        keywords: "Registrieren, Schweiz, zuerst, verarbeiten, viele, gehen, dann ankommen",
        keywordsLocalized: true,
        category: "Formalitäten und Bewilligungen",
        hub: "tramites",
        slug: "actualidad-2026-04-26-4-empadronarse-en-suiza-el-primer-tramite-que-muchos-dejan-para-despues",
        readingTime: 1,
        dateUpdated: "26 April 2026",
        featuredImage: "media/guides/actualidad-empadronamiento-suiza.jpg",
        imageAlt: "Registro municipal Anmeldung en Suiza",
        summary: "In der Schweiz ankommen, sich einleben, mit der Arbeit beginnen, eine Kita für die Kinder suchen... Und die Anmeldung beim Gemeindeamt steht noch aus für „wann…“",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>In der Schweiz ankommen, sich einleben, mit der Arbeit beginnen, eine Kita für die Kinder suchen... Und die Anmeldung beim Einwohnermeldeamt steht noch aus, „wenn es Zeit ist“. Das Problem ist, dass dieses Verfahren eine gesetzliche Frist hat: in den meisten Kantonen 14 Tage ab Ankunft. In Genf, 8 Tage. Es handelt sich hierbei nicht um eine Empfehlung, sondern um eine Verpflichtung. Bei Nichteinhaltung kann eine Geldstrafe verhängt werden.</p>
<p>Das Amt heißt je nach Sprachregion: Contrôle des habitants in der Romandie, Einwohnerkontrolle in der Deutschschweiz, Controllo degli abitanti im Tessin. Das Verfahren findet persönlich statt und erfordert einen Reisepass oder Personalausweis, einen unterschriebenen Arbeits- oder Mietvertrag sowie ein aktuelles Lichtbild. In vielen Kommunen können Sie online einen Termin vereinbaren. Dort meldet die Gemeinde das kantonale Migrationsamt, welches für die Bearbeitung der Aufenthaltsbewilligung zuständig ist. Ohne kommunale Anmeldung besteht die Genehmigung nicht.</p>
<p>Was viele verwirren, ist die Reihenfolge des Prozesses: Zuerst meldet man sich bei der Gemeinde an und die Genehmigung kommt später, meist per Post in Form einer Karte. Es handelt sich nicht um ein separates Verfahren, das Sie alleine durchführen müssen. Es handelt sich um denselben Verwaltungsakt. In manchen Kantonen wird die Bewilligung sofort zugestellt, in anderen kann die Frist mehrere Wochen betragen.</p>
<p>Die Anmeldung bei der Gemeinde ist auch der Ausgangspunkt für praktisch alles andere: die Eröffnung eines Bankkontos, die Anmeldung der Kinder in der Schule, den Zugang zu bestimmten öffentlichen Dienstleistungen. Ohne diese offiziell anerkannte Einwohnerzahl bleiben viele Türen stecken. Es lohnt sich, es richtig und pünktlich zu machen.</p>
</div>`
    },

    "fb-daily-20260426-05-el-salario-minimo-en-suiza-que-cantones-lo-fijan-y-que-pasa-en-los-que-no": {
        title: "Der Mindestlohn in der Schweiz: Welche Kantone legen ihn fest und was passiert in denen, in denen dies nicht der Fall ist?",
        description: "In der Schweiz gibt es keinen eidgenössischen Mindestlohn. Dies überrascht viele Menschen, die in das Land kommen, mit der Vorstellung, dass es im gesamten Gebiet eine einzige Gehaltsuntergrenze gibt. Es ist nicht…",
        keywords: "Gehalt, Mindestgehalt, Schweiz, Kantone, festlegen, bestehen, haben, föderal",
        keywordsLocalized: true,
        category: "Arbeit und Löhne",
        hub: "trabajo",
        slug: "actualidad-2026-04-26-5-el-salario-minimo-en-suiza-que-cantones-lo-fijan-y-que-pasa-en-los-que-no",
        readingTime: 2,
        dateUpdated: "26 April 2026",
        featuredImage: "media/guides/actualidad-salario-minimo-canton-suiza.jpg",
        imageAlt: "Mindestlohn nach Kanton in der Schweiz",
        summary: "In der Schweiz gibt es keinen eidgenössischen Mindestlohn. Das überrascht viele Menschen, die ins Land kommen, mit der Vorstellung, dass es eine einzige Gehaltsuntergrenze gibt, die für alles gilt ...",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>In der Schweiz gibt es keinen eidgenössischen Mindestlohn. Dies überrascht viele Menschen, die in das Land kommen, mit der Vorstellung, dass es im gesamten Gebiet eine einzige Gehaltsuntergrenze gibt. So ist es nicht. Der Bund hat diese Entscheidung den Kantonen und den Gesamtarbeitsverträgen überlassen, und das Ergebnis ist eine Karte, die vor der Unterzeichnung eines Vertrags bekannt sein sollte.</p>
<p>Nur fünf Kantone haben einen gesetzlichen Mindestlohn festgelegt: Genf, Neuenburg, Jura, Basel-Stadt und Tessin. Genf liegt mit 24,59 Franken pro Stunde im Jahr 2026 an der Spitze, was knapp über 4.400 Franken brutto pro Monat für einen ganzen Tag entspricht und offiziell den höchsten Mindestlohn der Welt darstellt. In den anderen vier Kantonen sind die Zahlen etwas kleiner, aber auch signifikant. In allen anderen Schweizer Kantonen ergibt sich die Lohnuntergrenze nicht aus dem Gesetz, sondern aus Branchenkollektivverträgen.</p>
<p>Tarifverträge, auf Französisch CCT oder auf Deutsch GAV genannt, decken ganze Sektoren ab: Baugewerbe, Gastgewerbe, Reinigung, Handel, Haushaltsdienstleistungen und andere. Wenn ein CCT als allgemein verbindlich erklärt wird, ist der Arbeitgeber gesetzlich verpflichtet, die von ihm festgelegten Mindestlöhne anzuwenden, unabhängig davon, ob er einem Arbeitgeberverband angeschlossen ist oder nicht. Das Problem besteht darin, dass nicht alle Branchen über ein CCT verfügen und nicht alle Vereinbarungen allgemein verbindlich sind. Wenn Sie in einer Branche ohne Absicherung und in einem Kanton ohne gesetzlichen Mindestlohn arbeiten, gibt es keinen formellen Mindestlohn.</p>
<p>Die praktische Schlussfolgerung ist einfach: Wenn Ihnen ein Gehalt angeboten wird, das Ihnen niedrig erscheint, müssen Sie zunächst prüfen, ob eine CCT vorliegt, die Ihre Tätigkeit abdeckt. Das SECO, Staatssekretär für Wirtschaft, veröffentlicht alle aktuellen Vereinbarungen. Das überprüfen auch die Gewerkschaften. Und wenn es einen Verstoß gibt, gibt es einen administrativen Weg zur Beschwerde. Es kommt zwar nicht häufig vor, kommt aber vor, insbesondere im häuslichen Bereich, im Gastgewerbe und bei Reinigungsdiensten, wo die Informationen schlechter ankommen.</p>
</div>`
    },

    "fb-daily-20260426-06-rc-y-seguro-del-hogar-en-suiza-dos-seguros-distintos-que-van-juntos-pero-no-son-": {
        title: "Haftpflicht- und Hausratversicherung in der Schweiz: zwei verschiedene Versicherungen, die zusammenpassen, aber nicht gleich sind",
        description: "Wenn jemand in der Schweiz ankommt und seinen ersten Mietvertrag unterschreibt, fragt er fast immer nach einer Versicherung. Was genau sie verlangen und warum, ist nicht immer klar. Es gibt zwei Versicherungen…",
        keywords: "sicher, Zuhause, Schweiz, Versicherung, anders, zusammen, gleich, jemand",
        keywordsLocalized: true,
        category: "Wohnen",
        hub: "vivienda",
        slug: "actualidad-2026-04-26-6-rc-y-seguro-del-hogar-en-suiza-dos-seguros-distintos-que-van-juntos-pero-no-son-",
        readingTime: 2,
        dateUpdated: "26 April 2026",
        featuredImage: "media/guides/actualidad-seguro-rc-hogar-suiza.jpg",
        imageAlt: "Haftpflicht- und Hausratversicherung in der Schweiz",
        summary: "Wenn jemand in der Schweiz ankommt und seinen ersten Mietvertrag unterschreibt, fragt er fast immer nach einer Versicherung. Was genau sie verlangen und warum, ist nicht immer klar ...",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>Wenn jemand in der Schweiz ankommt und seinen ersten Mietvertrag unterschreibt, fragt er fast immer nach einer Versicherung. Was genau sie verlangen und warum, ist nicht immer klar. Es gibt zwei Versicherungen, die üblicherweise zusammen abgeschlossen werden und die viele Menschen verwechseln, weil sie in derselben Police enthalten sind: die private Haftpflichtversicherung (responsabilité Civile privée) und die Hausratversicherung (assurance ménage). Sie sind nicht gleich, obwohl der Gesamtpreis angemessen ist.</p>
<p>Die Privathaftpflicht deckt Schäden ab, die Sie versehentlich Dritten zufügen können: ein Leck, das den Boden darunter ruiniert, ein Blumentopf, der auf die Terrasse fällt, ein Fleck, der jemandes Kleidung ruiniert. Ohne diese Versicherung zahlen Sie den Schaden aus eigener Tasche. In der Schweiz ist dies gesetzlich nicht zwingend vorgeschrieben, in der Praxis verlangt die überwiegende Mehrheit der Eigentümer sie jedoch als Voraussetzung für den Abschluss des Mietvertrags. Die Kosten sind gering: zwischen 100 und 180 CHF pro Jahr für eine Einzelperson. Die Hausratversicherung hingegen schützt das eigene Eigentum vor Diebstahl, Feuer, Wasserschäden oder Naturkatastrophen. Sie ist nur in vier Kantonen gesetzlich vorgeschrieben: Waadt, Freiburg, Jura und Nidwalden, für die Komponente Feuer und Naturphänomene. Im Übrigen ist es optional.</p>
<p>Die häufigste Verwirrung besteht darin, zu glauben, dass CR bereits die Objekte selbst abdeckt. Das ist nicht der Fall. Wenn Ihr Computer gestohlen wird, wenn ein Wasserleck Ihre Couch zerstört oder wenn ein Feuer Ihre Sachen zerstört, zahlt RC nichts davon. Dafür gibt es eine Hausratversicherung. Nur das RC-Fahrzeug und nicht den Hausrat zu mieten, ist ein Fehler, den viele Menschen zur falschen Zeit entdecken, wenn der Unfall bereits passiert ist.</p>
<p>Der praktische Ratschlag besteht darin, beides in derselben Police abzuschließen. Der Preisunterschied zwischen einem oder beiden beträgt in der Regel 50 bis 100 Franken pro Jahr und die Absicherung ist qualitativ unterschiedlich. Unternehmen wie Zürich, AXA, Generali, Mobilière oder Helvetia bieten Standardkombinationen an. Der Vergleich erfolgt am besten über das Portal comparais.ch, das die Angebote in Echtzeit zusammenfasst. Und wenn Sie das eine bereits haben, das andere jedoch nicht, lohnt es sich, die Richtlinie zu überprüfen, bevor etwas passiert, das Sie bereuen lässt.</p>
</div>`
    },

    "fb-daily-20260426-07-el-teletrabajo-desde-espana-siendo-residente-en-suiza-las-reglas-que-ya-son-defi": {
        title: "Telearbeit von Spanien aus bei gleichzeitigem Aufenthalt in der Schweiz: die nun endgültigen Regeln",
        description: "Per 1. Januar 2026 ist der Rahmen zur Regelung der grenzüberschreitenden Telearbeit zwischen der Schweiz und den unterzeichnenden europäischen Ländern in Kraft getreten. In den vergangenen Jahren…",
        keywords: "Telearbeit, Spanien, Sein, Bewohner, Schweiz, Regeln, endgültig, Januar",
        keywordsLocalized: true,
        category: "Arbeit und Löhne",
        hub: "trabajo",
        slug: "actualidad-2026-04-26-7-el-teletrabajo-desde-espana-siendo-residente-en-suiza-las-reglas-que-ya-son-defi",
        readingTime: 2,
        dateUpdated: "26 April 2026",
        featuredImage: "media/guides/actualidad-teletrabajo-espana-suiza.jpg",
        imageAlt: "Telearbeit aus Spanien für Einwohner der Schweiz",
        summary: "Per 1. Januar 2026 ist der Rahmen zur Regelung der grenzüberschreitenden Telearbeit zwischen der Schweiz und den unterzeichnenden europäischen Ländern in Kraft getreten. Während...",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>Per 1. Januar 2026 ist der Rahmen zur Regelung der grenzüberschreitenden Telearbeit zwischen der Schweiz und den unterzeichnenden europäischen Ländern in Kraft getreten. In den vergangenen Jahren fungierte es als provisorisches Regime. Mittlerweile ist es die stabile Norm, und Spanien gehört zu den Ländern, die das Abkommen unterzeichnet haben, was unmittelbare Konsequenzen für jeden Spanier hat, der in der Schweiz lebt und für einen hier niedergelassenen Arbeitgeber arbeitet.</p>
<p>Als Grundregel gilt die Schwelle von 49,9 %: Solange die Zeit der Fernarbeit im Wohnsitzland diesen Anteil an der Gesamtarbeitszeit eines Jahres nicht überschreitet, kann der Arbeitnehmer seine Zugehörigkeit zum schweizerischen Sozialversicherungssystem aufrechterhalten. In der Praxis bedeutet dies: Wenn Ihr Unternehmen in der Schweiz ansässig ist und Sie mehrere Wochen im Jahr von Spanien aus arbeiten möchten, ist dies legal und mit Ihrer Schweizer Situation vereinbar, solange Sie diese Schwelle nicht überschreiten. Mit anderen Worten: Bis fast die Hälfte des Jahres können Sie von Spanien aus Telearbeit leisten, ohne den Schweizer AVS-, BVG- oder Unfallversicherungsschutz zu verlieren.</p>
<p>Was nicht automatisch funktioniert, ist der Verwaltungsvorgang. Sowohl der Arbeitnehmer als auch der Arbeitgeber müssen die Situation der zuständigen Ausgleichskasse (deutsch: Caisse de Compensation) melden und sich die A1-Bescheinigung besorgen, das Dokument, das bescheinigt, welche Sozialversicherungsgesetze gelten. Ohne dieses Zertifikat kann die Arbeit von Spanien aus im Falle einer Inspektion oder eines Arbeitsunfalls zu Problemen führen, egal wie legitim die tatsächliche Situation auch sein mag.</p>
<p>Der fiskalische Teil ist derjenige, der noch nicht vollständig gelöst ist. Das Doppelbesteuerungsabkommen zwischen Spanien und der Schweiz sieht die Aufteilung der Steuerbefugnisse für Telearbeitstage aus Spanien nicht ausdrücklich vor. Abhängig von der Anzahl der Tage und dem Einkommen kann es eine Grauzone geben, die es wert ist, mit einem Berater besprochen zu werden, bevor man Pläne schmiedet, die längere Arbeitsperioden von der Halbinsel aus beinhalten.</p>
</div>`
    },

    "fb-daily-20260426-08-la-jubilacion-de-las-mujeres-en-suiza-lo-que-cambia-en-2026-con-la-reforma-avs-2": {
        title: "Frauenpensionierung in der Schweiz: Was sich 2026 mit der AHV-21-Reform ändert",
        description: "Die AHV-Reform „AHV 21“ wurde im September 2022 in einer Volksabstimmung angenommen und wird ab 2025 schrittweise umgesetzt. Ihre konkretesten Auswirkungen hat sie auf Frauen...",
        keywords: "Ruhestand, Frauen, Schweiz, Veränderungen, 2026, Reform, bekannt, genehmigt",
        keywordsLocalized: true,
        category: "Arbeit und Löhne",
        hub: "trabajo",
        slug: "actualidad-2026-04-26-8-la-jubilacion-de-las-mujeres-en-suiza-lo-que-cambia-en-2026-con-la-reforma-avs-2",
        readingTime: 2,
        dateUpdated: "26 April 2026",
        featuredImage: "media/guides/actualidad-jubilacion-mujeres-suiza-avs.jpg",
        imageAlt: "Pensionierung von Frauen in der Schweiz mit der Reform der AHV 21",
        summary: "Die AHV-Reform „AVS 21“ wurde im September 2022 in einer Volksabstimmung angenommen und wird seit 2025 schrittweise umgesetzt. Ihre bedeutendste Wirkung…",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>Die AHV-Reform „AVS 21“ wurde in einer Volksabstimmung im September 2022 angenommen und wird ab 2025 schrittweise umgesetzt. Ihre konkretste Auswirkung für in der Schweiz lebende und arbeitende Frauen ist eine schrittweise Anhebung des Rentenalters: von 64 auf 65 Jahre, gestaffelt zwischen 2025 und 2028. Wenn Sie eine Spanierin mit jahrelangen Beitragszahlungen in der Schweiz sind und darüber nachdenken, wann Sie an der Reihe sind, in den Ruhestand zu gehen, lautet die Antwort jetzt Es ist nicht so einfach wie „mit 64“.</p>
<p>Der genaue Zeitpunkt des Übergangs ist wie folgt: Frauen des Jahrgangs 1961 erreichten das Referenzalter im Jahr 2025 mit 64 Jahren und drei Monaten. Die im Jahr 1962 Geborenen erreichen dieses Alter im Jahr 2026 mit 64 Jahren und sechs Monaten. Wer im Jahr 1963 geboren ist, wird dies im Jahr 2027 mit 64 Jahren und neun Monaten tun. Für den Jahrgang 1964 liegt das Referenzalter nun wie für Männer bei 65 Jahren. Dabei handelt es sich nicht um einen obligatorischen Renteneintrittstermin, sondern um das Alter, ab dem die volle AHV-Rente ohne Strafe zu beziehen beginnt.</p>
<p>Frauen dieser Übergangsgenerationen erhalten eine Entschädigung in der zweiten Säule, dem BVG, um die Auswirkungen dieses zusätzlichen Arbeitsjahres abzumildern. Diese Entschädigungen erfolgen jedoch nicht in allen Fällen automatisch, sondern hängen vom geltenden Tarifvertrag und der Berufsgruppe ab. In vielen Unternehmen haben die Personalabteilungen ihre internen Kalkulationen noch nicht aktualisiert. Wenn Ihr Arbeitgeber Ihnen einen Pensionierungstermin ohne Berücksichtigung dieser Reform mitgeteilt hat, empfiehlt es sich, dies direkt bei Ihrer Pensionskasse abzuklären.</p>
<p>Eine zusätzliche Überlegung für spanische Frauen, die vor ihrer Ankunft in der Schweiz jahrelang Beiträge in Spanien geleistet haben: Die in beiden Ländern geleisteten Arbeitszeiten können zur Erfüllung der Mindestbeitragsanforderungen kombiniert werden, das in jedem System geltende Referenzalter ist jedoch unabhängig. Sie können die spanische Rente in einem anderen Alter als in der Schweiz beantragen. Dies kann sich auf die Job-Exit-Strategie auswirken, wenn dies im Voraus geplant wird.</p>
</div>`
    },

    "fb-daily-20260426-09-las-multas-en-suiza-el-sistema-por-el-que-una-infraccion-de-trafico-puede-costar": {
        title: "Bußgelder in der Schweiz: Das System, nach dem ein Verkehrsverstoß einen Monatslohn kosten kann",
        description: "Wenn man in der Schweiz lebt, stellt man relativ schnell fest, dass Bußgelder nicht wie in Spanien gelten. Sie werden nicht nur fast ihre gesamte Lebensdauer häufig und effizient aufgeladen...",
        keywords: "Bußgelder, Schweiz, System, Verstoß, Verkehr, Dose, Kosten, Gehalt",
        keywordsLocalized: true,
        category: "Leben in der Schweiz",
        hub: "vivir-en-suiza",
        slug: "actualidad-2026-04-26-9-las-multas-en-suiza-el-sistema-por-el-que-una-infraccion-de-trafico-puede-costar",
        readingTime: 2,
        dateUpdated: "26 April 2026",
        featuredImage: "media/guides/actualidad-multas-trafico-suiza.jpg",
        imageAlt: "Verkehrsstrafensystem in der Schweiz",
        summary: "Wenn man in der Schweiz lebt, stellt man relativ schnell fest, dass Bußgelder nicht wie in Spanien gelten. Sie sind nicht nur häufig und kostenintensiv …",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>Wenn man in der Schweiz lebt, stellt man relativ schnell fest, dass Bußgelder nicht wie in Spanien gelten. Sie sind nicht nur häufig und werden mit nahezu schweizerischer Effizienz angeklagt, sondern ihre Höhe wird in bestimmten Fällen auch auf der Grundlage des Einkommens des Täters berechnet. Das Ergebnis sind Zahlen, die übertrieben wirken, bis man die Logik dahinter versteht.</p>
<p>Das System heißt auf Deutsch „Tagessatz“ oder auf Französisch „taux journalistier“ und findet im Bereich des Schweizer Strafrechts Anwendung, nicht bei geringfügigen Ordnungswidrigkeiten. Wenn ein Verkehrsverstoß bestimmte Schwereschwellen überschreitet, ist er kein Bußgeld mehr und wird zu einer strafrechtlichen Sanktion. Zu diesem Zeitpunkt berechnet das Gericht oder der Staatsanwalt den Betrag, indem es eine Anzahl von „Geldstrafentagen“ (die die Schwere des Verhaltens widerspiegeln) mit dem täglichen Nettoeinkommen der verurteilten Person multipliziert. Wenn Sie 9000 Franken im Monat verdienen, beträgt Ihr tägliches Einkommen etwa 300 Franken. Dreißig Tage Strafe betragen dann 9000 Franken. Deshalb war die Nachricht über den Manager, der wegen Geschwindigkeitsüberschreitung zu einer Geldstrafe von 300.000 Franken verurteilt wurde, kein Scherz: Das System funktionierte genau so, wie es geplant war.</p>
<p>Die praktische Frage ist, wann die Grenze zwischen Ordnungswidrigkeit und Straftat überschritten wird. Für Geschwindigkeitsüberschreitungen sind die Grenzwerte in der Straßenverkehrsordnung festgelegt: Eine Überschreitung der Geschwindigkeit um mehr als 30 km/h in Zonen von 30 oder 50, um mehr als 40 km/h auf Straßen mit einer Breite von 80 oder um mehr als 50 km/h auf einer Autobahn führt zu einer schweren Straftat, die als „Délit“ eingestuft und strafrechtlich verfolgt wird. Von dort aus betritt der Tagessatz die Szene. Unterhalb dieser Grenzen ist das Bußgeld festgesetzt und nicht vom Einkommen abhängig.</p>
<p>Wichtig zu wissen ist, dass Schweizer Radargeräte stationär, mobil und immer zahlreicher sind und die Fristen für die Bearbeitung und Erhebung von Bussen streng sind. Wenn Sie eine Benachrichtigung per Post erhalten, verschwindet sie nicht, wenn Sie sie ignorieren: Das Schuldbetreibungssystem (poursuite) in der Schweiz ist effektiv und hat Auswirkungen auf Ihre Kredit- und Schuldenregisterauskunft, die viele Vermieter und Arbeitgeber konsultieren, bevor sie etwas mit Ihnen unterschreiben.</p>
</div>`
    },

    "fb-daily-20260427-01-el-acuerdo-pnr-suiza-ue-tus-datos-de-vuelo-ya-no-son-solo-tuyos": {
        title: "Das PNR-Abkommen zwischen der Schweiz und der EU: Ihre Flugdaten gehören nicht mehr nur Ihnen",
        description: "Am 5. März 2026 unterzeichneten die Schweiz und die Europäische Union ein Abkommen zum Austausch von Passagierdatensätzen (PNR) für alle kommerziellen Flüge zwischen den beiden…",
        keywords: "Vereinbarung, Schweiz, Daten, Flug, nur Ihr, März, 2026",
        keywordsLocalized: true,
        category: "Warnungen und Ressourcen",
        hub: "recursos",
        slug: "actualidad-2026-04-27-1-el-acuerdo-pnr-suiza-ue-tus-datos-de-vuelo-ya-no-son-solo-tuyos",
        readingTime: 1,
        dateUpdated: "27 April 2026",
        featuredImage: "media/guides/actualidad-acuerdo-pnr-suiza-ue.jpg",
        imageAlt: "Acuerdo PNR datos de vuelo Suiza Unión Europea",
        summary: "Am 5. März 2026 haben die Schweiz und die Europäische Union ein Abkommen zum Austausch von Passagierdatensätzen (PNR) für alle Flüge mit… unterzeichnet.",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>Am 5. März 2026 unterzeichneten die Schweiz und die Europäische Union ein Abkommen zum Austausch von Passagierdatensätzen (PNR) für alle kommerziellen Flüge zwischen beiden Gebieten. Konkret: Wenn Sie einen Flug von Genf nach Madrid buchen, übermittelt die Fluggesellschaft den Schweizer und europäischen Behörden Informationen wie Ihren Namen, die Zahlungsmethode, die Sitzplatznummer oder mit wem Sie reisen. In Europa als solchem ​​ist es kein Novum: Dieses System besteht bereits seit Jahren zwischen der EU und Ländern wie den Vereinigten Staaten, Australien oder Kanada. Was sich nun ändert, ist, dass die Schweiz beginnt, sich bidirektional zu integrieren, was bisher nicht der Fall war.</p>
<p>Erklärtes Ziel ist die Bekämpfung von Terrorismus, organisierter Kriminalität und Drogenhandel. Die Behörden argumentieren, dass durch den Abgleich dieser Daten mit Sicherheitsdatenbanken Muster erkannt werden können, bevor etwas passiert. Niemand, der das Flugzeug nimmt, um seine Familie in Valencia zu besuchen, muss sich praktisch Sorgen machen. Es ist jedoch wichtig zu wissen, dass der Raum der Anonymität im Flugverkehr zwischen der Schweiz und der EU immer kleiner wird und dieser Prozess nun über eine unterzeichnete Rechtsgrundlage verfügt.</p>
<p>Das Abkommen ist unterzeichnet, steht jedoch noch vor der formellen Ratifizierung, sowohl durch das Europäische Parlament als auch durch die Schweizer Mechanismen. Bis dieser Vorgang abgeschlossen ist, ist das System nicht vollständig betriebsbereit. Wie so vieles in der Schweiz vergeht auch das institutionelle Verfahren seiner Zeit. Aber die Richtung ist bereits festgelegt.</p>
</div>`
    },

    "fb-daily-20260427-02-alquilar-en-suiza-en-2026-cuando-el-mercado-decide-que-no-eres-prioridad": {
        title: "Mieten in der Schweiz im Jahr 2026: Wenn der Markt entscheidet, dass Sie keine Priorität haben",
        description: "Der Mietmarkt in der Schweiz ist nach wie vor einer der angespanntesten in Europa und das Jahr 2026 hat keine sichtbare Entspannung gebracht. In Zürich eine 40 bis 50 Quadratmeter große Wohnung im…",
        keywords: "Miete, Schweiz, 2026, Markt, entscheiden, Sie sind, Priorität, Miete",
        keywordsLocalized: true,
        category: "Wohnen",
        hub: "vivienda",
        slug: "actualidad-2026-04-27-2-alquilar-en-suiza-en-2026-cuando-el-mercado-decide-que-no-eres-prioridad",
        readingTime: 2,
        dateUpdated: "27 April 2026",
        featuredImage: "media/guides/actualidad-alquiler-suiza-2026.jpg",
        imageAlt: "Mietmarkt in der Schweiz im Jahr 2026",
        summary: "Der Mietmarkt in der Schweiz ist nach wie vor einer der angespanntesten in Europa und das Jahr 2026 hat keine sichtbare Entspannung gebracht. In Zürich eine Wohnung von 40 bis 50 m…",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>Der Mietmarkt in der Schweiz ist nach wie vor einer der angespanntesten in Europa und das Jahr 2026 hat keine sichtbare Entspannung gebracht. In Zürich kostet eine 40 bis 50 Quadratmeter große Wohnung im Stadtzentrum rund 2.500 Franken pro Monat. In Genf sind die Preise gleichwertig und liegen zwischen 2.000 und 2.800 Franken für ähnliche Oberflächen. Was die Situation noch komplizierter macht, ist der Relaunch-Effekt auf dem Markt: Wenn eine Wohnung in Genf den Mieter wechselt, steigt der Preis im Durchschnitt um 13,5 %. In Neuenburg beträgt der Anstieg rund 12 %, in Zürich 6 %. Mit anderen Worten: Die Wohnungen sind nicht nur teuer, sondern jedes Mal, wenn eine Wohnung frei wird, treibt der Umsatz den Preis noch weiter in die Höhe.</p>
<p>Die direkte Folge ist, dass immer mehr Menschen, die in Großstädten arbeiten, außerhalb dieser Städte leben. Der sinnvolle Suchradius für Zürich umfasst bereits ganze Kantone. Von Genf aus sind viele direkt nach Frankreich ausgereist und haben dafür die Lebenshaltungskosten in einem anderen Land übernommen und dafür weniger bezahlt. Dieser Trend hat in der Schweiz einen Namen: Zwangssuburbanisierung. Und es betrifft nicht nur Neuankömmlinge; Es gibt Menschen, die seit Jahren in derselben Wohnung wohnen und Angst vor einem Umzug haben, weil sie wissen, dass sie für diesen Preis nichts Gleichwertiges finden werden.</p>
<p>Wenn Sie gerade auf der Suche nach einer Wohnung sind, sollten Sie wissen, dass die Kantone im Landesinneren und im Alpenraum am wenigsten belastet sind: Uri, Glarus oder Appenzell haben lockerere Märkte, sind aber mit längeren Anreisen verbunden. In einem Kanton zu leben und in einem anderen zu arbeiten, ist in der Schweiz üblich und mit öffentlichen Verkehrsmitteln einfacher als in anderen Ländern. Es ist nicht die ideale Lösung, aber in einem Markt wie diesem ist es wertvoller, die tatsächlichen Alternativen zu kennen, als weiterhin nach Wohnungen in derselben Postleitzahl zu suchen.</p>
</div>`
    },

    "fb-daily-20260427-03-el-seguro-de-accidentes-en-suiza-el-que-tienes-sin-saberlo-y-que-no-es-el-seguro": {
        title: "Unfallversicherung in der Schweiz: Was Sie haben, ohne es zu wissen, und was keine Krankenversicherung ist",
        description: "Viele Menschen verwechseln die obligatorische Krankenpflegeversicherung (KVG) mit der Unfallversicherung (GKV). Es handelt sich um zwei völlig unterschiedliche Dinge, die von unterschiedlichen Systemen abgedeckt werden, und das Verständnis der…",
        keywords: "Versicherung, Unfälle, Schweiz, Sie haben, wissen Sie, Arzt, viele, Menschen",
        keywordsLocalized: true,
        category: "Gesundheit und KVG",
        hub: "salud",
        slug: "actualidad-2026-04-27-3-el-seguro-de-accidentes-en-suiza-el-que-tienes-sin-saberlo-y-que-no-es-el-seguro",
        readingTime: 2,
        dateUpdated: "27 April 2026",
        featuredImage: "media/guides/actualidad-seguro-accidentes-suiza-laa.jpg",
        imageAlt: "Obligatorische Unfallversicherung der UVG in der Schweiz",
        summary: "Viele Menschen verwechseln die obligatorische Krankenpflegeversicherung (KVG) mit der Unfallversicherung (GKV). Es sind zwei völlig unterschiedliche Dinge, die von unterschiedlichen Systemen abgedeckt werden …",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>Viele Menschen verwechseln die obligatorische Krankenpflegeversicherung (KVG) mit der Unfallversicherung (GKV). Es handelt sich um zwei völlig unterschiedliche Dinge, die von unterschiedlichen Systemen abgedeckt werden, und wenn Sie in der Schweiz arbeiten, ist es nicht optional, den Unterschied zu verstehen. Das UVG ist für alle Arbeitnehmer verpflichtend. Für Arbeits- und Wegeunfälle kommt der Arbeitgeber voll auf. Bei Nichtberufsunfällen besteht Versicherungsschutz auch dann, wenn Sie mehr als acht Stunden pro Woche beim gleichen Arbeitgeber arbeiten. Wenn Sie weniger arbeiten, wird dieser Schutz nicht automatisch aktiviert und Sie müssen ihn ausdrücklich bei Ihrer Krankenkasse beantragen.</p>
<p>Was die LAA abdeckt, hat ganz konkrete praktische Konsequenzen: Im Falle eines Unfalls zahlt die Versicherung ab dem dritten Tag der Invalidität 80 % des versicherten Lohns, medizinische Kosten ohne Selbstbeteiligung oder Selbstbeteiligung und in schweren Fällen Invaliditäts- oder Hinterlassenenrenten für die Anspruchsberechtigten. Bei einem Arbeitsunfall oder auf dem Weg zur Arbeit kommt die Unfallversicherung nicht in Frage und eine Krankenversicherung kommt für diese Kosten nicht in Frage. Sie zahlen nicht die LAMal-Franchise. Sie bekommen die Krankenhausrechnung nicht. Das System nimmt es direkt auf.</p>
<p>Das bekannteste Unternehmen, das die LAA verwaltet, ist die SUVA, die automatisch Arbeitnehmer in Branchen wie dem Baugewerbe, der Industrie oder dem Gastgewerbe abdeckt. In anderen Branchen kann der Arbeitgeber Versicherungen bei privaten Unternehmen abschließen: Am häufigsten sind Allianz, AXA oder Helvetia. Wenn Sie nicht wissen, welche Einrichtung Sie abdeckt, steht dies auf Ihrer Gehaltsliste oder in Ihrem Arbeitsvertrag. Es lohnt sich zu wissen, bevor etwas passiert, nicht danach, denn im Moment des Unfalls bleibt keine Zeit mehr, die richtigen Fragen zu stellen.</p>
</div>`
    },

    "fb-daily-20260428-01-la-obligacion-del-empleador-que-pocos-conocen-el-stellenmeldepflicht-en-2026": {
        title: "Eine Arbeitgeberpflicht, die nur wenige kennen: die Stellenmeldepflicht im Jahr 2026",
        description: "Seit 2020 sind Schweizer Arbeitgeber gesetzlich dazu verpflichtet, bestimmte offene Stellen vor der Veröffentlichung beim RAV (Regionalvermittlungsstelle) zu melden. Sie dauern...",
        keywords: "Verpflichtung, Arbeitgeber, wenige, wissen, stellenmeldepflicht, 2026, 2020, Arbeitgeber",
        keywordsLocalized: true,
        category: "Arbeit und Löhne",
        hub: "trabajo",
        slug: "actualidad-2026-04-28-1-la-obligacion-del-empleador-que-pocos-conocen-el-stellenmeldepflicht-en-2026",
        readingTime: 1,
        dateUpdated: "28 April 2026",
        featuredImage: "media/guides/actualidad-stellenmeldepflicht-suiza.jpg",
        imageAlt: "Pflicht des Arbeitgebers Stellenmeldepflicht in der Schweiz",
        summary: "Seit 2020 sind Schweizer Arbeitgeber gesetzlich verpflichtet, bestimmte offene Stellen vor der Veröffentlichung beim RAV (Regionalvermittlungsstelle) zu melden.",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>Seit 2020 sind Schweizer Arbeitgeber gesetzlich dazu verpflichtet, bestimmte offene Stellen vor der Veröffentlichung beim RAV (Regionalvermittlungsstelle) zu melden. Fünf Werktage lang sind diese Angebote nur für im RAV registrierte Personen sichtbar. Ab 2026 wird diese Pflicht deutlich ausgeweitet: Betraf sie im Jahr 2025 noch 6,5 % der Arbeitnehmer des Landes, so umfasst sie nun 10,8 % der Erwerbsbevölkerung.</p>
<p>Zu den neuen Branchen, die im Jahr 2026 hinzukommen, gehören Reinigungskräfte und Assistenten in Büros, Hotels und ähnlichen Einrichtungen, Köche, Grafik- und Multimediadesigner, Veranstaltungsorganisatoren und einige Call-Center-Profile. Wenn Sie arbeitslos und beim RAV gemeldet sind, haben Sie in der Praxis vorrangig Zugang zu diesen Angeboten. Niemand sagt es dir am Fenster, aber es ist ein konkretes Recht.</p>
<p>Die Liste der meldepflichtigen Berufe wird jedes Jahr auf der Grundlage der Arbeitslosenquote nach Branchen aktualisiert. Um herauszufinden, ob Ihr Berufsprofil in der Liste enthalten ist, können Sie es direkt auf der Website von arbeit.swiss überprüfen (Tool „Check-Up 2026“). Wenn Ihr Beruf darin aufgeführt ist und Sie auf der Suche nach Arbeit sind, ist die Registrierung im RAV nicht mehr nur ein Verfahren: Sie ist ein echter Vorteil gegenüber denen, die nicht im RAV eingetragen sind.</p>
</div>`
    },

    "fb-daily-20260428-02-el-1-de-mayo-en-suiza-festivo-en-algunos-cantones-dia-de-mudanzas-en-casi-todos": {
        title: "1. Mai in der Schweiz: Feiertag in einigen Kantonen, Umzugstag in fast allen",
        description: "Nächsten Freitag ist der 1. Mai, und wenn Sie Spanier sind, verbinden Sie dieses Datum Ihr ganzes Leben lang mit dem Tag der Arbeit, einem ausnahmslosen Nationalfeiertag. In der Schweiz ist das anders und…",
        keywords: "Mai, Schweiz, Feiertag, Kantone, Umzug, alle, Freitag, kommend",
        keywordsLocalized: true,
        category: "Leben in der Schweiz",
        hub: "vivir-en-suiza",
        slug: "actualidad-2026-04-28-2-el-1-de-mayo-en-suiza-festivo-en-algunos-cantones-dia-de-mudanzas-en-casi-todos",
        readingTime: 2,
        dateUpdated: "28 April 2026",
        featuredImage: "media/guides/actualidad-primero-mayo-suiza.jpg",
        imageAlt: "Maifeiertag in der Schweiz, Feiertag und Umzugstag",
        summary: "Nächsten Freitag ist der 1. Mai, und wenn Sie Spanier sind, verbinden Sie dieses Datum Ihr ganzes Leben lang mit dem Tag der Arbeit, einem ausnahmslosen Nationalfeiertag. In der Schweiz ist die…",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>Nächsten Freitag ist der 1. Mai, und wenn Sie Spanier sind, verbinden Sie dieses Datum Ihr ganzes Leben lang mit dem Tag der Arbeit, einem ausnahmslosen Nationalfeiertag. In der Schweiz ist das anders, und es lohnt sich, dies zu verstehen, bevor man davon ausgeht, dass man einen freien Tag hat. Der 1. Mai ist nur in bestimmten Kantonen ein offizieller Feiertag: Basel-Stadt, Basel-Landschaft, Zürich, Luzern, Schaffhausen, Solothurn, Thurgau, Tessin, Neuenburg und Jura. Ansonsten ist es ein normaler Werktag. Genf, Waadt, Wallis, Freiburg oder Bern beispielsweise erkennen ihn nicht als Feiertag an.</p>
<p>Aber es gibt noch einen weiteren Aspekt des Schweizer 1. Mai, der diejenigen, die erst seit kurzem hier sind, noch mehr überrascht: Es ist ein bewegender Tag. Zusammen mit dem 1. Oktober markiert er den traditionellen Wechsel der Mietverträge. Ein erheblicher Teil der Mietverträge beginnt und endet an diesem Tag, was diesen Tag zu einem landesweiten Fest blockierter Lieferwagen, Kisten und Aufzüge macht. Wenn Ihr Mietvertrag jetzt beginnt oder endet, ist das kein Zufall, sondern Schweizer Brauch.</p>
<p>Die Kombination kann malerisch sein: In den Kantonen, in denen Feiertag ist, zieht man bei geschlossener Stadt um. In denen, in denen das nicht der Fall ist, gehen Sie zur Arbeit und kehren zurück, um den Treppenabsatz voller Möbel vorzufinden. Die praktische Schlussfolgerung ist klar: Wenn Sie wissen möchten, ob Sie am 1. Mai einen freien Tag haben, kommt es nicht auf den spanischen Kalender an, sondern auf den Kanton, in dem sich Ihr Unternehmen befindet. Und wenn Sie kurz davor stehen, einen Mietvertrag zu unterzeichnen oder zu kündigen, besteht eine gute Chance, dass Ihr Start- oder Enddatum der 1. Mai oder 1. Oktober ist, denn so funktioniert es hier seit Jahrzehnten.</p>
</div>`
    },

    "fb-daily-20260428-03-teletrabajo-desde-francia-para-empresa-suiza-las-nuevas-reglas-que-entraron-en-v": {
        title: "Telearbeit aus Frankreich für ein Schweizer Unternehmen: Die neuen Regeln, die im Januar in Kraft traten",
        description: "Viele Spanier, die in der Schweiz arbeiten, leben im französischen Grenzgebiet, insbesondere in der Region Genf, Basel oder im Jurabogen. Für sie alle bedeutet Telearbeit …",
        keywords: "Telearbeit, Frankreich, Unternehmen, Schweiz, neue, Regeln, eingegeben, Kraft",
        keywordsLocalized: true,
        category: "Grenzgänger",
        hub: "fronterizos",
        slug: "actualidad-2026-04-28-3-teletrabajo-desde-francia-para-empresa-suiza-las-nuevas-reglas-que-entraron-en-v",
        readingTime: 1,
        dateUpdated: "28 April 2026",
        featuredImage: "media/guides/actualidad-teletrabajo-francia-suiza.jpg",
        imageAlt: "Telearbeit aus Frankreich für Schweizer Unternehmen neue Regeln",
        summary: "Viele Spanier, die in der Schweiz arbeiten, leben im französischen Grenzgebiet, insbesondere in der Region Genf, Basel oder im Jurabogen. Für all das...",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>Viele Spanier, die in der Schweiz arbeiten, leben im französischen Grenzgebiet, insbesondere in der Region Genf, Basel oder im Jurabogen. Für sie alle stellte Telearbeit bisher eine steuerliche und administrative Grauzone dar. Seit dem 1. Januar 2026 gibt es in dieser Grauzone schriftliche und klar definierte Regeln, die die während der Pandemie ermöglichten und verlängerten Übergangsregelungen endgültig ersetzen.</p>
<p>Das überarbeitete Abkommen zwischen Frankreich und der Schweiz sieht vor, dass ein in Frankreich ansässiger Grenzgänger bis zu 40 % seiner jährlichen Arbeitstage von zu Hause aus Telearbeit leisten kann, ohne in der Schweiz Steuern zu verlieren. Darin enthalten ist auch ein Paket mit 10 Zusatztagen für berufliche Tätigkeiten im Ausland. Wesentliche Voraussetzung ist, dass der Schweizer Arbeitgeber die Meldepflichten einhält: Er muss monatlich Telearbeitsberichte einreichen und die entsprechenden A1-Sozialversicherungsbescheinigungen einholen. Es handelt sich nicht um ein optionales Verfahren.</p>
<p>Was sich in der Praxis ändert, ist, dass keine Unklarheiten mehr bestehen: Wer die 40 %-Grenze überschreitet, kann von den französischen Behörden steuerlich umgestuft werden. Auch die an speziellen grenzüberschreitenden Abkommen beteiligten Kantone, darunter Genf, Waadt, Basel-Stadt und Neuenburg, sind ab 2027 verpflichtet, Lohndaten mit der französischen Steuerverwaltung auszutauschen. Wer seine Situation unberücksichtigt bewältigt, kann in der Meldung eine böse Überraschung erleben. Wenn Sie für ein Schweizer Unternehmen arbeiten und jenseits der Grenze leben, ist es an der Zeit zu prüfen, wie viele Tage Sie im Homeoffice arbeiten und ob Ihr Arbeitgeber sein Meldesystem aktualisiert hat.</p>
</div>`
    },

    "fb-daily-20260429-01-la-lex-koller-endurecida-lo-que-propone-el-gobierno-suizo-y-por-que-los-espanole": {
        title: "Der hartgesottene Lex Koller: Was die Schweizer Regierung vorschlägt und warum die Spanier es wissen sollten",
        description: "Am 15. April hat der Bundesrat eine Vernehmlassung zu einer bedeutenden Reform der Lex Koller, dem Gesetz zur Regelung des Immobilienerwerbs durch Ausländer in der Schweiz, eingeleitet.…",
        keywords: "koller, gehärtet, schlägt vor, Regierung, Schweizer, Spanisch, sollte, wissen",
        keywordsLocalized: true,
        category: "Wohnen",
        hub: "vivienda",
        slug: "actualidad-2026-04-29-1-la-lex-koller-endurecida-lo-que-propone-el-gobierno-suizo-y-por-que-los-espanole",
        readingTime: 1,
        dateUpdated: "29 April 2026",
        featuredImage: "media/guides/actualidad-lex-koller-suiza.jpg",
        imageAlt: "Lex Koller Beschränkungen für ausländische Immobilien in der Schweiz",
        summary: "Am 15. April hat der Bundesrat eine Vernehmlassung zu einer bedeutenden Reform der Lex Koller eingeleitet, dem Gesetz, das den Erwerb von Immobilien durch Immobilien regelt.",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>Am 15. April hat der Bundesrat eine Vernehmlassung zu einer bedeutenden Reform der Lex Koller, dem Gesetz, das den Erwerb von Immobilien durch Ausländer in der Schweiz regelt, eingeleitet. Das Projekt, zu dem noch bis zum 15. Juli Stellung genommen werden kann, sieht deutlich strengere Beschränkungen für Staatsangehörige aus Ländern ausserhalb der EU und der EFTA vor: Sie müssten vor dem Kauf eines Erstwohnsitzes eine Einzelbewilligung beantragen und wären bei Ausreise aus der Schweiz verpflichtet, die Immobilie innerhalb von zwei Jahren zu verkaufen. Darüber hinaus entfällt die Möglichkeit, Gewerbeimmobilien mit dem alleinigen Zweck der Vermietung zu erwerben.</p>
<p>Für Spanier lautet die direkte Antwort: Sie sind von dieser Reform nicht betroffen: Als EU-Bürger sind Sie weiterhin von den Beschränkungen der Lex Koller für den Erwerb eines Hauptwohnsitzes ausgenommen. Aber der Kontext ist wichtig. Die Schweizer Regierung begründet diese Änderungen mit dem Druck auf den Immobilienmarkt, wo die Kaufpreise seit Anfang 2024 um mehr als 20 % gestiegen sind. Wenn der Kaufmarkt für Nicht-Europäer noch komplizierter wird, wird die Mietnachfrage weiter steigen, und das betrifft uns alle, die wir hier leben.</p>
<p>Was beobachtet werden sollte, ist der indirekte Effekt: Multinationale Unternehmen, die Wohnungen gekauft haben, um ausländische Führungskräfte aus Drittstaaten unterzubringen, könnten dies nicht mehr zu den gleichen Bedingungen tun. Dies steigert die Nachfrage nach Privatvermietungen in Gebieten wie Zürich, Genf oder Basel, die ohnehin schon schwierig genug sind. Das ist keine Panikmache, es ist Marktarithmetik.</p>
</div>`
    },

    "fb-daily-20260429-02-el-impuesto-a-la-fuente-la-correccion-que-muy-pocos-solicitan-y-que-puede-devolv": {
        title: "Die Quellensteuer: die Korrektur, die nur sehr wenige verlangen und die Ihnen Geld zurückgeben kann",
        description: "Die Mehrheit der Spanier, die ohne C-Bewilligung oder ohne Schweizer Ehegatten in die Schweiz gekommen sind, zahlen Steuern durch Quellensteuer, die auf Deutsch Quellensteuer und auf Französisch impô genannt wird.",
        keywords: "Steuer, Quelle, Korrektur, wenige, anfordern, können, Ihnen Geld zurückgeben",
        keywordsLocalized: true,
        category: "Steuern",
        hub: "impuestos",
        slug: "actualidad-2026-04-29-2-el-impuesto-a-la-fuente-la-correccion-que-muy-pocos-solicitan-y-que-puede-devolv",
        readingTime: 2,
        dateUpdated: "29 April 2026",
        featuredImage: "media/guides/actualidad-impuesto-fuente-correccion.jpg",
        imageAlt: "Korrektur der Quellensteuer in der Schweiz",
        summary: "Die Mehrheit der Spanier, die ohne C-Bewilligung oder ohne Schweizer Ehegatten in die Schweiz gekommen sind, zahlen Steuern durch Quellensteuer, die auf Deutsch Quellensteuer genannt wird.",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>Die Mehrheit der Spanier, die ohne C-Bewilligung oder ohne Schweizer Ehegatten in die Schweiz einreisen, zahlen Steuern durch Quellensteuer, die auf Deutsch Quellensteuer und auf Französisch impôt à la source genannt wird. Ihr Arbeitgeber zieht jeden Monat einen Prozentsatz Ihres Lohns ab und überweist ihn direkt an den Kanton. Es scheint einfach: Sie müssen nichts tun. Das Problem besteht darin, dass dieser Prozentsatz auf Basis der Grundparameter Familienstand und Kinder berechnet wird, ohne dass irgendetwas anderes berücksichtigt wird. Und das bedeutet: Wenn Sie abzugsfähige Ausgaben haben, zahlen Sie mit ziemlicher Sicherheit zu viel.</p>
<p>Seit der Reform von 2021, die in den meisten Kantonen vollständig in Kraft getreten ist, kann jede Person, die dem Quellensteuerabzug unterliegt, eine „ordentliche Berichtigung“ beantragen, wenn sie Ausgaben hat, die einen zusätzlichen Abzug rechtfertigen: dritte Säule (3a), Krankenkassenprämien, die den Regelabzug übersteigen, Berufsausbildungskosten, Transport zur Arbeit mit echten Kosten oder ungedeckte medizinische Kosten, die die Abzugsgrenze überschreiten. Es ist kein komplexer Vorgang, aber Sie müssen damit beginnen. In den meisten Kantonen war die ordentliche Frist der 31. März, eine Verlängerung kann jedoch beantragt werden und wird in vielen Kantonen automatisch oder mit einem einfachen Formular gewährt.</p>
<p>Es gibt einen anderen Fall, der beachtet werden sollte: Wenn Ihr Bruttoeinkommen im Jahr 2025 in einem einzelnen Kanton 120'000 Franken pro Jahr übersteigt, müssen Sie laut Gesetz eine vollständige ordentliche Erklärung abgeben, unabhängig davon, ob Sie abzugsfähige Ausgaben haben oder nicht. Es ist nicht optional. Wenn Sie sich nicht sicher sind, in welcher Situation Sie sich befinden, lohnt es sich, vor Ende des ersten Semesters beim kantonalen Steueramt nachzufragen. Jeder Kanton handhabt dies etwas anders und die Fristen und Schwellenwerte können variieren.</p>
</div>`
    },

    "fb-daily-20260429-03-el-psicologo-cubierto-por-el-seguro-medico-basico-como-funciona-y-por-que-tanta-": {
        title: "Der grundversicherte Psychologe: Wie er funktioniert und warum ihn so viele Menschen noch nicht kennen",
        description: "Seit Juli 2022 gibt es in der Schweiz eine Veränderung, die vielen Einwohnern, darunter auch vielen Spaniern, noch nicht bewusst ist: Zugelassene Psychologen können die...",
        keywords: "Psychologe, versichert, sicher, Arzt, einfach, arbeitet, so viele Menschen",
        keywordsLocalized: true,
        category: "Gesundheit und KVG",
        hub: "salud",
        slug: "actualidad-2026-04-29-3-el-psicologo-cubierto-por-el-seguro-medico-basico-como-funciona-y-por-que-tanta-",
        readingTime: 1,
        dateUpdated: "29 April 2026",
        featuredImage: "media/guides/actualidad-psicologo-seguro-basico-suiza.jpg",
        imageAlt: "Psychologe, der in der Schweiz in der Grundversicherung der Krankenkassenversicherung versichert ist",
        summary: "Seit Juli 2022 gibt es in der Schweiz eine Veränderung, die vielen Einwohnern, darunter auch vielen Spaniern, noch nicht bewusst ist: Zugelassene Psychologen können abrechnen...",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>Seit Juli 2022 gibt es in der Schweiz eine Änderung, die vielen Einwohnern, darunter auch vielen Spaniern, noch nicht bewusst ist: Zugelassene Psychologen können direkt mit der obligatorischen Krankenversicherung (KVG) abrechnen, ohne dass für die Behandlung ein Psychiater erforderlich ist. Es gibt nur eine Bedingung: Der Hausarzt muss ein Rezept ausstellen, aus dem hervorgeht, dass eine psychologische Überwachung erforderlich ist. Von dort aus kann sich der Patient an einen nach diesem System akkreditierten Psychologen wenden, dessen Sitzungen in die Grundversorgung fallen.</p>
<p>Vor dieser Reform war die übliche Route länger und teurer. Entweder ging er zu einem Psychiater, dessen Termine lange Wartelisten haben, oder er bezahlte aus eigener Tasche. Jetzt genügt ein Besuch beim Hausarzt, um das Rezept zu erhalten, was in der Praxis den Zugang zu einer Versorgung ermöglicht, die bisher für viele Menschen außerhalb der finanziellen Möglichkeiten lag. Es gilt weiterhin die übliche Selbstbeteiligung: Zuerst wird die Franchise verbraucht, dann die 10-Prozent-Beteiligung bis zur Jahresgrenze von 700 Franken für Erwachsene.</p>
<p>Bevor Sie beginnen, ist es wichtig zu wissen, dass nicht alle Psychologen berechtigt sind, im Rahmen dieses Systems abzurechnen. Es muss überprüft werden, ob der Berufstätige über die entsprechende Zulassung verfügt und im Register der Gesundheitsberufe (RPS) eingetragen ist. Wenn Sie in einem französischsprachigen Kanton wohnen, kann Ihnen die Plattform psyfinder.ch dabei helfen, Psychologen zu finden, die mit diesem System verfügbar sind. In den deutschen Kantonen verfügt in der Regel jeder Versicherer über eine eigene Suchmaschine. Und ja, es funktioniert auch mit Sitzungen auf Spanisch, wenn Sie einen lizenzierten spanischsprachigen Psychologen finden.</p>
</div>`
    },

    "fb-daily-20260430-01-la-nueva-ley-del-ruido-entra-en-vigor-y-cambia-las-reglas-para-construir-viviend": {
        title: "Das neue Lärmgesetz tritt in Kraft und ändert die Regeln für den Wohnungsbau in der Schweiz",
        description: "Seit dem 1. April 2026 ist die Teilrevision des Bundesgesetzes über den Umweltschutz (Umweltschutzgesetz) und der Lärmschutzverordnung (VVG) in Kraft.",
        keywords: "neu, Lärm, eintreten, erzwingen, verändern, Regeln, bauen, Wohnen",
        keywordsLocalized: true,
        category: "Wohnen",
        hub: "vivienda",
        slug: "actualidad-2026-04-30-1-la-nueva-ley-del-ruido-entra-en-vigor-y-cambia-las-reglas-para-construir-viviend",
        readingTime: 2,
        dateUpdated: "30. April 2026",
        featuredImage: "media/guides/actualidad-ley-ruido-vivienda-suiza.jpg",
        imageAlt: "Neues Lärmgesetz für den Wohnungsbau in der Schweiz",
        summary: "Seit dem 1. April 2026 ist die Teilrevision des Bundesgesetzes über den Umweltschutz (Umweltschutzgesetz) und der Umweltschutzverordnung in Kraft.",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>Seit dem 1. April 2026 ist die Teilrevision des Bundesgesetzes über den Umweltschutz (Umweltschutzgesetz) und der Lärmschutzverordnung (VBV) in Kraft. In der Praxis können kantonale und kommunale Behörden nun den Bau von Wohnungen in Lärmgebieten auch dann bewilligen, wenn die Immissionsgrenzwerte vor den Fenstern leicht überschritten werden. Bisher blockierten diese Beschränkungen Tausende von Projekten in der Nähe von Straßen, Bahngleisen oder Straßenbahnlinien, insbesondere in Genf, Waadt und Zürich, wo der Mangel an Wohnungen seit Jahren das beliebteste Gesprächsthema zwischen Nachbarn ist.</p>
<p>Die neue Regel ist kein Blankoscheck. Können die Grenzwerte mit vertretbarem Aufwand nicht eingehalten werden, wird die Baugenehmigung nur dann erteilt, wenn entsprechende Ausgleichsmaßnahmen in den Räumen eingebaut werden, etwa eine kontrollierte Lüftung, um das Öffnen der Fenster zu vermeiden, oder besondere Schutzmaßnahmen an den Fassaden. Die erklärte Idee des Bundesrates besteht darin, die Lärmbekämpfung besser mit der Stadtentwicklung nach innen zu koordinieren, was seit der Revision des LAT die offizielle Linie ist.</p>
<p>Für Wohnungssuchende ändert sich daran kurzfristig zwar wenig, wissenswert ist aber, dass ein völlig legaler Neubau auch in einer Gegend stehen kann, für die vorher noch nicht einmal eine Genehmigung erteilt worden wäre. Wenn in der Anzeige „lumineux et quiete“ steht, die Immobilie aber an einer stark befahrenen Straße liegt, sind mechanische Belüftung und verstärktes Glas heute die Norm und kein Luxus mehr. Vor der Unterzeichnung lohnt es sich, den Eigentümer nach dem Akustikbericht des Projekts zu fragen, während der Hauptverkehrszeit das Fenster zu öffnen und zu fragen, mit welcher Lüftungsanlage das Haus ausgestattet ist.</p>
<p>Für diejenigen, die über den Kauf oder die Investition in ein neues Projekt nachdenken, öffnet die Reform die Tür zu mehr Angeboten in gut angebundenen Stadtteilen, die bisher eingefroren waren. Der Bund spricht von mehreren zehntausend potenziellen Wohnungen in der ganzen Schweiz. Es wird einige Zeit dauern, bis die Maßnahme auf dem Markt wahrgenommen wird, denn zwischen Planung, Einspruch und Bau baut in diesem Land niemand in sechs Monaten ein Gebäude, außer in Wahlreden.</p>
</div>`
    },

    "fb-daily-20260430-02-defectos-de-construccion-ahora-tienes-60-dias-para-reclamar-y-eso-cambia-muchas-": {
        title: "Baumängel: Sie haben jetzt 60 Tage Zeit, um Ihre Ansprüche geltend zu machen, und das ändert viele Dinge",
        description: "Seit dem 1. Januar 2026 ist die Regelung des Mängelgewährleistungsrechts im Obligationenrecht in Kraft. Die wichtigste Neuerung ist eine einzige gesetzliche Frist von 60 Tagen für…",
        keywords: "Mängel, Konstruktion, jetzt, Sie haben, Tage, Anspruch, Änderung, viele",
        keywordsLocalized: true,
        category: "Wohnen",
        hub: "vivienda",
        slug: "actualidad-2026-04-30-2-defectos-de-construccion-ahora-tienes-60-dias-para-reclamar-y-eso-cambia-muchas-",
        readingTime: 2,
        dateUpdated: "30. April 2026",
        featuredImage: "media/guides/actualidad-defectos-construccion-suiza.jpg",
        imageAlt: "60-tägige Frist zur Geltendmachung von Baumängeln in der Schweiz",
        summary: "Seit dem 1. Januar 2026 ist die Regelung des Mängelgewährleistungsrechts im Obligationenrecht in Kraft. Die wichtigste Neuerung ist eine gesetzliche Frist…",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>Ab dem 1. Januar 2026 ist die Überprüfung des Gewährleistungsrechts bei Mängeln im Obligationenrecht in Kraft. Die wichtigste Neuerung ist eine einzige gesetzliche Frist von 60 Tagen, um den Bauunternehmer oder Verkäufer über jeden Mangel zu informieren, unabhängig davon, ob dieser bei Erhalt der Arbeit sichtbar ist oder erst später entdeckt wird. Früher war gesetzlich vorgeschrieben, dass der Mangel „unverzüglich“ angezeigt werden muss, ein dehnbarer Ausdruck, der in der Praxis meist als drei bis zehn Tage ausgelegt wurde, wobei der Mieter oder Käufer auf ewig dem Risiko ausgesetzt war, bei einer geringfügigen Verzögerung seine Rechte zu verlieren.</p>
<p>Die Änderung ist wichtig, da die Frist nun zwingend ist. Jede Klausel in einem am oder nach dem 1. Januar 2026 unterzeichneten Vertrag, die diese 60 Tage zum Nachteil des Käufers oder Eigentümers verkürzt, ist ungültig. Gleiches gilt für die fünfjährige Verjährungsfrist im Immobilienbau, die auch vertraglich nicht verkürzt werden kann. Und Menschen, die ein neues Haus oder ein Gebäude kaufen, das ab Lieferung weniger als zwei Jahre alt ist, haben jetzt einen gesetzlichen Anspruch auf kostenlose Reparatur, zu Bedingungen, die der Verkäufer mit einer Standardklausel nicht mehr vermeiden kann.</p>
<p>Für Spanier, die hier eine Wohnung kaufen, sind das gute Nachrichten. Viele Schweizer Verträge enthielten Klauseln, die die Haftung des Verkäufers so stark einschränkten, dass die Geltendmachung eines schwerwiegenden Mangels fast einem Rechtswunder gleichkam. Diese Klauseln bleiben für den Rest des Garantieregimes gültig, jedoch nicht mehr für die durch das neue Gesetz geschützten Punkte. Wenn Sie einen Vertrag vor 2026 unterzeichnet haben, gilt weiterhin die bisherige Regelung. Es lohnt sich daher, das genaue Datum zu prüfen, bevor Sie mit dem Veranstalter streiten.</p>
<p>In der Praxis empfiehlt es sich, alles zu dokumentieren, was ab der Schlüsselübergabe zu sehen ist: Fotos, Videos, ein technischer Besuch mit einem Experten, wenn es sich um einen wichtigen Eingriff handelt. Und stellen Sie etwaige Ansprüche schriftlich, am besten per Einschreiben, damit die Frist von 60 Tagen nicht zu einer Frage des Wortes wird. Die Schweiz verzeiht keine Fristen, aber jetzt wissen Sie zumindest, worum es geht.</p>
</div>`
    },

    "fb-daily-20260430-03-la-electricidad-baja-en-2026-pero-solo-si-vives-en-el-canton-correcto": {
        title: "Der Strom sinkt im Jahr 2026, aber nur, wenn Sie im richtigen Kanton wohnen",
        description: "Die Eidgenössische Elektrizitätskommission (ElCom) hat bestätigt, dass die Grundversorgungstarife für Haushalte im Jahr 2026 um durchschnittlich 4% sinken werden. Der durchschnittliche Gesamttarif beträgt…",
        keywords: "Strom, niedrig, 2026, nur, Sie wohnen, Kanton, rechts, Kommission",
        keywordsLocalized: true,
        category: "Leben in der Schweiz",
        hub: "vivir-en-suiza",
        slug: "actualidad-2026-04-30-3-la-electricidad-baja-en-2026-pero-solo-si-vives-en-el-canton-correcto",
        readingTime: 2,
        dateUpdated: "30. April 2026",
        featuredImage: "media/guides/actualidad-electricidad-canton-suiza.jpg",
        imageAlt: "Strompreis nach Kantonen in der Schweiz 2026",
        summary: "Die Eidgenössische Elektrizitätskommission (ElCom) hat bestätigt, dass die Grundversorgungstarife für Haushalte im Jahr 2026 um durchschnittlich 4 % sinken werden. Der Tarif…",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>Die Eidgenössische Elektrizitätskommission (ElCom) hat bestätigt, dass die Grundversorgungstarife für Haushalte im Jahr 2026 um durchschnittlich 4% sinken werden. Der durchschnittliche Gesamttarif beträgt 27,7 Rp./kWh, im Vergleich zu 29 im Vorjahr, was einer durchschnittlichen Ersparnis von 58 Franken pro Jahr für einen typischen Haushalt entspricht. Der Grund ist einfach: Die in den Jahren 2022 und 2023 zu überhöhten Preisen abgeschlossenen Lieferverträge laufen aus und die neuen werden zu günstigeren Preisen auf dem europäischen Markt abgeschlossen.</p>
<p>Das unangenehme Detail ist, dass sich hinter dem Landesdurchschnitt je nach Wohnort enorme Unterschiede verbergen. La Chaux-de-Fonds fällt um 15 %, Lausanne um 12 %, während Zermatt um 18 % steigt. Es gibt Nachbargemeinden mit Tarifunterschieden von mehr als 30 %, einfach weil der örtliche Versorger auf unterschiedliche Verträge, seinen Produktionsmix und die geplanten Netzinvestitionen angewiesen ist. In der Schweiz entscheidet die Postleitzahl nicht nur darüber, wie viel Steuern Sie zahlen, sondern auch darüber, wie viel es kostet, das Licht anzuschalten.</p>
<p>Eine weitere praktische Neuerung: Messkosten, die bisher in der Rechnung enthalten waren, werden jetzt separat in Rechnung gestellt. Für ein normales Eigenheim sind es 74.40 Franken pro Jahr, je nach Profilart 1.65 ct./kWh zusätzlich. Es handelt sich nicht um eine versteckte Erhöhung, die Beträge waren schon vorher da, aber die Aufschlüsselung wird mehr als einen überraschen, wenn die Jahresrechnung mit zwei neuen Zeilen statt einer eintrifft.</p>
<p>Wer sich einen Lieferanten aussuchen kann, dem empfiehlt sich ein Vergleich vor Juni, wenn viele liberalisierte Verträge für den Winter neu ausgehandelt werden. Im Übrigen ist die einzige sinnvolle Maßnahme, die an den Grundversorger der Gemeinde gebunden ist, die weitere Reduzierung des Verbrauchs: Elektronik im Standby, elektrische Heizkörper und der Warmwasserbereiter sind weiterhin die großen stillen Übeltäter. Die Preise sinken, aber wir zahlen immer noch 30 % mehr als im Jahr 2022, daher ist die Party nichts Besonderes.</p>
</div>`
    },

    "fb-daily-20260501-01-el-referendum-del-14-de-junio-lo-que-realmente-esta-en-juego-para-los-espanoles-": {
        title: "Das Referendum vom 14. Juni: Was für die Spanier in der Schweiz wirklich auf dem Spiel steht",
        description: "Am 14. Juni stimmen die Schweizer Bürger über eine SVP-Initiative ab, die eine Begrenzung der Bevölkerungszahl in der Schweiz auf maximal 10 Millionen Menschen vorsieht. Derzeit h…",
        keywords: "Referendum, Juni, wirklich, Spiel, Spanisch, Schweiz, Bürger, Schweizer",
        keywordsLocalized: true,
        category: "Warnungen und Ressourcen",
        hub: "recursos",
        slug: "actualidad-2026-05-01-1-el-referendum-del-14-de-junio-lo-que-realmente-esta-en-juego-para-los-espanoles-",
        readingTime: 1,
        dateUpdated: "1 Mai 2026",
        featuredImage: "media/guides/actualidad-referendum-junio-espanoles.jpg",
        imageAlt: "Referendum vom 14. Juni und seine Auswirkungen auf die Spanier in der Schweiz",
        summary: "Am 14. Juni stimmen die Schweizer Bürger über eine SVP-Initiative ab, die eine Begrenzung der Bevölkerungszahl in der Schweiz auf maximal 10 Millionen Menschen vorsieht.",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>Am 14. Juni stimmen die Schweizer Bürger über eine SVP-Initiative ab, die eine Begrenzung der Bevölkerungszahl in der Schweiz auf maximal 10 Millionen Menschen vorsieht. Derzeit sind es knapp über 9,1 Millionen, der Spielraum ist also nicht so groß, wie es scheint. Was dies für uns zu einem ernsten Problem macht, ist, was nach der Grenze kommt: Wenn die Initiative angenommen wird, wäre die Regierung verpflichtet, das Personenfreizügigkeitsabkommen mit der Europäischen Union zu beenden, sobald die 9,5 Millionen erreicht sind.</p>
<p>Für europäische Bürger, die hier mit einer B- oder C-Bewilligung leben, ist dieses bilaterale Abkommen die rechtliche Grundlage unseres Status. Dadurch kann ein Spanier mit einem Arbeitsvertrag in die Schweiz einreisen und eine Aufenthaltserlaubnis erhalten, ohne die für Drittländer vorgesehenen Quoten in Anspruch nehmen zu müssen. Wenn diese Vereinbarung scheitert, verschwinden wir nicht über Nacht, die bereits erteilten Genehmigungen sind gültig, aber der rechtliche Rahmen, der sie unterstützt, ändert sich strukturell und zukünftige Erneuerungen könnten kompliziert werden.</p>
<p>Die Bundesregierung und die Mehrheit des Bundesrates lehnen die Initiative ab. Laut aktuellen Umfragen besteht eine reelle Chance auf Ablehnung, doch in der Schweiz haben Volksabstimmungen die schlechte Angewohnheit, überraschend zu sein. Das Vernünftigste ist, die Debatte zu verfolgen, genau zu verstehen, was der Text vorschlägt, und, wenn Sie das Wahlrecht haben, weil Sie bereits die Schweizer Staatsangehörigkeit besitzen, es zu nutzen. Und wenn Sie es nicht haben, wissen Sie zumindest, worum es geht, damit Sie es nicht am 15. Juni erfahren.</p>
</div>`
    },

    "fb-daily-20260501-02-el-acoso-es-ahora-delito-independiente-en-suiza-lo-que-cambia-con-el-nuevo-artic": {
        title: "Belästigung ist in der Schweiz nun ein eigenständiges Verbrechen: Was sich mit dem neuen Artikel des Strafgesetzbuches ändert",
        description: "Seit dem 1. Januar 2026 ist Stalking, die andauernde Belästigung einer Person, ein autonomes Verbrechen im Schweizer Strafgesetzbuch. Zuvor wurden diese Verhaltensweisen auf unterschiedliche Weise verfolgt ...",
        keywords: "Belästigung, jetzt, Kriminalität, unabhängige, Schweiz, Veränderung, neuer, Artikel",
        keywordsLocalized: true,
        category: "Warnungen und Ressourcen",
        hub: "recursos",
        slug: "actualidad-2026-05-01-2-el-acoso-es-ahora-delito-independiente-en-suiza-lo-que-cambia-con-el-nuevo-artic",
        readingTime: 1,
        dateUpdated: "1 Mai 2026",
        featuredImage: "media/guides/actualidad-acoso-delito-codigo-penal-suiza.jpg",
        imageAlt: "Belästigung als eigenständiges Verbrechen im Schweizer Strafgesetzbuch",
        summary: "Seit dem 1. Januar 2026 ist Stalking, die andauernde Belästigung einer Person, ein autonomes Verbrechen im Schweizer Strafgesetzbuch. Früher waren diese Verhaltensweisen pro…",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>Seit dem 1. Januar 2026 ist Stalking, die andauernde Belästigung einer Person, ein autonomes Verbrechen im Schweizer Strafgesetzbuch. Zuvor wurden diese Verhaltensweisen verstreut im Rahmen anderer Straftaten verfolgt, was das Vorgehen der Polizei und der Justiz erheblich erschwerte. Nun gibt es einen speziellen Artikel, der es erlaubt, zu handeln, ohne dass andere Nebenstraftatbestände nachgewiesen werden müssen, und die Strafen können bis zu drei Jahre Gefängnis betragen.</p>
<p>In der Praxis bedeutet das: Wenn Ihnen jemand folgt, Sie zwanghaft anruft, wiederholt bei Ihnen am Arbeitsplatz oder zu Hause auftaucht oder Sie in den sozialen Medien systematisch belästigt, besteht bereits eine unmittelbare Rechtsgrundlage für die Meldung. Die „Beharrlichkeitsschwelle“ ist wichtig, es ist kein unangenehmer Anruf, es ist ein wiederholtes Verhaltensmuster, das Ihr tägliches Leben stört. Aber die Tatsache, dass es sich nun um eine klassifizierte Straftat handelt, verändert die Einstellung der Polizei, wenn Sie eine Anzeige erstatten.</p>
<p>Für die spanische Community, die häufig in Expat-Gruppen und sozialen Netzwerken mit Menschen unterwegs ist, die sie nicht gut kennt, ist dies auch auf digitaler Ebene relevant. Anhaltendes Cybermobbing fällt unter die Kategorie der Kriminalität. Wenn Sie sich in einer solchen Situation befinden, ist die Anzeige bei der Kantonspolizei der erste Schritt und hat neu mehr rechtliches Gewicht als zuvor.</p>
</div>`
    },

    "fb-daily-20260501-03-las-deducciones-fiscales-del-pilar-3a-en-2026-el-limite-sube-y-todavia-hay-tiemp": {
        title: "Steuerabzüge in der Säule 3a im Jahr 2026: Die Grenze steigt und es bleibt noch Zeit, davon zu profitieren",
        description: "Im Jahr 2026 ist die maximale Steuerabzugsgrenze für die Säule 3a erneut angestiegen. Arbeitnehmer, die in die zweite Säule einzahlen, können bis zu 7.258 Franken von der Steuerbemessungsgrundlage abziehen…",
        keywords: "Abzüge, Steuern, Säule, 2026, Grenze, Anstieg, immer noch, Zeit",
        keywordsLocalized: true,
        category: "Steuern",
        hub: "impuestos",
        slug: "actualidad-2026-05-01-3-las-deducciones-fiscales-del-pilar-3a-en-2026-el-limite-sube-y-todavia-hay-tiemp",
        readingTime: 1,
        dateUpdated: "1 Mai 2026",
        featuredImage: "media/guides/actualidad-pilar-3a-deduccion-fiscal.jpg",
        imageAlt: "Steuerabzug Säule 3a in der Schweiz 2026",
        summary: "Im Jahr 2026 ist die maximale Steuerabzugsgrenze für die Säule 3a erneut angestiegen. Arbeitnehmer, die in die zweite Säule einzahlen, können bis zu 7.258 Franken abziehen…",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>Im Jahr 2026 ist die maximale Steuerabzugsgrenze für die Säule 3a erneut angestiegen. Arbeitnehmer, die in die zweite Säule einzahlen, können bis zu 7.258 Franken von der Steuer absetzen, Selbständige ohne zweite Säule können bis zu 36.288 Franken von der Steuer absetzen. Diese Zahlen werden periodisch auf der Grundlage des durchschnittlichen versicherten Lohns aktualisiert und 2026 bringt eine Steigerung im Vergleich zum Vorjahr.</p>
<p>Bei der Säule 3a handelt es sich um private Altersvorsorgesparen, die der Schweizer Staat steuerlich fördert. Sie eröffnen ein Konto bei einer Bank oder Versicherung, geben den gewünschten Betrag bis zur gesetzlichen Höchstgrenze ein und dieser Betrag wird in der Erklärung direkt von Ihrem steuerpflichtigen Einkommen abgezogen. Je nach Kanton und Lohnniveau kann die tatsächliche Steuerersparnis beträchtlich sein, bei einem durchschnittlichen Lohn liegen die Steuerersparnisse zwischen 1.500 und 3.000 Franken weniger pro Jahr.</p>
<p>Was viele Menschen nicht wissen, insbesondere diejenigen, die erst seit ein paar Jahren in der Schweiz sind, ist, dass man mehrere 3a-Konten bei verschiedenen Institutionen eröffnen und diese bei Erreichen der Pensionierung in verschiedenen Jahren einlösen kann, um die steuerlichen Auswirkungen der Rettung zu reduzieren. Die Frist für den Beitrag für das Geschäftsjahr 2026 ist der 31. Dezember. Wenn Sie die Konditionen zwischen Banken und Investmentfonds vergleichen möchten, ist es jedoch ratsam, dies nicht bis zur letzten Minute zu verschieben. Einige bieten deutlich bessere Renditen als andere.</p>
</div>`
    },

    "fb-daily-20260503-01-cambiar-de-canton-en-suiza-no-es-solo-cambiar-de-direccion": {
        title: "Einen Kanton in der Schweiz zu wechseln bedeutet mehr als nur eine neue Adresse",
        description: "Wenn Sie schon eine Weile hier sind, wissen Sie wahrscheinlich bereits, dass die Schweiz nicht wie ein zentralisiertes Land funktioniert. Jeder Kanton hat sein…",
        keywords: "einen, kanton, schweiz, wechseln, bedeutet, mehr, neue, adresse",
        keywordsLocalized: true,
        category: "Formalitäten und Bewilligungen",
        hub: "tramites",
        slug: "actualidad-2026-05-03-1-cambiar-de-canton-en-suiza-no-es-solo-cambiar-de-direccion",
        readingTime: 1,
        dateUpdated: "3. Mai 2026",
        featuredImage: "media/guides/actualidad-cambio-canton-suiza.jpg",
        imageAlt: undefined,
        summary: "Wenn Sie schon eine Weile hier sind, wissen Sie wahrscheinlich bereits, dass die Schweiz nicht wie ein zentralisiertes Land funktioniert. Jeder…",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>Wenn Sie schon eine Weile hier sind, wissen Sie wahrscheinlich bereits, dass die Schweiz nicht wie ein zentralisiertes Land funktioniert. Jeder Kanton hat sein eigenes Steuersystem, seine eigene Verwaltung und, wenn Sie Ausländer mit Aufenthaltsbewilligung sind, bringt ein Kantonswechsel mehr Formalitäten mit sich, als viele erwarten.</p>
<p>Zunächst müssen Sie Ihren Wegzug bei der aktuellen Gemeinde melden und sich in der neuen Gemeinde innerhalb von 14 Tagen nach Ihrem Umzug anmelden. Das ist nicht optional: Es ist eine gesetzliche Pflicht. Wenn Sie eine Bewilligung B oder C haben, übernimmt die neue kantonale Migrationsbehörde und stellt die entsprechende Bewilligung für den neuen Kanton aus. Für EU/EFTA-Bürger ist der Vorgang unkompliziert, aber Sie müssen ihn selbst in die Wege leiten.</p>
<p>Ein wichtiger Punkt, den man kennen sollte: Wenn Sie eine Bewilligung B haben und Arbeitslosenunterstützung (Chômage/RAV) beziehen, können Sie den Kanton nicht frei wechseln. Das Gesetz verlangt, dass Sie nicht arbeitslos sind, um dieses Mobilitätsrecht ausüben zu können. Das RAV-Büro im neuen Kanton wird dies überprüfen, und wenn keine Kommunikation mit dem Büro des Herkunftskantons besteht, wird der Umzug blockiert.</p>
<p>Was die Steuern betrifft: Wenn Sie im Laufe des Jahres umziehen, zahlen Sie kantonale Steuern proportional zur Zeit, die Sie in jedem Kanton verbracht haben. Für Personen mit Quellenbesteuerung gilt der neue Satz ab dem Monat nach dem Wohnsitzwechsel. Die Unterschiede zwischen den Kantonen sind erheblich: Zug, Schwyz oder Nidwalden haben viel niedrigere Sätze als Genf, Bern oder Waadt. Es ist nicht ungewöhnlich, dass jemand einen Umzug aus rein steuerlichen Gründen in Betracht zieht.</p>
<p>Die Krankenkasse ändert sich nicht automatisch, aber die Prämien variieren je nach Wohnkanton. Es ist der richtige Moment zum Vergleichen.</p>
<p>Zusammenfassend: Es ist möglich, der Prozess existiert und funktioniert. Aber er läuft nicht von allein.</p>
</div>`
    },

    "fb-daily-20260503-02-si-te-pones-enfermo-en-suiza-esto-es-lo-que-debes-hacer-y-lo-que-te-cu": {
        title: "Wenn Sie in der Schweiz krank werden: Was Sie tun müssen und was Sie absichert",
        description: "Krankheitsbedingte Arbeitsunfähigkeit in der Schweiz funktioniert ganz anders als im spanischen System, und es zu verstehen, bevor man es braucht, vermeidet…",
        keywords: "wenn, schweiz, krank, werden, müssen, absichert, krankheitsbedingte, arbeitsunfähigkeit",
        keywordsLocalized: true,
        category: "Gesundheit und KVG",
        hub: "salud",
        slug: "actualidad-2026-05-03-2-si-te-pones-enfermo-en-suiza-esto-es-lo-que-debes-hacer-y-lo-que-te-cu",
        readingTime: 1,
        dateUpdated: "3. Mai 2026",
        featuredImage: "media/guides/actualidad-baja-enfermedad-suiza.jpg",
        imageAlt: undefined,
        summary: "Krankheitsbedingte Arbeitsunfähigkeit in der Schweiz funktioniert ganz anders als im spanischen System, und es zu verstehen, bevor man es braucht,…",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>Krankheitsbedingte Arbeitsunfähigkeit in der Schweiz funktioniert ganz anders als im spanischen System, und es zu verstehen, bevor man es braucht, vermeidet unangenehme Überraschungen.</p>
<p>Zunächst: Wenn Sie krankheitsbedingt nicht arbeiten können, müssen Sie Ihren Arbeitgeber so bald wie möglich informieren, möglichst noch am selben Tag. Das ist keine Formalität: Es ist eine vertragliche Pflicht. In den meisten Unternehmen wird ein Arztzeugnis ab dem dritten Tag verlangt, obwohl der Vertrag es ab dem ersten verlangen kann. Wenn Sie es nicht rechtzeitig einreichen, kann der Arbeitgeber Ihnen diese Tage vom Lohn abziehen.</p>
<p>Was Ihnen zusteht: Artikel 324a des Obligationenrechts (OR) verpflichtet den Arbeitgeber, Ihnen während der Krankheit den vollen Lohn zu zahlen. Im ersten Vertragsjahr beträgt dieses Minimum drei Wochen. Ab dem zweiten Jahr verlängert sich die Dauer mit der Betriebszugehörigkeit gemäss den kantonalen Referenzskalen. Die meisten Unternehmen haben jedoch eine Krankentaggeldversicherung, die 80 % des Lohnes für 720 Tage innerhalb eines Zeitraums von 900 Tagen abdeckt. Diese Versicherung ersetzt die direkte Arbeitgeberpflicht, und das Gesetz verlangt, dass der Arbeitgeber mindestens die Hälfte der Prämie bezahlt.</p>
<p>Zwei Dinge, die viele nicht wissen: Es gibt in der Schweiz keine Wartefrist, das heisst, Sie erhalten ab dem ersten Krankheitstag eine Entschädigung. Und Ihr Vertrag ist geschützt: Der Arbeitgeber kann Sie während einer Krankschreibung nicht kündigen, und zwar für eine Dauer von 30, 90 oder 180 Tagen je nach Betriebszugehörigkeit.</p>
<p>Wenn das Unternehmen diese Versicherung nicht hat und nur das gesetzliche Minimum anwendet, ist die Deckung deutlich kürzer. Es lohnt sich, das zu wissen, bevor man es braucht.</p>
</div>`
    },

    "fb-daily-20260503-03-el-dentista-en-suiza-no-lo-cubre-el-seguro-basico-y-muchos-lo-descubre": {
        title: "Zahnarztbehandlungen werden in der Schweiz nicht von der Grundversicherung übernommen, und viele lernen das auf die harte Tour",
        description: "In Spanien deckt die Sozialversicherung einige grundlegende Zahnleistungen ab. Es ist nicht viel, aber es gibt etwas. In der Schweiz deckt die obligatorische…",
        keywords: "zahnarztbehandlungen, werden, schweiz, nicht, grundversicherung, übernommen, viele, lernen",
        keywordsLocalized: true,
        category: "Gesundheit und KVG",
        hub: "salud",
        slug: "actualidad-2026-05-03-3-el-dentista-en-suiza-no-lo-cubre-el-seguro-basico-y-muchos-lo-descubre",
        readingTime: 1,
        dateUpdated: "3. Mai 2026",
        featuredImage: "media/guides/actualidad-dentista-suiza-seguro.jpg",
        imageAlt: undefined,
        summary: "In Spanien deckt die Sozialversicherung einige grundlegende Zahnleistungen ab. Es ist nicht viel, aber es gibt etwas. In der Schweiz deckt die…",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>In Spanien deckt die Sozialversicherung einige grundlegende Zahnleistungen ab. Es ist nicht viel, aber es gibt etwas. In der Schweiz deckt die obligatorische Krankenversicherung (KVG/LAMal) praktisch nichts ab, was in einer Zahnarztpraxis passiert. Es lohnt sich, das zu wissen, bevor man sich in den Stuhl setzt.</p>
<p>Das KVG übernimmt Zahnbehandlungen nur in drei konkreten Situationen: wenn sie Folge einer schweren Erkrankung des Kauapparates sind, wenn sie zur Behandlung einer anderen schweren Krankheit notwendig sind oder wenn der Schaden durch einen Unfall verursacht wurde und keine andere Versicherung einspringt. Ausserhalb dieser drei Ausnahmen greift das KVG nicht. Eine Karies, eine Extraktion, eine Reinigung, eine Erwachsenenorthopädie, ein Implantat, eine Krone: Das alles geht direkt zu Ihren Lasten.</p>
<p>Die Preise entsprechen dem Landesniveau. Eine Basiszahnreinigung kostet rund 150-200 Franken. Eine Extraktion kann zwischen 200 und 500 kosten. Ein vollständiges Implantat zwischen 3'000 und 5'000 Franken pro Einheit. Eine Erwachsenenorthopädie kann 10'000 Franken übersteigen.</p>
<p>Es ist möglich, eine Zahnzusatzversicherung abzuschliessen, aber sie hat ihre eigenen Regeln: Fast alle Versicherer verlangen eine Wartezeit von mehreren Monaten, bevor die Deckung beginnt, sie haben eine Jahreshöchstgrenze (in der Regel zwischen 50 und 75 % der Kosten, bis zu einem Maximum von 1'000 bis 3'000 Franken pro Jahr) und decken keine Behandlungen ab, die bei Vertragsabschluss bereits laufen.</p>
<p>Der praktische Rat: Schliessen Sie sie ab, wenn Sie ankommen, bevor es irgendwelche Probleme gibt. Und gehen Sie regelmässig zum Zahnarzt; hier ist Vorsorge viel günstiger als Behandlung.</p>
</div>`
    },

    "fb-daily-20260502-01-el-seguro-de-accidentes-en-suiza-no-cubre-todo-lo-que-te-pasa-en-el-tr": {
        title: "Die Unfallversicherung in der Schweiz deckt nicht alles ab, was Ihnen am Arbeitsplatz passiert",
        description: "Viele Spanier, die hier arbeiten gehen, gehen davon aus, dass die „Unfallversicherung“ alle arbeitsbedingten Gesundheitsprobleme abdeckt. Diese Verwirrung...",
        keywords: "Versicherung, Unfälle, Schweiz, Deckungen, alles, passiert, Arbeit, viele",
        keywordsLocalized: true,
        category: "Gesundheit und KVG",
        hub: "salud",
        slug: "actualidad-2026-05-02-1-el-seguro-de-accidentes-en-suiza-no-cubre-todo-lo-que-te-pasa-en-el-tr",
        readingTime: 2,
        dateUpdated: "2. Mai 2026",
        featuredImage: "media/guides/actualidad-seguro-accidentes-trabajo-suiza.jpg",
        imageAlt: "Einschränkungen der Arbeitsunfallversicherung in der Schweiz",
        summary: "Viele Spanier, die hier arbeiten gehen, gehen davon aus, dass die „Unfallversicherung“ alle arbeitsbedingten Gesundheitsprobleme abdeckt. Das…",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>Viele Spanier, die zur Arbeit hierher kommen, gehen davon aus, dass die „Unfallversicherung“ alle gesundheitlichen Probleme im Zusammenhang mit der Arbeit abdeckt. Diese Verwirrung kann teuer sein. Das Bundesunfallversicherungsgesetz (Unfallversicherungsgesetz) unterscheidet zwei klar getrennte Kategorien: den Unfall, bei dem es sich um ein plötzliches, äußeres und unfreiwilliges Ereignis handelt, und die Berufskrankheit, bei der es sich um eine Krankheit handelt, die hauptsächlich durch die Arbeitstätigkeit verursacht wird. Der Unterschied ist nicht semantisch: Er hängt davon ab, wer wie viel und wie lange zahlt.</p>
<p>Damit die SUVA eine Krankheit als berufsbedingt anerkennt, muss sie bei den in der amtlichen Liste aufgeführten Krankheiten zu mehr als 50 % und bei den nicht aufgeführten Krankheiten zu mehr als 75 % durch Arbeit verursacht sein. In diese Kategorie können Beschwerden fallen, die auf Dauerlärm zurückzuführen sind, bestimmte Hautallergien, Atemwegserkrankungen aufgrund der Exposition gegenüber toxischen Substanzen oder Erkrankungen des Bewegungsapparats im Zusammenhang mit der Arbeitshaltung. Die Erkennung erfolgt jedoch nicht automatisch und der Prozess kann Monate dauern. Viele Arbeitnehmer befinden sich in dieser Zeit in einer Grauzone, die ihnen niemand erklärt hat.</p>
<p>Das eigentliche Problem entsteht, wenn die SUVA feststellt, dass die Arbeitsursache nicht den erforderlichen Schwellenwert erreicht. In diesem Fall fällt die Krankheit in den Verantwortungsbereich der Krankenversicherung (KVG), die die Behandlungskosten übernimmt, jedoch keinen Gehaltsersatz garantiert. Die finanzielle Entschädigung während des Krankheitsurlaubs hängt dann vom Einzelvertrag ab und davon, ob der Arbeitgeber eine Taggeldversicherung (LCA) abgeschlossen hat, die in der Schweiz nicht obligatorisch ist.</p>
<p>Das Wichtigste: Wenn Sie den Verdacht haben, dass ein gesundheitliches Problem seinen Ursprung in Ihrer Arbeit hat, benachrichtigen Sie schnellstmöglich Ihren Arbeitgeber und verlangen Sie, dass er es bei der SUVA als mögliche Berufskrankheit meldet. Es gibt Verjährungsfristen, und wenn sie einmal verstrichen sind, ist die Wiederherstellung dieser Rechte praktisch unmöglich. Zur Abgabe der Erklärung ist der Arbeitgeber verpflichtet, Sie können diese aber auch direkt anfordern.</p>
</div>`
    },

    "fb-daily-20260502-02-tener-un-perro-en-suiza-cuesta-mas-de-lo-que-parece": {
        title: "Ein Hund in der Schweiz zu haben kostet mehr, als es scheint",
        description: "In Spanien ist die Anschaffung eines Hundes mit einigen bekannten Kosten verbunden: Tierarzt, Futter, Chip. In der Schweiz kommt noch ein Steuerverfahren hinzu, das fast jeden überrascht...",
        keywords: "mit, Hund, Schweiz, Kosten, scheint, Spanien, impliziert, einige",
        keywordsLocalized: true,
        category: "Leben in der Schweiz",
        hub: "vivir-en-suiza",
        slug: "actualidad-2026-05-02-2-tener-un-perro-en-suiza-cuesta-mas-de-lo-que-parece",
        readingTime: 2,
        dateUpdated: "2. Mai 2026",
        featuredImage: "media/guides/actualidad-perro-suiza-coste.jpg",
        imageAlt: "Kosten für den Besitz eines Hundes in der Schweiz, Steuern und Registrierung",
        summary: "In Spanien ist die Anschaffung eines Hundes mit einigen bekannten Kosten verbunden: Tierarzt, Futter, Chip. In der Schweiz kommt noch ein Steuerverfahren hinzu, das fast überrascht...",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>In Spanien ist die Anschaffung eines Hundes mit einigen bekannten Kosten verbunden: Tierarzt, Futter, Chip. In der Schweiz müssen wir ein Steuerverfahren hinzufügen, das fast alle Neulinge überrascht: die Hundesteuer. Es ist kein Witz oder eine urbane Legende. Es handelt sich um eine echte Steuer, die je nach Kanton auf kantonaler oder kommunaler Ebene verwaltet wird und jährlich gezahlt wird, solange Sie das Tier haben.</p>
<p>Die Beträge variieren stark je nach Wohnort. Im Kanton Waadt beträgt die kantonale Gebühr 100 Franken pro Jahr und Hund, inklusive Anmeldung. In Genf kombiniert die Berechnung eine kantonale Steuer, kommunale Zusatzsteuern und einen Festsatz, was zu ähnlichen oder höheren Beträgen führen kann. Einige Kantone im Zentrum des Landes wie Zug oder Schwyz haben sich gegen eine Anwendung entschieden, sind aber die Ausnahme. Der erste Schritt ist in jedem Kanton immer derselbe: Registrieren Sie das Tier in der landesweit obligatorischen nationalen AMICUS-Datenbank. Nach der Registrierung sendet Ihnen die Verwaltung die Jahresrechnung zu.</p>
<p>Über den steuerlichen Aspekt hinaus gibt es Verkehrsregeln, die in der Praxis viel strenger sind als in Spanien. In vielen öffentlichen Räumen, Parks, Seestränden und öffentlichen Verkehrsmitteln muss der Hund an der Leine geführt werden. In Zürich gibt es Freilaufgebiete, doch in den meisten Kantonen sind die Beschränkungen streng und die Strafen bei Nichteinhaltung hoch. Die Bauordnung (Hausordnung) regelt meist auch die Stunden, in denen das Tier Lärm machen darf, und Schweizer Nachbarn sind meist keine, die wegschauen.</p>
<p>Für diejenigen, die den Hund aus Spanien mitbringen, gelten die im Schengen-Raum üblichen tierärztlichen Anforderungen: gültiger europäischer Heimtierausweis, Identifikations-Mikrochip und gültiger Tollwutimpfstoff. Sobald Sie in der Schweiz angekommen sind, müssen Sie einen örtlichen Tierarzt finden, die Registrierung bei AMICUS abschließen und die Gemeindesteuer begleichen. Es ist Bürokratie, keine Komplikation, aber es braucht Zeit und hat seine Termine.</p>
</div>`
    },

    "fb-daily-20260502-03-trabajar-a-tiempo-parcial-en-suiza-no-es-solo-cobrar-la-mitad": {
        title: "Bei Teilzeitarbeit geht es in der Schweiz nicht nur darum, die Hälfte zu verdienen",
        description: "Wenn jemand in der Schweiz einen 60-Prozent- oder 80-Prozent-Vertrag aushandelt, denkt er meist in einfachen Worten: weniger Stunden, weniger Lohn, mehr Freizeit. Was fast niemand...",
        keywords: "Arbeit, Zeit, Teilzeit, Schweiz, allein, bezahlt werden, die Hälfte, wann",
        keywordsLocalized: true,
        category: "Arbeit und Löhne",
        hub: "trabajo",
        slug: "actualidad-2026-05-02-3-trabajar-a-tiempo-parcial-en-suiza-no-es-solo-cobrar-la-mitad",
        readingTime: 2,
        dateUpdated: "2. Mai 2026",
        featuredImage: "media/guides/actualidad-tiempo-parcial-suiza.jpg",
        imageAlt: "Teilzeitarbeit in der Schweiz und soziale Rechte",
        summary: "Wenn jemand in der Schweiz einen 60-Prozent- oder 80-Prozent-Vertrag aushandelt, denkt er meist in einfachen Worten: weniger Stunden, weniger Lohn, mehr Freizeit. Welche…",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>Wenn jemand in der Schweiz einen 60-Prozent- oder 80-Prozent-Vertrag aushandelt, denkt er meist in einfachen Worten: weniger Stunden, weniger Lohn, mehr Freizeit. Was Ihnen fast niemand erklärt, ist, dass dieser Rentenprozentsatz direkte Auswirkungen auf die Rente, die Unfallversicherung und den Schutz bei Arbeitslosigkeit hat und dass einige dieser Konsequenzen dauerhaft sind.</p>
<p>Die zweite Säule, die Pensionskasse (BVG), hat eine verpflichtende Zugangsschwelle. Im Jahr 2026 ist nur noch obligatorisch versichert, wer mehr als 22'680 Franken pro Jahr beim gleichen Arbeitgeber verdient. Ein 30- oder 40-Prozent-Vertrag in einem durchschnittlichen Lohnbereich kann leicht unter diese Schwelle fallen, was bedeutet, dass diese Arbeitsjahre nichts in der zweiten Säule ansammeln. Die LPP-Reform, die diese Schwelle gesenkt hätte, wurde im September 2024 in einem Referendum abgelehnt, sodass sich an der Situation nichts geändert hat. Diese Situation betrifft überproportional Frauen, die in der Schweiz am häufigsten Teilzeit arbeiten.</p>
<p>Ein weiterer Punkt, der unbemerkt bleibt, ist die nichtberufliche Unfallversicherung. Wenn Sie weniger als acht Stunden pro Woche für denselben Arbeitgeber arbeiten, ist dieser Arbeitgeber nicht verpflichtet, Ihre Unfälle außerhalb der Arbeit zu decken. Bei einem Bruch auf der Skipiste oder beim Fahrradfahren am Wochenende übernimmt das LAMal die medizinischen Kosten, die SUVA greift jedoch nicht ein. Wer zu einem sehr geringen Prozentsatz arbeitet oder mehrere Minijobs kombiniert, ist möglicherweise nicht für jeden von ihnen abgesichert.</p>
<p>Bei Arbeitslosigkeit wird die Entschädigung im Verhältnis zum erhaltenen Gehalt berechnet. Wenn Sie bei 50 % Ihren Arbeitsplatz verloren haben, basiert die Arbeitslosigkeit auf diesen 50 %. Wenn Sie mehrere Nebenjobs haben und einen davon verlieren, wird nur dieser Teil berücksichtigt. Dies alles ist kein Grund, einen Teilvertrag abzulehnen, es handelt sich jedoch um Informationen, die Sie vor der Unterzeichnung und nicht nach dem Verlust Ihres Arbeitsplatzes einholen sollten.</p>
</div>`
    },

    "fb-daily-20260501-04-delinquir-en-suiza-hasta-que-se-acaba-la-paciencia": {
        title: "Kriminalität in der Schweiz bis zum Ende der Geduld",
        description: "Ich habe diese Nachrichten aus dem Wallis gesehen und denke, dass sie es wert sind, über die einfache Schlagzeile hinaus gelesen zu werden. Es handelt sich nicht um einen Einzelfall oder eine \"Sensation\". Es ist ziemlich repräsentativ dafür, wie r…",
        keywords: "Verbrechen, Schweiz, bis, es endet, Geduld, gesehen, Nachrichten, Wallis",
        keywordsLocalized: true,
        category: "Warnungen und Ressourcen",
        hub: "recursos",
        slug: "actualidad-2026-05-01-4-delinquir-en-suiza-hasta-que-se-acaba-la-paciencia",
        readingTime: 2,
        dateUpdated: "1 Mai 2026",
        featuredImage: "media/guides/actualidad-delinquir-suiza-justicia.jpg",
        imageAlt: "Schweizer Justizsystem und Konsequenzen für Ausländer",
        summary: "Ich habe diese Nachrichten aus dem Wallis gesehen und denke, dass sie es wert sind, über die einfache Schlagzeile hinaus gelesen zu werden. Es handelt sich nicht um einen Einzelfall oder eine \"Sensation\". Es ist ziemlich repräsentativ ...",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>Ich habe diese Nachrichten aus dem Wallis gesehen und denke, dass sie es wert sind, über die einfache Schlagzeile hinaus gelesen zu werden. Es handelt sich nicht um einen Einzelfall oder eine "Sensation". Es ist ziemlich repräsentativ dafür, wie das Schweizer System wirklich funktioniert, wenn jemand Verbrechen aneinanderreiht.</p>
<p>Wir sprechen von einer Person, die Dutzende von Verstößen begangen hat: Diebstähle, Sachbeschädigungen, illegale Einreisen... es handelt sich nicht um einen konkreten Fehler oder eine schlechte Nacht. Es ist eine Dynamik. Und die letzte Reaktion ist die, die früher oder später hier eintrifft: strafrechtliche Verurteilung und Ausweisung aus dem Land.</p>
<p>Und hier kommt das Interessante. Weil viele mit zwei ziemlich weit verbreiteten Vorstellungen weitermachen, die nicht zur Schweizer Realität passen.</p>
<p>Das erste: "In der Schweiz kann man machen, was man will." Nein. Was passiert, ist, dass das System eine Garantie ist. Sie weisen dich nicht gleich beim ersten Mal raus. In jedem Fall gilt der Grundsatz der Verhältnismäßigkeit, der bedeutet, dass die Gerichte die Situation analysieren, bevor sie eine so schwerwiegende Maßnahme wie eine Ausweisung ergreifen. Dies ist keine Improvisation, sondern die Grundlage des Schweizer Rechts und der Anwendung des Einwanderungsgesetzes durch das Secrétariat d'État aux migrations (SEM).</p>
<p>Der zweite: "Sie weisen dich für alles raus." Weder. Um an diesen Punkt zu gelangen, benötigen Sie Schwerkraft oder Akkumulation. Und in diesem Fall wurde diese Grenze eindeutig überschritten.</p>
<p>Seit der Reform der Ausländerausweisung nach Straftaten gibt es Situationen, in denen Richter praktisch keinen Spielraum mehr haben: Bei Vorliegen bestimmter Kriterien wird die Ausweisung vollstreckt. Und es kann auch mit einem jahrelangen Einreiseverbot einhergehen.</p>
<p>Letztlich ist die Schweizer Logik ganz klar, auch wenn sie von außen manchmal schwer zu erkennen ist: anfängliche Toleranz, aber null Geduld mit Rückfällen.</p>
<p>Praktische Übersetzung für uns: Zur Arbeit kommen, sein Leben leben und sich an die Regeln halten ist kein Problem. Aber in der Schweiz in eine Kriminalitätsspirale zu geraten, ist keine gute Idee. Es scheint, als würde nichts passieren ... bis alles plötzlich passiert.</p>
<p>Quelle: Watson-Artikel https://www.watson.ch/.../225591197-il-a-commis-des...</p>
</div>`
    },

    "fb-daily-20260501-05-en-suiza-la-puntualidad-es-clave-si-pero-otra-cosa-distinta-es-regalar-tiempo": {
        title: "In der Schweiz ist Pünktlichkeit das A und O, ja. Aber eine andere Sache ist, Zeit zu geben. ",
        description: "In den meisten Gesundheitseinrichtungen (Krankenhäusern oder Rettungsdiensten) ist das Prinzip ganz klar: Die Arbeitszeit beginnt, wenn Sie dem Arbeitgeber zur Verfügung stehen. Und dazu gehört auch, …",
        keywords: "Schweiz, Pünktlichkeit, Schlüssel, Sache, anders, geben, Zeit, Mehrheit",
        keywordsLocalized: true,
        category: "Leben in der Schweiz",
        hub: "vivir-en-suiza",
        slug: "actualidad-2026-05-01-5-en-suiza-la-puntualidad-es-clave-si-pero-otra-cosa-distinta-es-regalar-tiempo",
        readingTime: 1,
        dateUpdated: "1 Mai 2026",
        featuredImage: "media/guides/actualidad-puntualidad-cultura-suiza.jpg",
        imageAlt: "Pünktlichkeitskultur in der Schweiz und Arbeitszeiten",
        summary: "In den meisten Gesundheitseinrichtungen (Krankenhäusern oder Rettungsdiensten) ist das Prinzip ganz klar: Die Arbeitszeit beginnt, wenn Sie dem Arbeitgeber zur Verfügung stehen.…",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>In den meisten Gesundheitseinrichtungen (Krankenhäusern oder Rettungsdiensten) ist das Prinzip ganz klar: Die Arbeitszeit beginnt, wenn Sie dem Arbeitgeber zur Verfügung stehen. Und dazu gehört auch, dass Sie bereits gewechselt werden, wenn die Uniform Pflicht ist und Sie diese nicht von zu Hause aus tragen können. Es handelt sich dabei nicht um ein nebensächliches Detail, sondern ergibt sich aus dem schweizerischen Arbeitsrecht und der gängigen Praxis in internen Vereinbarungen und Vorschriften.</p>
<p>Einfach gesagt: - Wenn Ihre Schicht um 07:00 Uhr beginnt, müssen Sie um 07:00 Uhr einsatzbereit sein. - Allerdings sollte die Zeit, die für den Wechsel benötigt wird, wenn er in der Einrichtung zwingend durchgeführt werden muss, in die Arbeitszeit eingerechnet werden.</p>
<p>Eine andere Sache ist die Realität des täglichen Lebens, in der viele Teams mit dem "kulturellen Druck" arbeiten, früh anzukommen, schnell zu wechseln und um 07:00 Uhr in Kontakt zu sein, als ob es normal wäre. Es kommt vor, ja. Aber es macht die Praxis nicht richtig.</p>
<p>Zudem kommt es stark auf den Kanton, das Spital und die Vereinbarung an (in einigen Branchen CCT). In großen öffentlichen Krankenhäusern ist es meist stärker reguliert. Im kleinen oder privaten EMS gelinde gesagt "flexibler".</p>
<p>Also ja, Schweizer Pünktlichkeit: absolut.</p>
<p>Pünktlichkeit bedeutet jedoch nicht, jeden Tag fünf oder zehn Minuten lang kostenlos zu arbeiten.</p>
<p>Und das ist für jemanden, der einen Job beginnt, der Schlüssel zum Verständnis. Sich an das System anpassen, ja, aber ohne Dinge zu normalisieren, die nicht normal sind.</p>
</div>`
    },

    "fb-daily-20260501-06-desde-suiza-para-el-mundo": {
        title: "Von der Schweiz in die Welt",
        description: "An diesem Tag vor 33 Jahren stellte das CERN die World-Wide-Web-Software der Öffentlichkeit zur Verfügung. Die Idee wurde ursprünglich von Sir Tim Berners-Lee vorgeschlagen, damit Wissenschaftler…",
        keywords: "Schweiz, Welt, vor Jahren, CERN, Put, Software, Welt",
        keywordsLocalized: true,
        category: "Kultur und Veranstaltungen",
        hub: "cultura-eventos",
        slug: "actualidad-2026-05-01-6-desde-suiza-para-el-mundo",
        readingTime: 1,
        dateUpdated: "1 Mai 2026",
        featuredImage: "media/guides/actualidad-desde-suiza-para-el-mundo.jpg",
        imageAlt: "Spanische Expatriate-Community in der Schweiz mit einer globalen Vision",
        summary: "An diesem Tag vor 33 Jahren stellte das CERN die World-Wide-Web-Software der Öffentlichkeit zur Verfügung. Die Idee wurde ursprünglich von Sir Tim Berners-Lee vorgeschlagen…",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>An diesem Tag vor 33 Jahren stellte das CERN die World-Wide-Web-Software der Öffentlichkeit zur Verfügung.</p>
<p>Die Idee wurde von Sir Tim Berners-Lee ursprünglich vorgeschlagen, damit Wissenschaftler und Institute, die mit CERN-Daten arbeiten, Informationen schnell und genau auf der ganzen Welt austauschen können.</p>
<p>Später veröffentlichte CERN auch eine offene Lizenzversion, um die Verbreitung weiter zu erleichtern.</p>
<p>Auf den Bildern sehen Sie:</p>
<p>1 und 2. Das Dokument, das das World Wide Web offiziell öffentlich zugänglich machte. 3. Tim Berners-Lees NeXT-Computer, der als ursprünglicher Webserver und Entwicklungsmaschine für das WWW-Projekt diente. 4. Ein Screenshot der Neugestaltung der ersten Website.</p>
<p>Und so begann in einem CERN-Büro in Meyrin (Genf) etwas, das wir heute für fast alles nutzen. Sogar auf Facebook mit Leuten zu streiten, die die Quellen nicht lesen. Dinge des Fortschritts.</p>
<p>Quelle: CERN https://home.web.cern.ch/science/computing/birth-web</p>
</div>`
    },

    "fb-daily-20260502-04-si-manana-desaparecieran-los-inmigrantes-en-suiza-y-en-el-resto-de-europa": {
        title: "Wenn morgen Einwanderer in der Schweiz verschwinden würden... und im übrigen Europa",
        description: "Wenn morgen Einwanderer in der Schweiz verschwinden... und im Rest Europas... Eine Überlegung, die ich schon seit einiger Zeit objektiv zu verfassen versuche. Diese Debatte wird normalerweise angestoßen ...",
        keywords: "Morgen, verschwinden, Einwanderer, Schweiz, Ruhe, Europa, Reflexion, tragen",
        keywordsLocalized: true,
        category: "Arbeit und Löhne",
        hub: "trabajo",
        slug: "actualidad-2026-05-02-4-si-manana-desaparecieran-los-inmigrantes-en-suiza-y-en-el-resto-de-europa",
        readingTime: 2,
        dateUpdated: "2 Mai 2026",
        featuredImage: "media/guides/actualidad-inmigrantes-suiza-economia.jpg",
        imageAlt: "Wirtschaftliche Auswirkungen von Einwanderern in der Schweiz und in Europa",
        summary: "Wenn morgen Einwanderer in der Schweiz verschwinden... und im Rest Europas... Eine Überlegung, die ich schon seit einiger Zeit objektiv zu verfassen versuche. Das muss…",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>Wenn morgen Einwanderer in der Schweiz verschwinden... und im Rest Europas... Eine Überlegung, die ich schon seit einiger Zeit objektiv zu verfassen versuche.</p>
<p>Diese Debatte wird oft so dargestellt, als wäre sie etwas "typisches für die Schweiz". Das ist es nicht. Die Schweiz ist nur ein sehr sichtbares Beispiel für etwas, das in fast ganz Europa passiert.</p>
<p>Hier wären die Auswirkungen unmittelbar: Personalmangel im Gesundheitswesen, im Baugewerbe, im Gastgewerbe, in der Industrie ... und direkter Druck auf die AVS, weil ein wichtiger Teil der Beitragszahler verschwindet. Aber wenn man sich Deutschland anschaut, sagt man dort seit Jahren offen, dass es Hunderttausende ausländischer Arbeitskräfte braucht, um seine Wirtschaft aufrechtzuerhalten. In Italien ist das Problem noch offensichtlicher: alternde Bevölkerung und entleerende Regionen. Und in Spanien hängt die soziale Sicherheit zunehmend vom Beitrag ausländischer Arbeitnehmer ab, um das System auszugleichen.</p>
<p>Der Unterschied liegt nicht im Inhalt, sondern in der Form. Die Schweiz merkt das schnell, weil ihr Arbeitsmarkt klein und sehr spezialisiert ist und zudem stark auf Grenzgänger angewiesen ist. Andere Länder haben aufgrund ihrer Größe einen größeren Spielraum, aber das Problem ist genau das gleiche: weniger Geburten, mehr Rentner und weniger Menschen im erwerbsfähigen Alter.</p>
<p>Um es klar auszudrücken: Wenn man heute in Europa die Einwanderung abschafft, "befiehlt" man nichts. Sie entfernen einen Teil, der bereits Teil der Bilanz ist.</p>
<p>Und nun der Teil, der in diesen Debatten fast nie erwähnt wird: Jeder Spanier, der in der Schweiz arbeitet, ist ein Beitrag weniger in Spanien. Das heißt, Spanien verliert einen Beitragszahler und die Schweiz gewinnt einen. So einfach ist das.</p>
<p>Wenn man es als Ganzes betrachtet, ist es das gleiche Phänomen, nur von der anderen Seite betrachtet. Länder wie Spanien, Italien oder Portugal verlieren seit Jahren Erwerbsbevölkerung, die schließlich in der Schweiz, in Deutschland oder in Frankreich arbeitet. Dabei handelt es sich nicht um Randgruppen, sondern in vielen Fällen um junge Menschen im Beitragsalter, genau um diejenigen, die jedes Rentensystem am meisten braucht.</p>
<p>Hier geht es nicht um "sie nehmen von uns" oder "sie geben uns". Es geht um Flüsse. Europa funktioniert als vernetztes System, in dem einige Länder Arbeitskräfte exportieren und andere sie absorbieren. Und innerhalb dieses Systems sind viele derjenigen, die die Einwanderung in der Schweiz kritisieren, Teil genau desselben Mechanismus.</p>
</div>`
    },

    "fb-daily-20260502-05-beber-en-la-calle-en-suiza-el-problema-no-es-la-cerveza-es-lo-que-dejas-detras": {
        title: "Trinken auf der Straße in der Schweiz: Das Problem ist nicht das Bier, sondern das, was man zurücklässt! ",
        description: "Dies ist das Ergebnis einer Nachricht in einer unserer WhatsApp-Gruppen und ich denke, dass dies eine Überlegung wert ist. Dieses Foto ist keine Ausnahme, es passiert, wenn man \"erlaubt\" mit… verwechselt.",
        keywords: "trinken, Straße, Schweiz, Problem, Bier, zurücklassen, kommt",
        keywordsLocalized: true,
        category: "Leben in der Schweiz",
        hub: "vivir-en-suiza",
        slug: "actualidad-2026-05-02-5-beber-en-la-calle-en-suiza-el-problema-no-es-la-cerveza-es-lo-que-dejas-detras",
        readingTime: 1,
        dateUpdated: "2 Mai 2026",
        featuredImage: "media/guides/actualidad-alcohol-calle-suiza.jpg",
        imageAlt: "Koexistenzregeln zu Alkohol im öffentlichen Raum in der Schweiz",
        summary: "Dies ist das Ergebnis einer Nachricht in einer unserer WhatsApp-Gruppen und ich denke, dass dies eine Überlegung wert ist. Dieses Foto ist keine Ausnahme, es ist das, was passiert, wenn Sie…",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>Dies ist das Ergebnis einer Nachricht in einer unserer WhatsApp-Gruppen und ich denke, dass dies eine Überlegung wert ist.</p>
<p>Dieses Foto ist keine Ausnahme, es ist das, was passiert, wenn man "erlaubt" mit "alles ist möglich" verwechselt. In der Schweiz darf man an vielen öffentlichen Orten trinken, ja. Dazu gehört aber nicht, einen See in eine Mülltonne im Freien zu verwandeln. Und hier kommt der Kulturschock, den viele nicht erleben wollen.</p>
<p>Weil die ungeschriebene Regel besser funktioniert als das Gesetz. Niemand wird Ihnen etwas dafür sagen, dass Sie mit Freunden am See etwas trinken gehen. Von Ihnen wird jedoch erwartet, dass Sie die Website genauso oder besser verlassen, als Sie sie vorgefunden haben. Es ist keine "Postkarten"-Höflichkeit; Es ist das Mindeste, dass dieses Modell ohne massive Verbote funktioniert. Wenn das fehlschlägt, kommen Einschränkungen, Bußgelder ... und dann die üblichen Beschwerden.</p>
<p>Und das Merkwürdigste: Dann sind wir die Ersten, die sagen, dass die Schweiz sauber ist, dass alles funktioniert, dass es angenehm ist. Ja natürlich. Aber es ist keine Zauberei. Es ist einfach so, dass die Mehrheit ihre eigenen sammelt. Wenn sich das ändert, ändert sich auch das System. So einfach ist das.</p>
</div>`
    },

    "fb-daily-20260502-06-reclamacion-de-salario-en-suiza-plazos-claros-y-sin-vueltas": {
        title: "Lohnantrag in der Schweiz - klare Fristen und keine Fragen",
        description: "Wenn Ihr Arbeitgeber Sie nicht bezahlt, besteht kein Grund, die Situation in die Länge zu ziehen, \"um zu sehen, ob das Problem behoben wird\". Das Gehalt ist eine gesetzliche Verpflichtung im Sinne des Schweizerischen Obligationenrechts (OR), und hier, was funktioniert…",
        keywords: "Anspruch, Gehalt, Schweiz, Fristen, klar, Rückgaben, Arbeitgeber, Bezahlung",
        keywordsLocalized: true,
        category: "Arbeit und Löhne",
        hub: "trabajo",
        slug: "actualidad-2026-05-02-6-reclamacion-de-salario-en-suiza-plazos-claros-y-sin-vueltas",
        readingTime: 1,
        dateUpdated: "2 Mai 2026",
        featuredImage: "media/guides/actualidad-reclamacion-salario-suiza.jpg",
        imageAlt: "Fristen zur Geltendmachung ausstehender Löhne in der Schweiz",
        summary: "Wenn Ihr Arbeitgeber Sie nicht bezahlt, besteht kein Grund, die Situation in die Länge zu ziehen, \"um zu sehen, ob das Problem behoben wird\". Der Lohn ist eine gesetzliche Verpflichtung im Sinne des Schweizerischen Obligationenrechts (OR).",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>Wenn Ihr Arbeitgeber Sie nicht bezahlt, besteht kein Grund, die Situation in die Länge zu ziehen, "um zu sehen, ob das Problem behoben wird". Das Gehalt ist eine gesetzliche Verpflichtung nach dem Schweizerischen Obligationenrecht (OR), und was hier funktioniert, ist, es schriftlich und unter Einhaltung von Fristen zu erledigen. Erster Schritt: Senden Sie eine formelle Mitteilung (Post oder Brief) mit Angabe des fälligen Betrags und der geleisteten Arbeitszeit, mit einer Frist von 5 bis 10 Tagen. Erfolgt keine Reaktion, erfolgt eine zweite Mahnung mit einer Frist von 7, maximal 10 Tagen, in der klargestellt wird, dass bei Nichtzahlung Maßnahmen ergriffen werden.</p>
<p>Wenn nach dieser zweiten Frist keine Zahlung erfolgt, warten oder verhandeln Sie nicht endlos weiter. Sie gehen direkt zum Inkassoverfahren beim Betreibungsamt. Sie brauchen keinen Anwalt, um loszulegen, und es ist der Standardkanal in der Schweiz. Wenn der Arbeitgeber Einspruch erhebt, ist bereits das arbeitsgerichtliche Verfahren zur Geltendmachung dieses Widerspruchs eingeleitet.</p>
<p>Eine wichtige Tatsache, die viele erst spät entdecken: Der Lohnanspruch erlischt nach 5 Jahren (Art. 128 OR). Aber das ist keine Aufforderung zum Warten, im Gegenteil. Je früher Sie handeln, desto mehr reale Optionen müssen Sie sammeln. Der Ansatz hier ist einfach: Schreiben, Kurzfristigkeit und Handeln. Wenn sie nicht zahlen, liegt das meist nicht an Unachtsamkeit.</p>
</div>`
    },

    "fb-daily-20260503-04-el-sms-blaster-ya-esta-en-suiza-y-si-es-tan-simple-como-peligroso": {
        title: "Der \"SMS Blaster\" ist bereits in der Schweiz. Und ja, es ist so einfach wie gefährlich. ",
        description: "Laut RTS ermöglicht diese neue Betrugsmethode Kriminellen, Massen-SMS-Nachrichten direkt an Mobiltelefone in der Nähe zu senden, ohne den Umweg über Betreiber oder Kontaktlisten. Es ist…",
        keywords: "Blaster, Schweiz, einfach, gefährlich, neu, Methode, Betrug, erlaubt",
        keywordsLocalized: true,
        category: "Warnungen und Ressourcen",
        hub: "recursos",
        slug: "actualidad-2026-05-03-4-el-sms-blaster-ya-esta-en-suiza-y-si-es-tan-simple-como-peligroso",
        readingTime: 1,
        dateUpdated: "3 Mai 2026",
        featuredImage: "media/guides/actualidad-sms-blaster-fraude-suiza.jpg",
        imageAlt: "SMS-Blaster-Betrug und Phishing in der Schweiz",
        summary: "Laut RTS ermöglicht diese neue Betrugsmethode Kriminellen, Massen-SMS-Nachrichten direkt an Mobiltelefone in der Nähe zu senden, ohne den Umweg über Betreiber oder Listen …",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>Laut RTS ermöglicht diese neue Betrugsmethode Kriminellen, Massen-SMS-Nachrichten direkt an Mobiltelefone in der Nähe zu senden, ohne den Umweg über Betreiber oder Kontaktlisten. Das heißt, sie brauchen Ihre Nummer nicht. Es reicht aus, wenn sie sich physisch in der Nähe befinden, beispielsweise an einem Bahnhof, einem Flughafen oder sogar in einem Einkaufszentrum.</p>
<p>Der Trick ist recht einfach, funktioniert aber: Sie erhalten eine SMS, die offiziell aussieht - Bank, Post, Zoll, sogar Behörden - mit einem Link. Und hier beginnt das Problem. Da die Nachricht nicht über herkömmliche Netzwerke läuft, werden viele Sicherheitsfilter umgangen. Ergebnis: mehr Glaubwürdigkeit, mehr Klicks ... und mehr Opfer.</p>
<p>Das ändert eine wichtige Sache: Bisher konnte man denken: "Wenn ich meine Nummer nicht gebe, ist alles in Ordnung." Nicht mehr. Hier erfolgt der Angriff durch Nähe, als ob jemand ein unsichtbares Netzwerk für alle Mobiltelefone in der Umgebung eingerichtet hätte. Und in Gebieten wie Genf mit so viel Verkehr ist das Gelände perfekt.</p>
<p>Praktische Ratschläge, ohne Dramatisierung, aber ohne Naivität: 1. Klicken Sie nicht auf SMS-Links, auch wenn diese offiziell aussehen 2. Verifizieren Sie immer über offizielle Kanäle (Bank-App, direkte Website) 3. Seien Sie besonders vorsichtig bei dringenden oder dringenden Nachrichten</p>
<p>In der Schweiz funktioniert alles gut... bis es nicht mehr funktioniert. Und wenn es um Betrug geht, sind sie immer einen Schritt voraus.</p>
</div>`
    },

    "fb-daily-20260503-05-el-queroseno-empieza-a-convertirse-en-un-problema-real-y-esta-vez-si-tiene-conse": {
        title: "Kerosin wird langsam zu einem echten Problem ... und dieses Mal hat es Konsequenzen",
        description: "In den Nachrichten geht es nicht um ferne Szenen. Wenn die Kerosinversorgung kompliziert wird, macht sich das schnell in der Tasche und in den Plänen bemerkbar. Fluggesellschaften arbeiten mit geringeren Margen…",
        keywords: "Kerosin, beginnt, wird, Problem, real, hat, Konsequenzen, Neuigkeiten",
        keywordsLocalized: true,
        category: "Leben in der Schweiz",
        hub: "vivir-en-suiza",
        slug: "actualidad-2026-05-03-5-el-queroseno-empieza-a-convertirse-en-un-problema-real-y-esta-vez-si-tiene-conse",
        readingTime: 1,
        dateUpdated: "3 Mai 2026",
        featuredImage: "media/guides/actualidad-queroseno-contaminacion-suiza.jpg",
        imageAlt: "Flugkerosinverschmutzung in der Schweiz",
        summary: "In den Nachrichten geht es nicht um ferne Szenen. Wenn die Kerosinversorgung kompliziert wird, macht sich das schnell in der Tasche und in den Plänen bemerkbar. Fluggesellschaften…",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>In den Nachrichten geht es nicht um ferne Szenen. Wenn die Kerosinversorgung kompliziert wird, macht sich das schnell in der Tasche und in den Plänen bemerkbar. Fluggesellschaften arbeiten mit sehr geringen Margen und Treibstoff ist einer ihrer Hauptkosten. Wenn es zunimmt oder knapp wird, übertragen sie es direkt auf die Banknoten. Hier gibt es keine Magie.</p>
<p>Für diejenigen, die planen, diesen Sommer nach Spanien zu fliegen, lassen sich die Dinge in drei ganz konkrete Dinge umsetzen: höhere Preise, weniger verfügbare Flüge und ein höheres Risiko von Änderungen oder Stornierungen. Weniger Treibstoff bedeutet, profitable Routen zu priorisieren und die Frequenzen zu reduzieren. Und wenn das Angebot geringer ist und die Nachfrage gleich bleibt, ist das Ergebnis dasselbe wie immer: mehr für das Gleiche bezahlen ... oder für weniger.</p>
<p>Nicht um in Panik zu geraten, sondern um realistisch zu sein. Wenn Sie klare Termine haben, scheint es dieses Jahr nicht die beste Strategie zu sein, bis zur letzten Minute zu warten. Denn hier geht es nicht um einen bestimmten Vorrat, der entweicht, sondern um einen strukturellen Faktor: Fällt das Kerosin aus, stellt sich das gesamte Luftsystem um. Und normalerweise wird diese Anpassung letztendlich vom Passagier bezahlt.</p>
</div>`
    },

    "fb-daily-20260503-06-trabajar-a-tiempo-parcial-en-suiza-no-es-solo-cobrar-menos": {
        title: "Bei Teilzeitarbeit geht es in der Schweiz nicht nur darum, weniger zu verdienen",
        description: "Wenn jemand in der Schweiz einen 60- oder 80-Prozent-Vertrag aushandelt, denkt er meist an etwas ganz Einfaches: weniger Stunden, weniger Lohn, mehr Freizeit. Bis dahin ist alles richtig. Das Problem…",
        keywords: "Arbeit, Zeit, Teilzeit, Schweiz, allein, Gebühr, weniger, jemand",
        keywordsLocalized: true,
        category: "Arbeit und Löhne",
        hub: "trabajo",
        slug: "actualidad-2026-05-03-6-trabajar-a-tiempo-parcial-en-suiza-no-es-solo-cobrar-menos",
        readingTime: 2,
        dateUpdated: "3 Mai 2026",
        featuredImage: "media/guides/actualidad-tiempo-parcial-suiza-derechos.jpg",
        imageAlt: "Rechte von Teilzeitbeschäftigten in der Schweiz",
        summary: "Wenn jemand in der Schweiz einen 60- oder 80-Prozent-Vertrag aushandelt, denkt er meist an etwas ganz Einfaches: weniger Stunden, weniger Lohn, mehr Freizeit. Bis dahin alles...",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>Wenn jemand in der Schweiz einen 60- oder 80-Prozent-Vertrag aushandelt, denkt er meist an etwas ganz Einfaches: weniger Stunden, weniger Lohn, mehr Freizeit. Bis dahin ist alles richtig. Das Problem besteht darin, dass der Rentenprozentsatz nicht in der Lohn- und Gehaltsabrechnung verbleibt. Es hat direkte Auswirkungen auf drei wichtige Säulen des Schweizer Systems: Rente, Unfallversicherung und Arbeitslosigkeit. Und einige dieser Folgen werden später nicht behoben.</p>
<p>Die zweite Säule, die Pensionskasse nach dem Bundesberufsvorsorgegesetz (BVG), hat eine Mindestzugangsschwelle. Pflichtbeiträge leistet im Jahr 2026 nur noch, wer beim gleichen Arbeitgeber mehr als 22'680 Franken pro Jahr verdient. Was passiert in der Praxis? Dass ein Vertrag mit 30 % oder sogar 40 % in vielen Branchen zu kurz kommt. Ergebnis: Sie arbeiten, sammeln aber nichts in der zweiten Säule an. Langfristig führt das zu echten Rentenlücken. Und nein, es ist kein kleines Detail. Die Reform, die dies korrigieren sollte, wurde in der Volksabstimmung über die BVG-Reform im September 2024 abgelehnt, so dass das System gleich bleibt. Wie immer sind diejenigen am stärksten betroffen, die am meisten Teilzeit arbeiten, insbesondere Frauen.</p>
<p>Ein weiterer Punkt, den viele erst spät entdecken: die Unfallversicherung. Wenn Sie weniger als 8 Stunden pro Woche beim gleichen Arbeitgeber arbeiten, sind Sie laut Bundesunfallversicherungsgesetz (BVG) nicht gegen Nichtberufsunfälle versichert. Das heißt, man bricht sich am Wochenende beim Skifahren oder Radfahren das Bein und die Unfallversicherung greift nicht ein. Es kommt nur Ihre Krankenversicherung (KVG) mit Selbstbehalt und Zuzahlung in Betracht. Bei mehreren Minijobs ist Vorsicht geboten: Die Berechnung erfolgt nach Arbeitgeber, nicht nach Gesamtsumme. Hier ist mehr als eine Person überrascht.</p>
<p>Was die Arbeitslosigkeit betrifft, ist die Schweizer Logik recht einfach. Die Arbeitslosigkeit wird anhand Ihres Verdienstes berechnet. Wenn Sie zu 50 % arbeiten, leisten Sie Ihren Beitrag als solchen und werden auch als solcher bezahlt. Wenn Sie mehrere Jobs haben und einen verlieren, wird nur dieser Prozentsatz berücksichtigt. Es gibt keine magische oder globale Entschädigung. Das System ist konsistent, aber es ist nicht nachsichtig.</p>
<p>Nichts davon bedeutet, dass Teilzeitarbeit vermieden werden sollte. In vielen Fällen ist es eine gute Lösung. Aber es ist eine gute Idee, genau zu wissen, was es bedeutet, bevor Sie unterschreiben, und nicht erst, wenn es zu spät ist. Denn wie fast immer sind es in der Schweiz die technischen Details, die den Unterschied machen.</p>
</div>`
    },

    "fb-daily-20260504-01-a-raiz-de-una-publicacion-vuelvo-sobre-un-tema-incomodo-el-hidjab": {
        title: "Aufgrund eines Beitrags komme ich noch einmal auf ein heikles Thema zurück... Der Hidschab",
        description: "Ein Beitrag in unserer WhatsApp-Gruppe hat bei mir alle Alarmglocken läuten lassen. Es ist eine Sache, jemanden ehrlich darauf hinzuweisen, dass er in vielen…",
        keywords: "aufgrund, eines, beitrags, komme, noch, einmal, heikles, thema",
        keywordsLocalized: true,
        category: "Arbeit und Löhne",
        hub: "trabajo",
        slug: "actualidad-2026-05-04-1-a-raiz-de-una-publicacion-vuelvo-sobre-un-tema-incomodo-el-hidjab",
        readingTime: 1,
        dateUpdated: "4 Mai 2026",
        featuredImage: "media/guides/actualidad-hijab-suiza-debate.jpg",
        imageAlt: undefined,
        summary: "Ein Beitrag in unserer WhatsApp-Gruppe hat bei mir alle Alarmglocken läuten lassen. Es ist eine Sache, jemanden ehrlich darauf hinzuweisen, dass er…",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>Ein Beitrag in unserer WhatsApp-Gruppe hat bei mir alle Alarmglocken läuten lassen. Es ist eine Sache, jemanden ehrlich darauf hinzuweisen, dass er in vielen Branchen auf mehr Hindernisse stoßen kann. Das ist nun mal so. Das zu leugnen, hieße, ihm etwas vorzumachen. In der Schweiz gibt es deutlich mehr Arbeitgeber als in anderen europäischen Ländern, die sichtbaren religiösen Symbolen gegenüber zurückhaltender sind, vor allem in Positionen mit Kundenkontakt. Das ist nicht schön, aber es ist die Realität.</p>
<p>Es ist jedoch etwas ganz anderes, diese Realität als Freibrief zu nutzen, um jemanden zu demütigen, zu beleidigen oder auf ein Taschentuch zu reduzieren. Ein Mädchen fragt, ob sie Chancen auf einen Job hat, und anstatt über Lebenslauf, Sprachkenntnisse, Erfahrung, Arbeitserlaubnis oder Branchen zu sprechen, in denen sie passen könnte, ziehen manche den ganzen Katalog an Vorurteilen hervor. Was für eine Überraschung – und dann beschweren sie sich, dass „die Leute nicht arbeiten wollen“.</p>
<p>Man könnte sagen: „Das wird schwieriger für dich.“ Man könnte sagen: „In den Bereichen Reinigung, Produktion, Logistik oder in internen Positionen hast du vielleicht einige Möglichkeiten.“ Man könnte raten, den Lebenslauf anzupassen, die Französischkenntnisse zu verbessern und sich auf große Unternehmen oder vielfältigere Strukturen zu konzentrieren.</p>
<p>Was man sich sparen kann, sind rassistische Bemerkungen, Spott oder Integrationstipps wie aus einer Autobahnraststätte.</p>
<p>Helfen bedeutet auch, die Wahrheit zu sagen. Aber die Wahrheit muss nicht unbedingt mit Verachtung einhergehen.</p>
<p>www.espanolesensuiza.ch</p>
</div>`
    },

    "fb-daily-20260504-02-se-viniste-con-una-idea-y-la-realidad-te-puso-en-tu-sitio": {
        title: "Du hattest eine Idee … und die Realität hat dich auf den Boden der Tatsachen zurückgeholt.",
        description: "Also gut, ich stelle die Frage mal hier, weil sie jede Woche privat auftaucht: Welchem „Gerücht“ hast du geglaubt, bevor du in die Schweiz gekommen bist?",
        keywords: "hattest, idee, realität, dich, boden, tatsachen, zurückgeholt, also",
        keywordsLocalized: true,
        category: "",
        hub: "",
        slug: "actualidad-2026-05-04-2-se-viniste-con-una-idea-y-la-realidad-te-puso-en-tu-sitio",
        readingTime: 1,
        dateUpdated: "4 Mai 2026",
        featuredImage: "media/guides/actualidad-expectativas-realidad-suiza.jpg",
        imageAlt: undefined,
        summary: "Also gut, ich stelle die Frage mal hier, weil sie jede Woche privat auftaucht: Welchem „Gerücht“ hast du geglaubt, bevor du in die Schweiz gekommen…",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>Also gut, ich stelle die Frage mal hier, weil sie jede Woche privat auftaucht: Welchem „Gerücht“ hast du geglaubt, bevor du in die Schweiz gekommen bist?</p>
<p>Die Behauptung, dass „hier alles perfekt läuft“, die, dass „man mit jedem Job gut leben kann“, die, dass „man auf jeden Fall Geld spart“ … oder der Klassiker: „In zwei Monaten habe ich mich integriert.“</p>
<p>Spoiler: Nicht immer.</p>
<p>Ich glaube, es gibt einige Fantasien, die immer wieder auftauchen, und dann kommt die Realität: Versicherungen, Mieten, Sprache, Arbeitsmarkt … und schon ist der Traum ziemlich schnell ausgeträumt.</p>
<p>Erzähl mir deine Geschichte!! Sicherlich findet sich der eine oder andere in den Kommentaren wieder.</p>
<p>www.espanolesensuiza.ch</p>
</div>`
    },

    "fb-daily-20260504-03-temu-en-el-punto-de-mira-en-suiza-y-no-es-casualidad": {
        title: "Temu steht in der Schweiz im Rampenlicht … und das ist kein Zufall",
        description: "Temu sorgt seit Monaten in Europa für Furore – mit Preisen, die, um es ganz klar zu sagen, keiner normalen Marktlogik entsprechen. Und in der Schweiz sorgt das…",
        keywords: "temu, steht, schweiz, rampenlicht, kein, zufall, sorgt, seit",
        keywordsLocalized: true,
        category: "Warnungen und Ressourcen",
        hub: "recursos",
        slug: "actualidad-2026-05-04-3-temu-en-el-punto-de-mira-en-suiza-y-no-es-casualidad",
        readingTime: 1,
        dateUpdated: "4 Mai 2026",
        featuredImage: "media/guides/actualidad-temu-suiza-aduanas.jpg",
        imageAlt: undefined,
        summary: "Temu sorgt seit Monaten in Europa für Furore – mit Preisen, die, um es ganz klar zu sagen, keiner normalen Marktlogik entsprechen. Und in der Schweiz…",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>Temu sorgt seit Monaten in Europa für Furore – mit Preisen, die, um es ganz klar zu sagen, keiner normalen Marktlogik entsprechen. Und in der Schweiz sorgt das Unternehmen bereits auf politischer Ebene für Stirnrunzeln.</p>
<p>Das Schweizer Parlament hinterfragt derzeit einige ziemlich schwerwiegende Punkte. Zum einen geht es um Transparenz bei Preisen und Geschäftspraktiken: ständige Rabatte, künstlich erzeugte Dringlichkeit à la „nur noch 3 Stück verfügbar“ und eine sehr aggressive Strategie, um den Konsum anzukurbeln. Für jeden, der die App schon einmal geöffnet hat, ist das nichts Neues, aber wenn es um Verbraucherschutz geht, ist das mehr als nur ein Ärgernis.</p>
<p>Auf der anderen Seite gibt es das heiklere Thema: Produktionsbedingungen und unlauterer Wettbewerb. Schweizer und europäische Unternehmen kritisieren seit Langem, dass Plattformen wie Temu zu unhaltbaren Preisen verkaufen können, weil sie nach Standards arbeiten, die hier keine Zulassung finden würden. Einfach ausgedrückt: Wenn man nach anderen Regeln konkurriert, konkurriert man nicht, sondern dominiert den Markt.</p>
<p>Und dann ist da noch der Schweizer Klassiker: das Steuerwesen und die Kontrolle. Viele dieser Produkte werden in kleinen Paketen verschickt, oft unterhalb bestimmter Schwellenwerte, was die Zollkontrolle und die Erhebung der Mehrwertsteuer erschwert. Das Ergebnis: ein Wettbewerbsvorteil gegenüber lokalen Händlern, die sich buchstabengetreu an alle Vorschriften halten.</p>
<p>Das bedeutet nicht, dass Temu morgen verboten wird. Aber es bedeutet, dass wir uns in einer Phase befinden, in der die Schweiz das tut, was sie am besten kann: regulieren, wenn ihr etwas aus den Händen gleitet. Und normalerweise ist es so, dass das Schweizer Parlament sich erst dann eingehend mit einer Sache befasst, wenn das Thema bereits so groß geworden ist, dass man es nicht mehr ignorieren kann.</p>
<p>Die Moral von der Geschichte: Sehr niedrige Preise haben immer ihren Preis. Manchmal zahlt ihn nicht der Käufer … aber irgendjemand zahlt ihn.</p>
<p>www.espanolesensuiza.ch</p>
</div>`
    },

    "fb-daily-20260504-04-recolectar-fresas-en-ginebra-plan-bonito-hasta-que-ves-el-precio-por-kilo": {
        title: "Erdbeeren pflücken in Genf: eine schöne Idee … bis man den Preis pro Kilo sieht",
        description: "Jedes Frühjahr taucht dasselbe Video auf: grüne Felder, perfekte Erdbeeren, glückliche Kinder und das Gefühl, man befinde sich in einem Joghurt-Werbespot. Und…",
        keywords: "erdbeeren, pflücken, genf, schöne, idee, preis, kilo, sieht",
        keywordsLocalized: true,
        category: "Kultur und Veranstaltungen",
        hub: "cultura-eventos",
        slug: "actualidad-2026-05-04-4-recolectar-fresas-en-ginebra-plan-bonito-hasta-que-ves-el-precio-por-kilo",
        readingTime: 1,
        dateUpdated: "4 Mai 2026",
        featuredImage: "media/guides/actualidad-fresas-ginebra-recolecta.jpg",
        imageAlt: undefined,
        summary: "Jedes Frühjahr taucht dasselbe Video auf: grüne Felder, perfekte Erdbeeren, glückliche Kinder und das Gefühl, man befinde sich in einem…",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>Jedes Frühjahr taucht dasselbe Video auf: grüne Felder, perfekte Erdbeeren, glückliche Kinder und das Gefühl, man befinde sich in einem Joghurt-Werbespot. Und ja, es gibt das wirklich. Aber wie bei allem in der Schweiz sollte man sich erst einmal genau darüber informieren, wie das Ganze funktioniert, bevor man sich darauf einlässt.</p>
<p>Hier kann man keine „kostenlosen Erdbeeren vom Feld pflücken“. Es handelt sich um ein Selbstpflücksystem. Man geht hin, pflückt selbst … und bezahlt beim Verlassen des Geländes nach Gewicht. Der Reiz liegt in dem Erlebnis, nicht in der Ersparnis. Tatsächlich ist das Kilo oft genauso teuer oder sogar teurer als im Supermarkt, vor allem, wenn man sich beim Befüllen der Kisten mitreißen lässt.</p>
<p>In der Gegend um Genf gibt es mehrere bekannte Lokale: - La Fraisière in Meyrin - La Flânerie in Troinex</p>
<p>Es funktioniert ähnlich: Man geht hinein, nimmt sich einen Behälter, pflückt und geht zur Kasse. Ganz einfach. Aber achte auf die Öffnungszeiten und die Saison. Wenn es geregnet hat oder schon viele Tage seit der letzten Ernte vergangen sind, findest du vielleicht mehr Blätter als Erdbeeren.</p>
<p>Praktischer Tipp: Nimm Bargeld mit oder informiere dich über die Zahlungsmöglichkeiten, achte auf das Gewicht (die Kisten sind schnell voll) und geh nicht mit der Vorstellung hin, dass du Geld sparen willst. Du gehst hin, weil es dir gerade so passt, und weil frisch gepflückte Erdbeeren einfach anders schmecken.</p>
<p>Und noch eine typisch schweizerische Kleinigkeit: Halte dich an die Regeln. Keine Erdbeeren „zur Probe“ naschen, während du pflückst. Klar, das denken wir alle … aber auch das muss man hier bezahlen.</p>
<p>Ein empfehlenswertes Programm, vor allem mit Kindern oder für einen etwas anderen Vormittag. Aber es ist kein Schnäppchen, sondern ein Erlebnis. Und in der Schweiz muss man für Erlebnisse auch tief in die Tasche greifen.</p>
<p>www.espanolesensuiza.ch</p>
</div>`
    },

    "fb-daily-20260504-05-pasaporte-caducado-y-denegacion-de-embarque-cuando-la-aerolinea-se-equivoca": {
        title: "Abgelaufener Reisepass und Verweigerung des Boardings: Wenn die Fluggesellschaft einen Fehler macht",
        description: "Im Oktober 2018 verweigerte die Fluggesellschaft EasyJet in Lyon einer Minderjährigen das Einsteigen in einen Flug nach Neapel, da ihr Reisepass seit 18 Tagen…",
        keywords: "abgelaufener, reisepass, verweigerung, boardings, wenn, fluggesellschaft, einen, fehler",
        keywordsLocalized: true,
        category: "Formalitäten und Bewilligungen",
        hub: "tramites",
        slug: "actualidad-2026-05-04-5-pasaporte-caducado-y-denegacion-de-embarque-cuando-la-aerolinea-se-equivoca",
        readingTime: 1,
        dateUpdated: "4 Mai 2026",
        featuredImage: "media/guides/actualidad-pasaporte-caducado-embarque.jpg",
        imageAlt: undefined,
        summary: "Im Oktober 2018 verweigerte die Fluggesellschaft EasyJet in Lyon einer Minderjährigen das Einsteigen in einen Flug nach Neapel, da ihr Reisepass seit…",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>Im Oktober 2018 verweigerte die Fluggesellschaft EasyJet in Lyon einer Minderjährigen das Einsteigen in einen Flug nach Neapel, da ihr Reisepass seit 18 Tagen abgelaufen war. Die Familie reiste schließlich mit dem Bus weiter und verlor eine Übernachtung im Hotel. Jahre später gab das Gericht in Lyon ihnen Recht (Urteil vom 26. Februar 2026): 871,70 € für materiellen Schaden und 2'000 € für immateriellen Schaden. Übersetzt: Nicht alles, was am Check-in-Schalter gesagt wird, ist Gesetz.</p>
<p>Der rechtliche Schlüssel liegt nicht in den klassischen europäischen Rechtsvorschriften, sondern im Europäischen Abkommen über den Personenverkehr von 1957. Dieses Abkommen, das nach wie vor in Kraft ist, ermöglicht es spanischen Staatsbürgern, mit einem Reisepass, der seit weniger als fünf Jahren abgelaufen ist, in bestimmte Länder zu reisen, sofern es sich um Kurzaufenthalte (maximal 90 Tage) ohne berufliche Tätigkeit handelt. Die Cour de cassation selbst hatte dies bereits 2020 bestätigt, ebenfalls gegen EasyJet. Das ist nichts Neues, aber anscheinend hat sich das an einigen Schaltern noch nicht herumgesprochen.</p>
<p>Wo gilt das? In den Unterzeichnerstaaten des Abkommens: unter anderem in Deutschland, Frankreich, Italien, Spanien, Portugal, Belgien, den Niederlanden, Luxemburg, Österreich, Griechenland, Ungarn, Slowenien, Malta, Zypern, der Türkei und der Schweiz. Es handelt sich also nicht um eine Randerscheinung.</p>
<p>Wenn du Spanier bist und dir unter diesen Umständen das Einsteigen verweigert wird, gibt es eine Rechtsgrundlage für eine Beschwerde. Eine andere Sache ist es, was du um 6:00 Uhr morgens am Flugsteig mit jemandem diskutieren möchtest, der lediglich interne Vorschriften befolgt. Recht zu haben ist gut. Dass man dich ins Flugzeug lässt, ist besser.</p>
<p>www.espanolesensuiza.ch</p>
</div>`
    },

    "fb-daily-20260505-01-lo-que-nadie-te-explica-sobre-heredar-en-suiza": {
        title: "Was dir niemand über das Erben in der Schweiz erklärt",
        description: "In der Schweiz gibt es keine Bundeserbschaftssteuer. Es gibt lediglich kantonale Steuern, und jeder Kanton regelt dies auf eigene Weise. Das bedeutet: Wenn du…",
        keywords: "niemand, über, erben, schweiz, erklärt, gibt, keine, bundeserbschaftssteuer",
        keywordsLocalized: true,
        category: "Steuern",
        hub: "impuestos",
        slug: "actualidad-2026-05-05-1-lo-que-nadie-te-explica-sobre-heredar-en-suiza",
        readingTime: 1,
        dateUpdated: "5 Mai 2026",
        featuredImage: "media/guides/actualidad-herencia-suiza-impuesto.jpg",
        imageAlt: undefined,
        summary: "In der Schweiz gibt es keine Bundeserbschaftssteuer. Es gibt lediglich kantonale Steuern, und jeder Kanton regelt dies auf eigene Weise. Das…",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>In der Schweiz gibt es keine Bundeserbschaftssteuer. Es gibt lediglich kantonale Steuern, und jeder Kanton regelt dies auf eigene Weise. Das bedeutet: Wenn du das Glück hast, hier etwas zu erben, musst du als Erstes nicht den Steuersatz kennen, sondern wissen, in welchem Kanton der Verstorbene gelebt hat.</p>
<p>Die gute Nachricht für die meisten: Alle Kantone befreien direkte Nachkommen (Kinder, Enkelkinder) sowie Ehepartner oder eingetragene Partner von der Erbschaftssteuer. In der Praxis bedeutet dies: Wenn Sie ein Kind einer in der Schweiz verstorbenen Person sind, zahlen Sie in den meisten Fällen keine Schweizer Erbschaftssteuer. Die Kantone Schwyz und Obwalden gehen noch einen Schritt weiter: Dort gibt es überhaupt keine Erbschaftssteuer, unabhängig vom Verwandtschaftsgrad.</p>
<p>Die Situation sieht anders aus, wenn du ein Geschwisterteil, ein Neffe oder eine Nichte, ein nicht eingetragener Lebenspartner oder einfach nur ein Dritter bist. Die Sätze variieren stark je nach Kanton und Verwandtschaftsgrad und können in Extremfällen bis zu 50 % betragen. Die Unterschiede zwischen den Kantonen bei derselben Art von Erbschaft können enorm sein.</p>
<p>Ein Detail, das viele Menschen, die in der Schweiz leben, übersehen: Wenn Sie in Spanien Vermögen erben, unterliegt dies der spanischen Erbschaftssteuer, nicht der schweizerischen. Und wenn Sie zum Zeitpunkt des Erbfalls außerhalb Spaniens wohnten, haben Sie seit 2018 genau dieselben Rechte wie die Einwohner der Autonomen Gemeinschaft, in der sich das Vermögen befindet. Es lohnt sich, dies mit einem Steuerberater zu klären, da die Unterschiede zwischen den Autonomen Gemeinschaften sehr groß sein können.</p>
</div>`
    },

    "fb-daily-20260505-02-las-vacaciones-en-suiza-lo-que-dice-la-ley-y-lo-que-depende-de-tu-cont": {
        title: "Urlaub in der Schweiz: Was das Gesetz vorschreibt und was von deinem Arbeitsvertrag abhängt",
        description: "In Spanien sind es 22 Arbeitstage, was etwa 30 Kalendertagen entspricht. In der Schweiz beträgt das gesetzliche Minimum für erwachsene Arbeitnehmer gemäß…",
        keywords: "urlaub, schweiz, gesetz, vorschreibt, deinem, arbeitsvertrag, abhängt, spanien",
        keywordsLocalized: true,
        category: "Arbeit und Löhne",
        hub: "trabajo",
        slug: "actualidad-2026-05-05-2-las-vacaciones-en-suiza-lo-que-dice-la-ley-y-lo-que-depende-de-tu-cont",
        readingTime: 1,
        dateUpdated: "5 Mai 2026",
        featuredImage: "media/guides/actualidad-vacaciones-suiza-ley.jpg",
        imageAlt: undefined,
        summary: "In Spanien sind es 22 Arbeitstage, was etwa 30 Kalendertagen entspricht. In der Schweiz beträgt das gesetzliche Minimum für erwachsene Arbeitnehmer…",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>In Spanien sind es 22 Arbeitstage, was etwa 30 Kalendertagen entspricht. In der Schweiz beträgt das gesetzliche Minimum für erwachsene Arbeitnehmer gemäß Artikel 329a des Obligationenrechts (OR) 4 Wochen pro Jahr. Für Arbeitnehmer unter 20 Jahren beträgt das Minimum 5 Wochen. Es handelt sich dabei weder um Kalendertage noch um Arbeitstage, sondern um Arbeitswochen, was bei einer 5-Tage-Woche in der Praxis 20 Arbeitstage für einen Erwachsenen entspricht.</p>
<p>Das Gesetz sieht außerdem vor, dass mindestens zwei Wochen am Stück genommen werden müssen und dass der Urlaub so weit wie möglich im laufenden Jahr genommen werden soll. Der Arbeitgeber legt die Termine unter Berücksichtigung der Wünsche des Arbeitnehmers fest, hat jedoch das letzte Wort, wenn objektive organisatorische Gründe vorliegen. Was das Gesetz eindeutig verbietet, ist die Ersetzung von Urlaubstagen durch Geld während der Laufzeit des Arbeitsvertrags: Urlaub wird nicht ausgezahlt, sondern genommen.</p>
<p>Viele Tarifverträge und Einzelarbeitsverträge sehen mehr als diesen gesetzlichen Mindestanspruch vor. In mittleren und großen Unternehmen sind fünf Wochen üblich, wobei die Dauer auch von der Betriebszugehörigkeit abhängt. Es lohnt sich, den Arbeitsvertrag und, falls vorhanden, den Branchentarifvertrag zu prüfen, da die Unterschiede erheblich sein können.</p>
<p>Was viele nicht wissen: Wenn du im Urlaub krank wirst und dies durch ein ärztliches Attest nachweisen kannst, werden diese Krankheitstage nicht von deinem Urlaubskontingent abgezogen. Du hast Anspruch darauf, sie nachzuholen. Dies ist in der Schweiz geltende Rechtsprechung, doch in der Praxis ist es ratsam, den Arbeitgeber unverzüglich und mit den entsprechenden Unterlagen darüber zu informieren.</p>
</div>`
    },

    "fb-daily-20260505-03-el-secreto-bancario-suizo-y-espana-lo-que-hay-que-saber-antes-de-lleva": {
        title: "Das Schweizer Bankgeheimnis und Spanien: Was man wissen sollte, um keine böse Überraschung zu erleben",
        description: "Falls dir jemand gesagt hat, dass ein Konto bei einer Schweizer Bank bedeutet, dass das spanische Finanzamt nichts davon erfährt, ist diese Information schon…",
        keywords: "schweizer, bankgeheimnis, spanien, wissen, sollte, keine, böse, überraschung",
        keywordsLocalized: true,
        category: "Steuern",
        hub: "impuestos",
        slug: "actualidad-2026-05-05-3-el-secreto-bancario-suizo-y-espana-lo-que-hay-que-saber-antes-de-lleva",
        readingTime: 1,
        dateUpdated: "5 Mai 2026",
        featuredImage: "media/guides/actualidad-secreto-bancario-suiza-espana.jpg",
        imageAlt: undefined,
        summary: "Falls dir jemand gesagt hat, dass ein Konto bei einer Schweizer Bank bedeutet, dass das spanische Finanzamt nichts davon erfährt, ist diese…",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>Falls dir jemand gesagt hat, dass ein Konto bei einer Schweizer Bank bedeutet, dass das spanische Finanzamt nichts davon erfährt, ist diese Information schon seit mehreren Jahren überholt. Seit 2017 tauscht die Schweiz automatisch Steuerdaten mit allen EU-Mitgliedstaaten aus, darunter auch Spanien. Das Abkommen basiert auf der OECD-Richtlinie zum automatischen Informationsaustausch (AEOI/EAR) und ist uneingeschränkt verbindlich.</p>
<p>Was die Schweizer Banken jedes Jahr an die Eidgenössische Steuerverwaltung (ESTV) übermitteln, die diese Informationen wiederum an die spanische Steuerbehörde weiterleitet: vollständiger Name, Wohnsitz, Land des steuerlichen Wohnsitzes, Steueridentifikationsnummer, Kontostände und erzielte Erträge (Zinsen, Dividenden, Kapitalgewinne). Es bedarf keiner gerichtlichen Untersuchung oder vorherigen Anfrage. Der Austausch erfolgt automatisch, jährlich und systematisch. Im Jahr 2026 wurden bereits Daten von mehr als 3,4 Millionen Finanzkonten mit 101 Ländern ausgetauscht.</p>
<p>Im Jahr 2026 wird dies ausgeweitet. Die Schweiz ist dabei, Krypto-Vermögenswerte in diesen Austausch einzubeziehen, wobei die Umsetzung für 2027 vorgesehen ist. Die Liste der Länder umfasst alle EU-Mitgliedstaaten. Wenn Sie also Bitcoin oder ähnliche Vermögenswerte auf einer registrierten Schweizer Plattform halten, wird dieser Vermögenswert in Kürze ebenfalls ins Visier der Steuerbehörden geraten.</p>
<p>Die praktische Konsequenz ist klar: Wenn Sie in Spanien steuerlich ansässig sind, sind Sie verpflichtet, Ihre Konten und Vermögenswerte im Ausland mittels des Formulars 720 anzugeben, sobald diese bestimmte Schwellenwerte überschreiten. Ein nicht angegebenes Guthaben in der Schweiz ist nicht verborgen. Das Schweizer Bankgeheimnis gegenüber ausländischen Steuerbehörden existiert seit fast einem Jahrzehnt nicht mehr, und wer sich dieser Realität noch nicht angepasst hat, steht vor einem Problem, das sich mit der Zeit verschärfen kann.</p>
</div>`
    },

    "fb-daily-20260430-04-la-noche-de-los-museos-en-ginebra": {
        title: "Die Museumsnacht in Genf",
        description: "Am Samstag, dem 30. Mai 2026, ab 18 Uhr findet in Genf wieder die Museumsnacht statt. Und es handelt sich nicht um einen gewöhnlichen, gemächlichen Rundgang ……",
        keywords: "museumsnacht, genf, samstag, findet, wieder, statt, handelt, sich",
        keywordsLocalized: true,
        category: "Kultur und Veranstaltungen",
        hub: "cultura-eventos",
        slug: "actualidad-2026-04-30-4-la-noche-de-los-museos-en-ginebra",
        readingTime: 1,
        dateUpdated: "30 April 2026",
        featuredImage: "media/guides/actualidad-noche-museos-ginebra.jpg",
        imageAlt: undefined,
        summary: "Am Samstag, dem 30. Mai 2026, ab 18 Uhr findet in Genf wieder die Museumsnacht statt. Und es handelt sich nicht um einen gewöhnlichen, gemächlichen…",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>Am Samstag, dem 30. Mai 2026, ab 18 Uhr findet in Genf wieder die Museumsnacht statt. Und es handelt sich nicht um einen gewöhnlichen, gemächlichen Rundgang … sondern eher um einen Kulturmarathon.</p>
<p>150 Aktivitäten an 29 Veranstaltungsorten. Ausstellungen, Workshops, Escape Games, Performances, Schnitzeljagden … und vieles mehr.</p>
<p>In diesem Jahr kommen zwei neue Orte hinzu: die Bibliothèque Braille Romande et livre parlé und das Geneva Skateboard Museum. Zwei recht unterschiedliche Angebote, die jedoch... Mehr anzeigen</p>
<p>www.espanolesensuiza.ch</p>
</div>`
    },

    "fb-daily-20260430-05-los-trajes-tradicionales-suizos-ni-disfraz-ni-folklore-barato": {
        title: "Schweizer Trachten: weder Kostüm noch billige Folklore",
        description: "Ich habe einen Artikel von SwissCommunity über die Trachten in der Schweiz gelesen und finde, es ist sinnvoll, ein wenig Hintergrundwissen zu vermitteln, da…",
        keywords: "schweizer, trachten, weder, kostüm, noch, billige, folklore, habe",
        keywordsLocalized: true,
        category: "Kultur und Veranstaltungen",
        hub: "cultura-eventos",
        slug: "actualidad-2026-04-30-5-los-trajes-tradicionales-suizos-ni-disfraz-ni-folklore-barato",
        readingTime: 1,
        dateUpdated: "30 April 2026",
        featuredImage: "media/guides/actualidad-trajes-tradicionales-suizos.jpg",
        imageAlt: undefined,
        summary: "Ich habe einen Artikel von SwissCommunity über die Trachten in der Schweiz gelesen und finde, es ist sinnvoll, ein wenig Hintergrundwissen zu…",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>Ich habe einen Artikel von SwissCommunity über die Trachten in der Schweiz gelesen und finde, es ist sinnvoll, ein wenig Hintergrundwissen zu vermitteln, da man das Thema von außen betrachtet oft nur schwer versteht.</p>
<p>Erstens gibt es nicht „die eine Schweizer Tracht“. Es gibt Hunderte davon. Im wahrsten Sinne des Wortes. Jeder Kanton, jede Region, manchmal sogar jedes Dorf hat seine eigene. Und das ist kein Zufall: Die Farben, Stickereien oder Accessoires signalisierten ganz konkrete Dinge wie den Familienstand, die Religion oder sogar die soziale Stellung. Es ging also nicht um Ästhetik, sondern es war fast wie ein Ausweis in Textilform.</p>
<p>Zweiter wichtiger Punkt: Viele der Trachten, die wir heute auf Festen sehen, sind gar nicht so alt, wie es den Anschein hat. Viele wurden im 19. Jahrhundert neu interpretiert, als die Schweiz – wie andere europäische Länder auch – ihre nationale Identität aufbaute. Tradition ja, aber gut neu gestaltet und inszeniert.</p>
<p>Heutzutage gibt es diese Trachten zwar noch, aber nur in ganz bestimmten Zusammenhängen: bei lokalen Festen, in Kulturvereinen oder bei Umzügen. Niemand geht so zur Arbeit, falls das jemand gedacht haben sollte. Und noch eine interessante Anmerkung: Ein komplettes Trachtenoutfit kann mehrere tausend Franken kosten. Es handelt sich also nicht um ein Souvenir für 50 CHF.</p>
<p>Letztendlich passt das ziemlich gut zu dem, wie die Schweiz funktioniert: echte Vielfalt, stark dezentralisiert und ein Verhältnis zur Tradition, das nicht nostalgisch, sondern strukturiert und in vielen Fällen recht pragmatisch ist.</p>
<p>https://www.espanolesensuiza.ch/</p>
</div>`
    },

    "fb-daily-20260430-06-la-lex-koller-se-endurece-no-te-afecta-pero-si-te-va-a-tocar": {
        title: "Das Lex-Koller-Gesetz wird verschärft: Es betrifft dich zwar nicht direkt … aber es wird dich dennoch betreffen",
        description: "Der Bundesrat hat eine Vernehmlassung zur Reform der Lex Koller eröffnet, und die Richtung ist klar: mehr Einschränkungen für Bürger aus Ländern außerhalb der…",
        keywords: "koller, gesetz, wird, verschärft, betrifft, dich, zwar, nicht",
        keywordsLocalized: true,
        category: "Leben in der Schweiz",
        hub: "vivir-en-suiza",
        slug: "actualidad-2026-04-30-6-la-lex-koller-se-endurece-no-te-afecta-pero-si-te-va-a-tocar",
        readingTime: 1,
        dateUpdated: "30 April 2026",
        featuredImage: "media/guides/actualidad-lex-koller-endurece-suiza.jpg",
        imageAlt: undefined,
        summary: "Der Bundesrat hat eine Vernehmlassung zur Reform der Lex Koller eröffnet, und die Richtung ist klar: mehr Einschränkungen für Bürger aus Ländern…",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>Der Bundesrat hat eine Vernehmlassung zur Reform der Lex Koller eröffnet, und die Richtung ist klar: mehr Einschränkungen für Bürger aus Ländern außerhalb der EU und der EFTA. Es wird vorgeschlagen, dass sie selbst für den Kauf eines Hauptwohnsitzes eine individuelle Bewilligung beantragen müssen, und sollten sie die Schweiz verlassen, wären sie verpflichtet, die Immobilie innerhalb von zwei Jahren zu verkaufen. Zudem soll der Kauf von Immobilien zu rein investiven Zwecken zur Vermietung unterbunden werden.</p>
<p>... Mehr anzeigen</p>
<p>www.espanolesensuiza.ch</p>
</div>`
    },

    "fb-daily-20260506-01-lo-que-nadie-te-explica-del-contrato-a-llamada-en-suiza": {
        title: "Was dir niemand über den Vertrag mit Abrechnung nach Verbrauch in der Schweiz erklärt",
        description: "Die „Auf Abruf“-Beschäftigung (travail sur appel) ist eine der häufigsten Beschäftigungsformen im Gastgewerbe, im Handel, in der Reinigungsbranche und im…",
        keywords: "niemand, über, vertrag, abrechnung, nach, verbrauch, schweiz, erklärt",
        keywordsLocalized: true,
        category: "Arbeit und Löhne",
        hub: "trabajo",
        slug: "actualidad-2026-05-06-1-lo-que-nadie-te-explica-del-contrato-a-llamada-en-suiza",
        readingTime: 2,
        dateUpdated: "6 Mai 2026",
        featuredImage: "media/guides/actualidad-contrato-llamada-suiza.jpg",
        imageAlt: undefined,
        summary: "Die „Auf Abruf“-Beschäftigung (travail sur appel) ist eine der häufigsten Beschäftigungsformen im Gastgewerbe, im Handel, in der Reinigungsbranche…",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>Die „Auf Abruf“-Beschäftigung (travail sur appel) ist eine der häufigsten Beschäftigungsformen im Gastgewerbe, im Handel, in der Reinigungsbranche und im Pflegebereich. Der Arbeitgeber ruft dich an, wenn Arbeit da ist, und wenn nicht, dann eben nicht. Das klingt für das Unternehmen bequem. In der Schweiz liegt das unternehmerische Risiko jedoch nicht beim Arbeitnehmer, und das verändert die Situation erheblich.</p>
<p>Die Rechtsprechung des Bundesgerichts ist eindeutig: Ein Abrufvertrag ist keine Aneinanderreihung von punktuellen Miniverträgen. Es handelt sich um einen einzigen Vertrag auf unbestimmte Zeit. Das hat eine direkte Konsequenz: Wenn der Arbeitgeber beschliesst, auf dich zu verzichten, kann er nicht einfach aufhören, dich anzurufen. Es gelten die gesetzlichen Kündigungsfristen des Obligationenrechts (OR), und während dieser Zeit hast du Anspruch auf den Durchschnittslohn, berechnet auf der Grundlage der letzten 6 oder 12 Monate, unabhängig davon, ob du an diesen Tagen gearbeitet hast oder nicht.</p>
<p>Artikel 324 des Arbeitsgesetzbuchs (CO) bringt es auf den Punkt: Wenn der Arbeitgeber dir aus Gründen, die er zu vertreten hat – einschließlich eines Rückgangs der Geschäftstätigkeit oder einer Nebensaison – keine Arbeit anbieten kann, trägt er das wirtschaftliche Risiko. Du hast Anspruch auf deinen Lohn, auch wenn du keinen Fuß in die Arbeit gesetzt hast. Das Gleiche gilt für den Urlaub: Mindestens vier Wochen pro Jahr sind garantiert, davon mindestens zwei aufeinanderfolgende Wochen, unabhängig von den tatsächlich gearbeiteten Tagen. Der Krankengeldanspruch richtet sich nach der Vertragsdauer, nicht nach den geleisteten Arbeitsstunden.</p>
<p>In der Praxis liegt das Problem in schlecht formulierten Verträgen. Wenn im Vertrag keine garantierte Mindeststundenzahl festgelegt ist, kann der Arbeitgeber argumentieren, dass der historische Durchschnitt niedrig ist, und entsprechend berechnen, was er dir während der Kündigungsfrist schuldet. Tarifverträge können diese Bedingungen verbessern: Im Gastgewerbe legt der L-GAV strengere Schutzbestimmungen fest als das allgemeine Arbeitsgesetz. Bevor du einen Vertrag auf Abruf unterzeichnest, lohnt es sich zu prüfen, welcher Tarifvertrag in deiner Branche gilt und was er konkret zu dieser Vertragsart sagt.</p>
<p>- Schweizer Bundesrat, Bericht zur Regulierung der Arbeit auf Abruf: https://www.newsd.admin.ch/newsd/message/attachments/69051.pdf - Parlament.ch, Motion 19.3748 „Reglementer le travail sur appel“: https://www.parlament.ch/fr/ratsbetrieb/suche-curia-vista/geschaeft?AffairId=20193748 - espacedroit.ch, Arbeit auf Abruf in der Schweiz: https://www.espacedroit.ch/en/topics/travail-sur-appel-en/</p>
</div>`
    },

    "fb-daily-20260506-02-deshacerse-de-muebles-viejos-en-suiza-lo-que-funciona-lo-que-no-y-lo-q": {
        title: "Alte Möbel in der Schweiz entsorgen: Was funktioniert, was nicht und was es kosten kann",
        description: "In der Schweiz funktioniert die Entsorgung von alten Sofas, Matratzen oder Möbeln aus einem Umzug nicht so wie in anderen Ländern. Sie auf der Straße…",
        keywords: "alte, möbel, schweiz, entsorgen, funktioniert, nicht, kosten, kann",
        keywordsLocalized: true,
        category: "Leben in der Schweiz",
        hub: "vivir-en-suiza",
        slug: "actualidad-2026-05-06-2-deshacerse-de-muebles-viejos-en-suiza-lo-que-funciona-lo-que-no-y-lo-q",
        readingTime: 2,
        dateUpdated: "6 Mai 2026",
        featuredImage: "media/guides/actualidad-muebles-viejos-suiza-reciclaje.jpg",
        imageAlt: undefined,
        summary: "In der Schweiz funktioniert die Entsorgung von alten Sofas, Matratzen oder Möbeln aus einem Umzug nicht so wie in anderen Ländern. Sie auf der Straße…",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>In der Schweiz funktioniert die Entsorgung von alten Sofas, Matratzen oder Möbeln aus einem Umzug nicht so wie in anderen Ländern. Sie auf der Straße abzustellen, ist keine Option: Je nach Kanton und Gemeinde kann dies einen Verstoß darstellen, der mit einer Geldstrafe geahndet wird oder dazu führt, dass die Kosten für die Abholung dem Bewohner in Rechnung gestellt werden. Auch der Hauskehrichtcontainer ist für solche Gegenstände nicht vorgesehen. Die sogenannten «Sperrmüllgegenstände» oder «Encombrants» haben ihren eigenen Entsorgungsweg.</p>
<p>Die wichtigste Möglichkeit ist die „Dechettérie“ oder kantonale Recyclingstelle. Jeder Kanton verfügt über ein Netz von Zentren, in denen Möbel, Haushaltsgeräte, Holz und andere sperrige Gegenstände abgegeben werden können. Für Einwohner der Gemeinde oder des Kantons ist dies meist kostenlos, allerdings gelten bestimmte Bedingungen: Man muss während der Öffnungszeiten kommen, die Sortierung der Materialien beachten und in einigen Kantonen entsprechende Nachweise vorlegen. In Genf ist es seit dem 1. Februar 2026 Pflicht, einen ESREC-Pass (Genfer Einwohnerausweis) vorzuweisen, um Zugang zu den kantonalen Recyclingstellen zu erhalten. Ohne diese Karte wird kein Zutritt gewährt.</p>
<p>Viele Gemeinden organisieren zudem ein- oder zweimal im Jahr eine Sperrmüllabholung. Der Terminplan ist in der Regel auf der Website der Gemeinde zu finden. In manchen Fällen muss man einen Termin vereinbaren. Ausserhalb dieser Termine gibt es die Möglichkeit, Entrümpelungsdienste in Anspruch zu nehmen – private Unternehmen, die die Gegenstände gegen Bezahlung zu Hause abholen. Die kostenlose Alternative besteht darin, sie auf Wiederverwendungsplattformen wie Ricardo, Tutti oder in lokalen Facebook-Gruppen anzubieten: In der Schweiz ist die Kultur, Gegenständen ein zweites Leben zu schenken, bevor man sie wegwirft, ziemlich tief verwurzelt und funktioniert gut bei Möbeln in gutem Zustand.</p>
<p>Was Sie beachten sollten: Das unbefugte Abstellen von Sperrmüll auf der Straße, in den Gemeinschaftsbereichen des Gebäudes oder in Hausmüllcontainern kann zu einer Geldstrafe führen oder dazu, dass die Kosten für die Abholung direkt demjenigen in Rechnung gestellt werden, der den Müll dort abgestellt hat. Jede Gemeinde hat ihre eigenen Vorschriften, die selbst innerhalb desselben Kantons stark variieren können. Bevor Sie etwas unternehmen, sollten Sie am besten die Website der Gemeinde konsultieren oder den kommunalen Entsorgungsdienst anrufen.</p>
<p>- ge.ch, Sperrmüll und Altmetall in Genf: https://www.ge.ch/comment-trier-ses-dechets-geneve/encombrants-ferraille - geneve.ch, Sperrmüllabholung: https://www.geneve.ch/themes/environnement-urbain-espaces-verts/recyclage-dechets-menagers/objets-encombrants - recyclage-express.ch, Wertstoffhöfe in der Westschweiz: https://recyclage-express.ch/en/tout-savoir-sur-les-dechetteries-en-suisse-romande/</p>
</div>`
    },

    "fb-daily-20260506-03-la-iniciativa-del-14-de-junio-no-expulsa-a-nadie-con-permiso-pero-en-l": {
        title: "Die Initiative vom 14. Juni sieht keine Ausweisung von Personen mit Aufenthaltsgenehmigung vor. In den Gruppen wird jedoch das Gegenteil behauptet.",
        description: "Jedes Mal, wenn in der Schweiz eine Abstimmung zum Thema Einwanderung ansteht, füllen sich die spanischen Gruppen mit Beiträgen, in denen berechtigte Bedenken…",
        keywords: "initiative, juni, sieht, keine, ausweisung, personen, aufenthaltsgenehmigung, gruppen",
        keywordsLocalized: true,
        category: "Formalitäten und Bewilligungen",
        hub: "tramites",
        slug: "actualidad-2026-05-06-3-la-iniciativa-del-14-de-junio-no-expulsa-a-nadie-con-permiso-pero-en-l",
        readingTime: 2,
        dateUpdated: "6 Mai 2026",
        featuredImage: "media/guides/actualidad-iniciativa-junio-permiso-suiza.jpg",
        imageAlt: undefined,
        summary: "Jedes Mal, wenn in der Schweiz eine Abstimmung zum Thema Einwanderung ansteht, füllen sich die spanischen Gruppen mit Beiträgen, in denen berechtigte…",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>Jedes Mal, wenn in der Schweiz eine Abstimmung zum Thema Einwanderung ansteht, füllen sich die spanischen Gruppen mit Beiträgen, in denen berechtigte Bedenken mit Gerüchten vermischt werden, die keinerlei Grundlage in dem zur Abstimmung stehenden Text haben. Mit der Initiative „Pas de Suisse à 10 millions!“ am 14. Juni wiederholt sich dieser Kreislauf pünktlich. Es lohnt sich, zu lesen, was der Text sagt – und vor allem, was er nicht sagt.</p>
<p>Die SVP-Initiative sieht vor, dass die ständige Wohnbevölkerung der Schweiz bis 2050 die 10-Millionen-Marke nicht überschreiten darf. Sollte die Schwelle von 9,5 Millionen vor diesem Zeitpunkt erreicht werden, wären der Bundesrat und das Parlament verpflichtet, Massnahmen zu ergreifen. Die im Text ausdrücklich genannten Massnahmen betreffen vor allem den Asylbereich: Personen mit vorläufiger Aufnahme könnten keine Niederlassungsbewilligung mehr erhalten, und die Familienzusammenführung würde in bestimmten Fällen eingeschränkt. Der Text enthält zudem eine Klausel zur Einhaltung des zwingenden Völkerrechts.</p>
<p>Der Text enthält jedoch keine Klausel über den Entzug bereits erteilter Aufenthaltsbewilligungen oder die Zwangsrückführung von Personen mit einem stabilen Rechtsstatus. Inhaber von B-, C- und G-Bewilligungen sowie deren Familienangehörige mit abgeleiteten Bewilligungen sind nicht von dieser Initiative betroffen. Dies ohne Grundlage im Text zu behaupten, ist schlichtweg falsch. Der Bundesrat und das Parlament lehnen die Initiative ab, jedoch wegen ihrer möglichen Auswirkungen auf die bilateralen Abkommen mit der EU und auf die Schweizer Wirtschaft, nicht weil sie bereits ansässige Personen ausweisen würde.</p>
<p>Die Verwirrung hat ihre eigene Logik: Eine Initiative, die von der „Kontrolle der Einwohnerzahl“ spricht, löst verständlicherweise Ängste aus, insbesondere bei jenen, die bereits Erfahrungen mit der Verlängerung von Aufenthaltsgenehmigungen oder komplizierten Einwanderungsverfahren gemacht haben. Aber aus Angst zu stimmen oder zu kommentieren, anstatt sich auf den Text selbst zu stützen, hilft niemandem. Der vollständige Text ist auf admin.ch verfügbar. Das Lesen dauert zehn Minuten und ist mehr wert als hundert WhatsApp-Beiträge.</p>
<p>- admin.ch, offizielle Website der Initiative: https://www.admin.ch/fr/initiative-durabilite - ejpd.admin.ch, Stellungnahme des Bundesrats: https://www.ejpd.admin.ch/fr/le-conseil-federal-rejette-linitiative-populaire-pas-de-suisse-a-10-millions- - rts.ch, Textanalyse: https://www.rts.ch/info/suisse/2026/article/plafonner-la-population-suisse-a-10-millions-un-objectif-simple-une-mise-en-uvre-complexe-29182359.html - bk.admin.ch, offizielles Datenblatt zur Initiative: https://www.bk.admin.ch/ch/f/pore/vi/vis555t.html</p>
</div>`
    },

    "fb-daily-20260507-01-el-13-salario-en-suiza-no-es-automatico-ni-esta-garantizado-por-ley": {
        title: "Das 13. Monatsgehalt ist in der Schweiz weder automatisch fällig noch gesetzlich garantiert",
        description: "Wenn man in die Schweiz kommt und vom „treizième salaire“ hört, nehmen viele das als selbstverständlich hin, als wäre es ein fester Bestandteil des Systems,…",
        keywords: "monatsgehalt, schweiz, weder, automatisch, fällig, noch, gesetzlich, garantiert",
        keywordsLocalized: true,
        category: "Arbeit und Löhne",
        hub: "trabajo",
        slug: "actualidad-2026-05-07-1-el-13-salario-en-suiza-no-es-automatico-ni-esta-garantizado-por-ley",
        readingTime: 1,
        dateUpdated: "7 Mai 2026",
        featuredImage: "media/guides/actualidad-13-salario-suiza.jpg",
        imageAlt: undefined,
        summary: "Wenn man in die Schweiz kommt und vom „treizième salaire“ hört, nehmen viele das als selbstverständlich hin, als wäre es ein fester Bestandteil des…",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>Wenn man in die Schweiz kommt und vom „treizième salaire“ hört, nehmen viele das als selbstverständlich hin, als wäre es ein fester Bestandteil des Systems, genau wie die AHV oder die Krankenversicherung. Das ist es aber nicht. Das 13. Monatsgehalt für Arbeitnehmer ist nicht im Obligationenrecht vorgeschrieben. Wenn es nicht in deinem Einzelvertrag oder im Gesamtarbeitsvertrag (GAV) für deine Branche vorgesehen ist, hast du keinen Anspruch darauf. Punkt.</p>
<p>In der Praxis betrifft dies jedoch eine große Mehrheit. Schätzungen zufolge erhalten rund 70 % der Beschäftigten in der Schweiz diese Zulage, da viele Branchen sie in ihre Gesamtarbeitsverträge aufgenommen haben: Baugewerbe, Gastgewerbe, Einzelhandel, Bankwesen ... Wenn du in einer dieser Branchen arbeitest, erhältst du sie wahrscheinlich bereits, ohne sie beantragt zu haben. Andernfalls solltest du den Vertrag vor der Unterzeichnung sorgfältig durchlesen.</p>
<p>Wenn es diese Zahlung gibt, ist die Berechnung einfach: Sie entspricht einem Zwölftel des Bruttojahresgehalts, also einem vollen Monatsgehalt zusätzlich. In der Regel wird sie im Dezember ausgezahlt, obwohl manche Verträge die Zahlung auf Juni und Dezember aufteilen. Und wenn du deinen Arbeitsplatz zur Jahresmitte verlässt, hast du Anspruch auf einen anteiligen Betrag, unabhängig vom Grund für dein Ausscheiden. Allerdings musst du diesen beantragen: Er wird nicht immer automatisch berechnet. Was die Besteuerung angeht, gibt es keinen Vorteil: Es wird genau wie jeder andere Monatslohn versteuert, mit allen üblichen Abzügen.</p>
<p>Man sollte hier nicht verwechseln: Dieses 13. Monatsgehalt für Arbeitnehmer hat nichts mit der im Referendum beschlossenen 13. AHV-Rente für Rentner zu tun, die 2025 in Kraft getreten ist. Es handelt sich um zwei verschiedene Dinge, auch wenn die Zahl dieselbe ist.</p>
</div>`
    },

    "fb-daily-20260507-02-la-buanderia-del-edificio-en-suiza-el-cuarto-que-nadie-te-explica-cuan": {
        title: "Der Waschraum im Schweizer Wohnhaus: Der Raum, den dir niemand erklärt, wenn du den Mietvertrag unterschreibst",
        description: "Einer der ersten Kulturschocks beim Mieten einer Wohnung in der Schweiz ist weder der Preis noch die Kaution noch das dreiseitige Inventar. Es ist die…",
        keywords: "waschraum, schweizer, wohnhaus, raum, niemand, erklärt, wenn, mietvertrag",
        keywordsLocalized: true,
        category: "Wohnen",
        hub: "vivienda",
        slug: "actualidad-2026-05-07-2-la-buanderia-del-edificio-en-suiza-el-cuarto-que-nadie-te-explica-cuan",
        readingTime: 2,
        dateUpdated: "7 Mai 2026",
        featuredImage: "media/guides/actualidad-lavanderia-edificio-suiza.jpg",
        imageAlt: undefined,
        summary: "Einer der ersten Kulturschocks beim Mieten einer Wohnung in der Schweiz ist weder der Preis noch die Kaution noch das dreiseitige Inventar. Es ist…",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>Einer der ersten Kulturschocks beim Mieten einer Wohnung in der Schweiz ist weder der Preis noch die Kaution noch das dreiseitige Inventar. Es ist die Erkenntnis, dass man keine Waschmaschine hat. Oder besser gesagt: dass die Waschmaschine im Keller steht und man sie dienstags von acht bis zwölf Uhr benutzen darf. Nicht mittwochs. Nicht sonntags. Dienstags, von acht bis zwölf Uhr. Und wenn man seinen Termin verpasst hat, muss man bis zur nächsten Woche warten.</p>
<p>Die Gemeinschaftswaschküche ist in Schweizer Wohnanlagen eine feste Einrichtung. In etwa der Hälfte der Fälle sind die Nutzungszeiten fest zugewiesen: Jeder Mieter hat einen festen Zeitblock, in der Regel einen halben Tag pro Woche. In der anderen Hälfte ist der Zugang innerhalb der zulässigen Zeiten frei, die in den meisten Gebäuden von Montag bis Samstag zwischen 7 und 21 Uhr liegen. An Feiertagen ist die Nutzung verboten, sofern die Hausordnung nichts anderes vorsieht. In jedem Fall besteht ein Mindestanspruch darauf, mindestens einmal alle zwei Wochen waschen zu können.</p>
<p>Was die Kosten angeht, so zahlen 80 % der Mieter direkt pro Waschgang, wobei der Durchschnittspreis bei 1,55 Franken pro Waschgang bei 40 Grad liegt. Es gibt Gebäude mit Münzsystemen, andere mit Kartensystemen, wieder andere, bei denen die Kosten am Jahresende in den Nebenkosten abgerechnet werden. Was der Vermieter jedoch nicht tun darf, ist, zweimal für dasselbe zu berechnen: Entweder rechnet er es über die Nebenkosten ab oder er berechnet es über den Zähler, aber nicht beides.</p>
<p>In Spanien ist dieses Wohnmodell so gut wie unbekannt. Die Waschmaschine ist Teil der Wohnung, der Einrichtung, des Alltags. Hier nicht: Der Waschraum ist ein Gemeinschaftsraum mit eigenen Regeln, und diese werden vom Eigentümer oder Verwalter festgelegt, nicht vom Gesetz. Bei zeitlichen Konflikten mit einem Nachbarn ist diese Hausordnung der Ausgangspunkt, nicht das Bürgerliche Gesetzbuch. Es lohnt sich, sie zu lesen, bevor jemand eine Notiz in deinem Briefkasten hinterlässt.</p>
</div>`
    },

    "fb-daily-20260507-03-vivir-juntos-sin-casarse-en-suiza-lo-que-perderias-si-tu-pareja-muere": {
        title: "In der Schweiz unverheiratet zusammenleben: Was dir entgehen würde, wenn dein Partner morgen verstirbt",
        description: "In der Schweiz gibt es weder den PACS noch eine vergleichbare Rechtsform, die Lebensgemeinschaften in gleicher Weise schützt wie die Ehe. Das „Konkubinat“, wie…",
        keywords: "schweiz, unverheiratet, zusammenleben, entgehen, würde, wenn, dein, partner",
        keywordsLocalized: true,
        category: "Leben in der Schweiz",
        hub: "vivir-en-suiza",
        slug: "actualidad-2026-05-07-3-vivir-juntos-sin-casarse-en-suiza-lo-que-perderias-si-tu-pareja-muere",
        readingTime: 2,
        dateUpdated: "7 Mai 2026",
        featuredImage: "media/guides/actualidad-pareja-hecho-suiza-derechos.jpg",
        imageAlt: undefined,
        summary: "In der Schweiz gibt es weder den PACS noch eine vergleichbare Rechtsform, die Lebensgemeinschaften in gleicher Weise schützt wie die Ehe. Das…",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>In der Schweiz gibt es weder den PACS noch eine vergleichbare Rechtsform, die Lebensgemeinschaften in gleicher Weise schützt wie die Ehe. Das „Konkubinat“, wie diese Situation gesetzlich bezeichnet wird, begründet keine automatischen Rechte zwischen den beiden Partnern. Weder Erbansprüche noch Witwenrente noch ein bevorzugter Anspruch auf das Vermögen des anderen. Stirbt dein Partner, ohne ein Testament hinterlassen zu haben, bist du rechtlich gesehen ein Fremder in Bezug auf die Erbschaft.</p>
<p>Was die AHV betrifft, so profitieren nichtehelich zusammenlebende Paare paradoxerweise, solange beide Partner am Leben sind. Jeder hat Anspruch auf 100 % seiner individuellen Rente, wobei der Höchstbetrag im Jahr 2026 bei 2520 Franken pro Person und Monat liegt. Bei Ehepaaren gilt eine Obergrenze für die gemeinsame Rente von 150 % der maximalen Einzelrente, was in der Praxis den Gesamtbetrag begrenzt, den das Paar gemeinsam erhält. Die Differenz kann mehrere hundert Franken pro Monat betragen. Doch wenn einer der beiden stirbt, beginnt das Problem: Der Hinterbliebene eines unverheirateten Paares hat keinen Anspruch auf die Witwen- oder Witwerrente der ersten Säule. Der Verstorbene nimmt seine Rente mit ins Grab.</p>
<p>Auch bei der zweiten und dritten Säule sieht die Situation unterschiedlich aus. In der Säule 3a können Lebenspartner den anderen als Begünstigten benennen, doch muss der Fonds oder der Versicherer zuvor schriftlich darüber informiert werden. Im BVG (zweite Säule) besteht zwar ein Anspruch auf Hinterbliebenenrente, jedoch unter strengeren Bedingungen als bei Verheirateten. Was die Wohnung betrifft: Liegt der Mietvertrag nur auf den Namen eines der beiden Partner, hat der andere keinen gesetzlichen Anspruch darauf, dort zu bleiben, wenn der Vertragsinhaber stirbt.</p>
<p>Die Lösung besteht nicht unbedingt darin, zu heiraten, sondern darin, die Situation rechtlich zu regeln: ein Testament, ein Lebenspartnerschaftsvertrag, eine Lebensversicherung, ausdrückliche Begünstigtenbestimmungen in den Pensionskassen. Viele Paare in der Schweiz leben seit Jahren zusammen, ohne irgendetwas davon zu tun, in der Überzeugung, dass die Zeit Rechte begründet. In diesem Land werden Rechte jedoch durch schriftliche Urkunden begründet.</p>
</div>`
    },

    "fb-daily-20260508-01-divorciarse-en-suiza-con-permiso-de-residencia-lo-que-depende-de-como": {
        title: "Scheidung in der Schweiz mit Aufenthaltsbewilligung: Was davon abhängt, wie du diese Bewilligung erhalten hast",
        description: "Eine Trennung oder Scheidung wirft viele Fragen bezüglich der Aufenthaltsgenehmigung auf, und die Antworten in den Gruppen sind nicht immer hilfreich. Zunächst…",
        keywords: "scheidung, schweiz, aufenthaltsbewilligung, davon, abhängt, diese, bewilligung, erhalten",
        keywordsLocalized: true,
        category: "Formalitäten und Bewilligungen",
        hub: "tramites",
        slug: "actualidad-2026-05-08-1-divorciarse-en-suiza-con-permiso-de-residencia-lo-que-depende-de-como",
        readingTime: 1,
        dateUpdated: "8 Mai 2026",
        featuredImage: "media/guides/actualidad-divorcio-suiza-permiso.jpg",
        imageAlt: undefined,
        summary: "Eine Trennung oder Scheidung wirft viele Fragen bezüglich der Aufenthaltsgenehmigung auf, und die Antworten in den Gruppen sind nicht immer…",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>Eine Trennung oder Scheidung wirft viele Fragen bezüglich der Aufenthaltsgenehmigung auf, und die Antworten in den Gruppen sind nicht immer hilfreich. Zunächst muss man verstehen, dass die Antwort ganz davon abhängt, wie man diese Genehmigung erhalten hat: ob man sie durch Arbeit erworben hat oder dank des Partners.</p>
<p>Wenn du EU-Bürger bist und deine Aufenthaltsbewilligung B aufgrund deiner Arbeit in der Schweiz besitzt, ändert sich durch die Scheidung zunächst nichts. Dein Aufenthaltsrecht ist an deine berufliche Tätigkeit gebunden, nicht an deinen Familienstand. Das Abkommen über die Personenfreizügigkeit (FZA) gewährt dir dieses Recht unabhängig davon. Eine Scheidung hat keine Auswirkungen auf deine Aufenthaltsbewilligung, solange du weiterhin arbeitest.</p>
<p>Anders verhält es sich, wenn du als „Familienangehöriger“ eines europäischen Arbeitnehmers in die Schweiz gekommen bist, ohne selbst einer Erwerbstätigkeit nachzugehen. In diesem Fall kann eine Scheidung die Verlängerung deiner Aufenthaltsbewilligung erschweren. Die Bewilligung wird dir nicht automatisch entzogen, aber der Kanton prüft, ob du über eigene Existenzmittel verfügst oder einer beruflichen Tätigkeit nachgehst. Das Gleiche gilt für Nicht-EU-Bürger: Artikel 50 des Ausländergesetzes (LEI) legt fest, dass das Recht auf Familienzusammenführung bestehen bleibt, wenn die eheliche Gemeinschaft mindestens drei Jahre gedauert hat und eine ausreichende Integration vorliegt oder wenn schwerwiegende persönliche Gründe vorliegen, die dies rechtfertigen.</p>
<p>In der Praxis ist jemand, der mit einem eigenen Arbeitsvertrag in die Schweiz gekommen ist und seit Jahren Sozialversicherungsbeiträge zahlt, in einer soliden Position. Wer ohne eigene Erwerbstätigkeit als Ehepartner eingereist ist, sollte sich vor der endgültigen Scheidung an die kantonale Migrationsbehörde wenden – nicht erst danach.</p>
</div>`
    },

    "fb-daily-20260508-02-la-caucion-del-alquiler-en-suiza-tres-meses-bloqueados-que-son-tuyos-p": {
        title: "Die Mietkaution in der Schweiz: Drei Monate, die dir gehören, über die du aber keine Kontrolle hast",
        description: "Wenn du in der Schweiz einen Mietvertrag unterzeichnest, verlangt der Vermieter in der Regel eine Kaution in Höhe von zwei oder drei Monatsmieten. Drei…",
        keywords: "mietkaution, schweiz, drei, monate, gehören, über, aber, keine",
        keywordsLocalized: true,
        category: "Wohnen",
        hub: "vivienda",
        slug: "actualidad-2026-05-08-2-la-caucion-del-alquiler-en-suiza-tres-meses-bloqueados-que-son-tuyos-p",
        readingTime: 1,
        dateUpdated: "8 Mai 2026",
        featuredImage: "media/guides/actualidad-caucion-alquiler-suiza.jpg",
        imageAlt: undefined,
        summary: "Wenn du in der Schweiz einen Mietvertrag unterzeichnest, verlangt der Vermieter in der Regel eine Kaution in Höhe von zwei oder drei Monatsmieten.…",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>Wenn du in der Schweiz einen Mietvertrag unterzeichnest, verlangt der Vermieter in der Regel eine Kaution in Höhe von zwei oder drei Monatsmieten. Drei Monatsmieten sind das gesetzlich zulässige Maximum gemäß dem Obligationenrecht, und es darf nicht mehr verlangt werden, auch wenn dies im Vertrag steht. Sollte dies der Fall sein, ist diese Klausel nichtig.</p>
<p>Was viele Menschen nicht wissen, ist, dass dieses Geld nicht auf ein Konto des Vermieters überwiesen werden darf: Es muss auf ein gesperrtes Bankkonto auf deinen Namen eingezahlt werden. Das Konto läuft auf deinen Namen, und die daran anfallenden Zinsen gehören dir. Der Vermieter kann dieses Geld nur einfordern, wenn am Ende des Mietvertrags eine nachgewiesene Schuld vorliegt, sei es in Form von ausstehenden Mieten oder Schäden, die über den normalen Verschleiß hinausgehen.</p>
<p>Wenn du die Schlüssel zurückgibst, beträgt die angemessene Frist für die Freigabe der Kaution ein bis drei Monate, sobald bestätigt ist, dass keine offenen Forderungen bestehen. Wenn der Vermieter nicht handelt, kannst du dich mit Zustimmung des Eigentümers direkt an die Bank wenden oder, falls du diese nicht erhältst, bei der zuständigen Behörde (in den meisten Kantonen beim Friedensrichter oder bei der Schlichtungskommission) die Freigabe beantragen. Für diesen ersten Schritt benötigst du keinen Anwalt.</p>
<p>Eine Alternative, die immer mehr Menschen nutzen, besteht darin, kein Bargeld zu hinterlegen, sondern eine Solidarbürgschaft über spezialisierte Unternehmen wie SwissCaution oder Firstcaution abzuschließen. Du zahlst eine Jahresprämie in Höhe von knapp 5 % der Bürgschaftssumme, und deine liquiden Mittel bleiben dir erhalten. In den meisten Kantonen akzeptieren Vermieter diese Lösungen, sind jedoch gesetzlich nicht dazu verpflichtet.</p>
</div>`
    },

    "fb-daily-20260508-03-la-pension-de-espana-y-la-de-suiza-no-se-suman-como-crees": {
        title: "Die Rente in Spanien und die in der Schweiz addieren sich nicht so, wie du denkst",
        description: "Es gibt eine weit verbreitete Vorstellung: Wenn man in der Schweiz arbeitet, nachdem man in Spanien Beiträge gezahlt hat, erhält man eine Rente aus beiden…",
        keywords: "rente, spanien, schweiz, addieren, sich, nicht, denkst, gibt",
        keywordsLocalized: true,
        category: "Arbeit und Löhne",
        hub: "trabajo",
        slug: "actualidad-2026-05-08-3-la-pension-de-espana-y-la-de-suiza-no-se-suman-como-crees",
        readingTime: 1,
        dateUpdated: "8 Mai 2026",
        featuredImage: "media/guides/actualidad-pension-espana-suiza.jpg",
        imageAlt: undefined,
        summary: "Es gibt eine weit verbreitete Vorstellung: Wenn man in der Schweiz arbeitet, nachdem man in Spanien Beiträge gezahlt hat, erhält man eine Rente aus…",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>Es gibt eine weit verbreitete Vorstellung: Wenn man in der Schweiz arbeitet, nachdem man in Spanien Beiträge gezahlt hat, erhält man eine Rente aus beiden Ländern, und das ist fast so, als würde man doppelt verdienen. Die Realität sieht jedoch deutlich nüchterner aus, und wenn man das rechtzeitig versteht, ändert sich die Art der Planung, die man vornehmen sollte.</p>
<p>Das bilaterale Sozialversicherungsabkommen zwischen der Schweiz und Spanien, ergänzt durch die in der Schweiz geltenden europäischen Koordinierungsvereinbarungen, sieht Folgendes vor: Jedes Land zahlt seinen Anteil und nur seinen Anteil. Wenn du 8 Jahre in Spanien und 22 Jahre in der Schweiz Beiträge gezahlt hast, berechnet Spanien eine Rente, als hättest du 30 Jahre Beiträge gezahlt, zahlt dir aber nur 8/30 dieses Betrags aus. Die Schweiz verfährt mit deinen 22 Jahren genauso. Das ist keine Doppelzahlung: Es ist proportional, und die Summe kann geringer ausfallen, als du erhalten würdest, wenn alle deine Beitragsjahre in einem einzigen System angesammelt wären.</p>
<p>Wo das Abkommen tatsächlich hilft, ist bei der Anspruchsberechtigung. Wenn du in Spanien 15 Beitragsjahre benötigst, um Anspruch auf eine beitragsabhängige Rente zu haben, und du nur 5 vorweisen kannst, kann Spanien deine Schweizer Beitragsjahre hinzurechnen, um zu prüfen, ob du diese Mindestgrenze erreichst. Sobald die Anspruchsberechtigung bestätigt ist, erfolgt die Berechnung ausschließlich auf der Grundlage der tatsächlichen Beitragsjahre in Spanien.</p>
<p>Das hat praktische Auswirkungen: Wenn du vorhast, kurz vor dem Ruhestand nach Spanien zurückzukehren, fließen die letzten in Spanien geleisteten Beitragsjahre in die spanische Bemessungsgrundlage ein, was deine spanische Rente erheblich verbessern kann. Bis zum letzten Tag vor dem Ruhestand in der Schweiz zu arbeiten, ist nicht immer die rentabelste Strategie, wenn du danach in Spanien leben möchtest. Es lohnt sich, die Zahlen im Voraus durchzurechnen – und nicht erst dann, wenn es keinen Spielraum mehr gibt, etwas zu ändern.</p>
</div>`
    },

    "fb-daily-20260509-01-die-avs-nummer-die-kennung-die-alles-uber-dich-weiss-ohne-deine-daten": {
        title: "Die AVS-Nummer: Die Kennung, die alles über dich weiß, ohne deine Daten zu enthalten",
        featuredImage: "media/guides/actualidad-el-numero-avs-el-identificador-que-lo-sabe-to.jpg",
        imageAlt: "Die AVS-Nummer: Die Kennung, die alles über dich weiß, ohne deine Daten zu enthalten",
        description: "Die AVS-Nummer besteht aus den 13 Ziffern, die auf deiner Krankenversicherungskarte stehen und bei denen viele erst nach Wochen verstehen, wozu sie genau…",
        keywords: "nummer, kennung, alles, über, dich, ohne, deine, daten",
        keywordsLocalized: true,
        category: "Formalitäten und Bewilligungen",
        hub: "tramites",
        slug: "actualidad-2026-05-09-1-die-avs-nummer-die-kennung-die-alles-uber-dich-weiss-ohne-deine-daten",
        readingTime: 1,
        dateUpdated: "9 Mai 2026",
        summary: "Die AVS-Nummer besteht aus den 13 Ziffern, die auf deiner Krankenversicherungskarte stehen und bei denen viele erst nach Wochen verstehen, wozu sie…",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>Die AVS-Nummer besteht aus den 13 Ziffern, die auf deiner Krankenversicherungskarte stehen und bei denen viele erst nach Wochen verstehen, wozu sie genau dienen. Wenn du bereits in der Schweiz lebst oder arbeitest, hast du bereits eine solche Nummer, auch wenn du sie nicht beantragt hast.</p>
<p>Sie heißt NAVS13 und ersetzt seit 2008 die alte 11-stellige Nummer. Im Gegensatz zur NIE oder der spanischen Sozialversicherungsnummer enthält sie keine personenbezogenen Daten: Sie ist vollkommen anonym. Wenn du heiratest, dich scheiden lässt oder deinen Namen änderst, bleibt die Nummer unverändert. Sie ist dauerhaft und begleitet dich dein ganzes Leben lang.</p>
<p>Wenn du mit einer Aufenthaltsbewilligung eingereist bist oder eine Arbeit in der Schweiz aufgenommen hast, wurde dir diese Nummer automatisch zugewiesen. Die Zentrale Ausgleichsstelle in Bern generiert sie und teilt sie deinem Arbeitgeber, der Ausgleichskasse und der Versicherung mit. Du musst sie nicht beantragen und auch kein Formular ausfüllen. Die meisten sehen sie zum ersten Mal auf ihrer Krankenversicherungskarte im Feld „Versicherungsnummer“.</p>
<p>Wozu dient sie eigentlich: Berechnung der AHV- und IV-Beiträge, Familienzulagen, Steuererklärung, Registrierung in der zweiten Säule, Abwicklung von Angelegenheiten beim RAV bei Arbeitslosigkeit, Formalitäten bei den kantonalen Ausgleichskassen. Sie erscheint aber auch in kantonalen Verwaltungsregistern, im Betreibungsregister und auf amtlichen Bescheinigungen. In der Schweiz entspricht sie funktional der Kombination aus Personalausweis und Sozialversicherungsnummer, zusammengefasst in einer einzigen, unveränderlichen Nummer.</p>
<p>Wenn du nicht weißt, bei welcher Krankenkasse du versichert bist, schau auf deine Krankenversicherungskarte. Falls du noch keine hast, wende dich an die kantonale Ausgleichskasse oder frag deinen Arbeitgeber. Es gibt keinen speziellen Schalter und keine vorherige Formalität: Die Versicherung besteht einfach schon seit deiner Ankunft.</p>
</div>`
    },

    "fb-daily-20260509-02-ein-gebrauchtwagen-in-der-schweiz-kaufen-das-kennzeichen-gehort-dem-be": {
        title: "Ein Gebrauchtwagen in der Schweiz kaufen: Das Kennzeichen gehört dem Besitzer, nicht dem Auto",
        featuredImage: "media/guides/actualidad-comprar-un-coche-de-segunda-mano-en-suiza-la.jpg",
        imageAlt: "Ein Gebrauchtwagen in der Schweiz kaufen: Das Kennzeichen gehört dem Besitzer, nicht dem Auto",
        description: "Wenn man in der Schweiz einen Gebrauchtwagen kauft, nimmt der Verkäufer die Nummernschilder mit. Im wahrsten Sinne des Wortes. Hier gehören die Nummernschilder…",
        keywords: "gebrauchtwagen, schweiz, kaufen, kennzeichen, gehört, besitzer, nicht, auto",
        keywordsLocalized: true,
        category: "Formalitäten und Bewilligungen",
        hub: "tramites",
        slug: "actualidad-2026-05-09-2-ein-gebrauchtwagen-in-der-schweiz-kaufen-das-kennzeichen-gehort-dem-be",
        readingTime: 1,
        dateUpdated: "9 Mai 2026",
        summary: "Wenn man in der Schweiz einen Gebrauchtwagen kauft, nimmt der Verkäufer die Nummernschilder mit. Im wahrsten Sinne des Wortes. Hier gehören die…",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>Wenn man in der Schweiz einen Gebrauchtwagen kauft, nimmt der Verkäufer die Nummernschilder mit. Im wahrsten Sinne des Wortes. Hier gehören die Nummernschilder dem Besitzer, nicht dem Fahrzeug. Wenn dir jemand mit Genfer Kennzeichen sein Auto verkauft, verlässt er den Parkplatz mit seinen Nummernschildern in der Hand, und du musst deine eigenen bei der kantonalen Kraftfahrzeugbehörde beantragen.</p>
<p>Bevor du das Fahrzeug auf deinen Namen zulassen kannst, musst du über eine gültige Haftpflichtversicherung verfügen. Ohne Haftpflichtversicherung gibt es keine Zulassung: Das ist der erste Schritt, nicht der zweite. Ohne diesen Versicherungsnachweis bearbeitet die kantonale Behörde nichts.</p>
<p>Das Zweite, womit viele nicht rechnen, ist die MFK, die technische Überprüfung. Die erste MFK findet 5 Jahre nach der Erstzulassung statt, danach alle 3 Jahre und anschließend alle 2 Jahre. Wenn du ein Auto kaufst, das älter als 10 Jahre ist und dessen letzte Überprüfung mehr als ein Jahr zurückliegt, musst du die MFK bestehen, bevor du damit fahren darfst. Diese Formalität erledigst du beim kantonalen Strassenverkehrsamt, nicht bei einer beliebigen Werkstatt.</p>
<p>Sobald du die Haftpflichtversicherung (RC) und die Kfz-Versicherung (MFK) in Ordnung gebracht hast, gehst du mit dem Original-Fahrzeugausweis und der Versicherungsbescheinigung zur kantonalen Zulassungsstelle. Du hast ab dem Kauf 14 Tage Zeit, um diesen Vorgang zu erledigen. Die Kennzeichen und der Fahrzeugausweis kosten je nach Kanton in der Regel zwischen 50 und 100 Franken.</p>
<p>Was Sie vor dem Abschluss des Kaufvertrags überprüfen sollten: das Datum der letzten Fahrzeugprüfung (MFK), ob der Fahrzeugausweis in Ordnung ist und ob gegen den Vorbesitzer keine Betreibungen anhängig sind. Ein Kauf ohne Überprüfung dieser drei Punkte kann ein gutes Angebot in einen wochenlangen Ärger mit den kantonalen Behörden verwandeln.</p>
</div>`
    },

    "fb-daily-20260509-03-in-der-schweiz-darf-man-streiken-aber-es-richtig-zu-machen-ist-wesentl": {
        title: "In der Schweiz darf man streiken. Aber es richtig zu machen, ist wesentlich komplizierter, als es scheint.",
        featuredImage: "media/guides/actualidad-en-suiza-se-puede-hacer-huelga-pero-hacerla-b.jpg",
        imageAlt: "In der Schweiz darf man streiken. Aber es richtig zu machen, ist wesentlich komplizierter, als es scheint.",
        description: "Die Schweizer Verfassung garantiert in Artikel 28 das Streikrecht, und die Europäische Menschenrechtskonvention bestätigt dies. Das Problem ist, dass für eine…",
        keywords: "schweiz, darf, streiken, aber, richtig, machen, wesentlich, komplizierter",
        keywordsLocalized: true,
        category: "Arbeit und Löhne",
        hub: "trabajo",
        slug: "actualidad-2026-05-09-3-in-der-schweiz-darf-man-streiken-aber-es-richtig-zu-machen-ist-wesentl",
        readingTime: 1,
        dateUpdated: "9 Mai 2026",
        summary: "Die Schweizer Verfassung garantiert in Artikel 28 das Streikrecht, und die Europäische Menschenrechtskonvention bestätigt dies. Das Problem ist, dass…",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>Die Schweizer Verfassung garantiert in Artikel 28 das Streikrecht, und die Europäische Menschenrechtskonvention bestätigt dies. Das Problem ist, dass für eine rechtmäßige Ausübung dieses Rechts vier Bedingungen gleichzeitig erfüllt sein müssen, wobei die strengste davon fast alles blockiert.</p>
<p>Damit ein Streik in der Schweiz rechtmäßig ist, muss er von einer Organisation ausgerufen werden, die zur Aushandlung eines Gesamtarbeitsvertrags befugt ist; seine Ziele müssen durch diesen Vertrag geregelt werden können; er darf nicht gegen die Verpflichtung zur „paix du travail“ verstoßen und muss verhältnismäßig sein. Ist eine dieser vier Bedingungen nicht erfüllt, ist der Streik unrechtmäßig, und der Arbeitgeber kann die Arbeitnehmer auf Ersatz des entstandenen Schadens verklagen. Das ist keine theoretische Drohung.</p>
<p>Die „Paix du travail“ hat ein genaues Entstehungsdatum: den 19. Juli 1937, als die Schweizer Metallgewerkschaften und die Arbeitgeber eine historische Vereinbarung unterzeichneten. Die Gewerkschaften verzichteten auf Streiks in Angelegenheiten, die bereits im Gesamtarbeitsvertrag geregelt waren; die Arbeitgeber verzichteten auf Aussperrungen. Dieses Modell breitete sich branchenweise aus, bis es zur Norm der Schweizer Arbeitsbeziehungen wurde.</p>
<p>Die Zahlen sprechen für sich. In den 1980er Jahren verlor die Schweiz pro Jahr 1,4 Arbeitstage durch Streiks pro 1.000 Arbeitnehmer. Deutschland verlor 666. Italien 1.123. Das liegt nicht daran, dass die Schweizer Arbeitnehmer mit ihren Arbeitsbedingungen zufriedener sind. Vielmehr ist das System so ausgelegt, dass Konflikte in Verhandlungen kanalisiert werden, bevor sie auf die Straße getragen werden.</p>
<p>Was viele Spanier, die hierherkommen, nicht wissen: Wenn in deiner Branche ein Tarifvertrag gilt und du an einem Streik teilnimmst, der sich auf einen Punkt bezieht, der bereits in diesem Vertrag geregelt ist, kann dein Arbeitgeber dich vor Gericht bringen. Wenn die Schweizer Presse daher von einem „Streik“ spricht, handelt es sich technisch gesehen oft um einen Aktionstag oder eine Demonstration, nicht um einen Arbeitsniederlegung im engeren Sinne. Der rechtliche Unterschied ist von Bedeutung.</p>
</div>`
    },

    "fb-daily-20260510-01-der-auszug-aus-dem-schweizer-strafregister-wie-man-ihn-beantragt-und-w": {
        title: "Der Auszug aus dem Schweizer Strafregister: Wie man ihn beantragt und wofür man ihn benötigt",
        featuredImage: "media/guides/actualidad-el-extracto-del-casier-judiciaire-suizo-como.jpg",
        imageAlt: "Der Auszug aus dem Schweizer Strafregister: Wie man ihn beantragt und wofür man ihn benötigt",
        description: "Das Strafregister ist das schweizerische Bundesregister für strafrechtliche Verurteilungen. Es wird in der ganzen Schweiz auf Französisch so genannt,…",
        keywords: "auszug, schweizer, strafregister, beantragt, wofür, benötigt, schweizerische, bundesregister",
        keywordsLocalized: true,
        category: "Formalitäten und Bewilligungen",
        hub: "tramites",
        slug: "actualidad-2026-05-10-1-der-auszug-aus-dem-schweizer-strafregister-wie-man-ihn-beantragt-und-w",
        readingTime: 2,
        dateUpdated: "10 Mai 2026",
        summary: "Das Strafregister ist das schweizerische Bundesregister für strafrechtliche Verurteilungen. Es wird in der ganzen Schweiz auf Französisch so genannt,…",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>Das Strafregister ist das schweizerische Bundesregister für strafrechtliche Verurteilungen. Es wird in der ganzen Schweiz auf Französisch so genannt, unabhängig davon, in welcher Region du lebst. Wenn jemand dich um einen „l'extrait du casier judiciaire“ bittet, verlangt er damit eine Bescheinigung darüber, dass du hier keine relevanten Verurteilungen hast. Es wird von Arbeitgebern, Gemeinden im Rahmen der Einbürgerung, einigen Vermietern und allen Stellen verwendet, die deine schweizerische Vorstrafenhistorie überprüfen möchten. Es ist nicht das Äquivalent zum Strafregisterauszug, den du in Spanien beantragst: Das ist ein spanisches Dokument und sagt nichts darüber aus, was du in der Schweiz haben könntest.</p>
<p>Es gibt zwei Arten von Führungszeugnissen. Das „extrait ordinaire“ enthält die schwersten Verurteilungen, die noch nicht aus dem Register gelöscht wurden. Der „extrait spécial“ wird beantragt, wenn man mit Minderjährigen oder schutzbedürftigen Personen arbeitet, beispielsweise als Erzieher, Sporttrainer oder in Kindertagesstätten: Dieser enthält alle für diese Tätigkeit relevanten Verurteilungen, auch wenn sie schon länger zurückliegen. Der Unterschied ist wichtig, da in vielen Berufen im Sozial- oder Bildungsbereich nur der spezielle Auszug akzeptiert wird.</p>
<p>Das Verfahren wird vollständig online über casier-judiciaire.admin.ch abgewickelt. Sie müssen weder eine Behörde aufsuchen noch Vermittler einschalten. Sie identifizieren sich mit der SwissID oder Ihrer elektronischen Schweizer Identität, oder Sie nutzen das postalische Verfahren, falls Sie über keine digitale Identifikation verfügen. Die Kosten betragen 17 CHF für die Standardversion, und der Auszug trifft innerhalb weniger Tage ein. Die Gültigkeitsdauer des Dokuments ist gesetzlich nicht festgelegt, doch in der Praxis akzeptieren die meisten Arbeitgeber und Behörden es bis zu 3 oder 6 Monate nach dem Ausstellungsdatum.</p>
<p>Ein Punkt, den nur wenige kennen: Wenn du diesen Auszug im Ausland vorlegen musst, zum Beispiel in Spanien, musst du ihn mit einer Apostille versehen lassen. Die Apostille wird vom Eidgenössischen Justizdepartement ausgestellt und kostet zusätzlich 35 CHF. Ohne sie hat das Dokument außerhalb der Schweiz keine offizielle Gültigkeit. Und wenn du für ein Verfahren in der Schweiz ein spanisches Führungszeugnis benötigst, musst du dieses direkt beim spanischen Justizministerium beantragen, das über ein eigenes Online-Portal verfügt.</p>
</div>`
    },

    "fb-daily-20260510-02-die-144-in-der-schweiz-anrufen-die-nummern-die-leben-retten-und-die-re": {
        title: "Die 144 in der Schweiz anrufen: Die Nummern, die Leben retten – und die Rechnung, mit der viele nicht rechnen",
        featuredImage: "media/guides/actualidad-llamar-al-144-en-suiza-los-numeros-que-salvan.jpg",
        imageAlt: "Die 144 in der Schweiz anrufen: Die Nummern, die Leben retten – und die Rechnung, mit der viele nicht rechnen",
        description: "Das Wichtigste zuerst: 117 ist die Polizei, 118 die Feuerwehr, 144 der Rettungsdienst und 1414 die Luftrettung, die Rega. Diese vier Nummern sind in der ganzen…",
        keywords: "schweiz, anrufen, nummern, leben, retten, rechnung, viele, nicht",
        keywordsLocalized: true,
        category: "Gesundheit und KVG",
        hub: "salud",
        slug: "actualidad-2026-05-10-2-die-144-in-der-schweiz-anrufen-die-nummern-die-leben-retten-und-die-re",
        readingTime: 2,
        dateUpdated: "10 Mai 2026",
        summary: "Das Wichtigste zuerst: 117 ist die Polizei, 118 die Feuerwehr, 144 der Rettungsdienst und 1414 die Luftrettung, die Rega. Diese vier Nummern sind in…",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>Das Wichtigste zuerst: 117 ist die Polizei, 118 die Feuerwehr, 144 der Rettungsdienst und 1414 die Luftrettung, die Rega. Diese vier Nummern sind in der ganzen Schweiz an 365 Tagen im Jahr und in jeder Sprache erreichbar. Falls der Telefonist dich nicht auf Spanisch bedienen kann, wird er dich mit jemandem verbinden, der dies kann. Sie stehen nicht im Reisepass, sind in keiner Willkommensbroschüre zu finden, und manche suchen sie erst im ungünstigsten Moment. Speichere sie dir jetzt.</p>
<p>Was viele Menschen nicht wissen, ist, dass ein Anruf unter der Nummer 144 nicht kostenlos ist, zumindest nicht in dem Sinne, wie wir es verstehen. Der Anruf selbst kostet nichts, der Einsatz jedoch schon. Die Grundversicherung nach KVG übernimmt einen Teil der Kosten für den Krankentransport: theoretisch die Hälfte, bis zu maximal 500 CHF pro Jahr für einen regulären Transport oder bis zu 5'000 CHF im Falle einer Rettungsaktion. Das Problem ist, dass ein Ambulanztransport in der Schweiz je nach Kanton und Einsatzdauer leicht zwischen 500 und 1'500 CHF allein für die Fahrt kosten kann. Was die Versicherung nicht übernimmt, musst du aus eigener Tasche bezahlen.</p>
<p>Der auffälligste Fall ist die Bergrettung. Die Rega entsendet Hubschrauber, wenn es die Situation erfordert, und ein einziger Einsatz kann zwischen 3'000 und 50'000 Franken kosten. Wenn du Mitglied der Rega bist, beträgt der Jahresbeitrag etwa 40 CHF für einen Erwachsenen oder 70 CHF für eine Familie, und die Rettungskosten, die nicht von der Grundversicherung übernommen werden, werden dir direkt erlassen. Wenn du kein Mitglied bist, erhältst du die vollständige Rechnung über den Betrag, den die KVG nicht bezahlt. Das ist eine dieser Sachen, die die Einheimischen in der Schweiz genau wissen, die aber niemand den Neuankömmlingen erklärt.</p>
<p>Erwähnenswert ist auch der TCS: Sein Assistance-Service deckt bestimmte medizinische Transporte ab und kann die Leistungen ergänzen, die das KVG nicht übernimmt. Er ist nicht mit der Rega vergleichbar und greift nicht immer bei Notfällen in den Bergen, aber bei Verkehrsunfällen oder medizinischen Notfällen unterwegs kann er je nach Vertrag nützlich sein. Auf jeden Fall ist es wichtig zu wissen, dass die Notfalldienste in der Schweiz zwar ausgezeichnet, aber nicht kostenlos sind, und dass es entscheidend ist, genau zu wissen, was Ihre Versicherung abdeckt, bevor Sie sie in Anspruch nehmen müssen.</p>
</div>`
    },

    "fb-daily-20260510-03-schwarzarbeit-in-der-schweiz-was-du-wirklich-riskierst-wenn-dir-jemand": {
        title: "Schwarzarbeit in der Schweiz: Was du wirklich riskierst, wenn dir jemand anbietet, dich bar und ohne Quittung zu bezahlen",
        featuredImage: "media/guides/actualidad-el-trabajo-en-negro-en-suiza-lo-que-realmente.jpg",
        imageAlt: "Schwarzarbeit in der Schweiz: Was du wirklich riskierst, wenn dir jemand anbietet, dich bar und ohne Quittung zu bezahlen",
        description: "Das Argument klingt immer plausibel: Es handelt sich um eine einmalige Sache, es gibt keine Unterlagen, das Geld fließt ungemeldet und niemand wird davon…",
        keywords: "schwarzarbeit, schweiz, wirklich, riskierst, wenn, jemand, anbietet, dich",
        keywordsLocalized: true,
        category: "Arbeit und Löhne",
        hub: "trabajo",
        slug: "actualidad-2026-05-10-3-schwarzarbeit-in-der-schweiz-was-du-wirklich-riskierst-wenn-dir-jemand",
        readingTime: 2,
        dateUpdated: "10 Mai 2026",
        summary: "Das Argument klingt immer plausibel: Es handelt sich um eine einmalige Sache, es gibt keine Unterlagen, das Geld fließt ungemeldet und niemand wird…",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>Das Argument klingt immer plausibel: Es handelt sich um eine einmalige Sache, es gibt keine Unterlagen, das Geld fließt ungemeldet und niemand wird davon erfahren. In der Schweiz hat dies einen offiziellen Namen, „travail au noir“, und das Gesetz gegen die Schwarzarbeit von 2005 sieht Konsequenzen vor, die weit über eine Verwaltungsstrafe hinausgehen. Der häufigste Irrtum ist die Annahme, dass nur der Arbeitgeber ein Risiko eingeht. In Wirklichkeit setzt auch der Arbeitnehmer konkrete Dinge aufs Spiel, von denen manche nur schwer wiederherzustellen sind.</p>
<p>Der erste und unmittelbarste Punkt betrifft die Versicherung. Wenn du in der Schweiz bei der Schwarzarbeit einen Arbeitsunfall erleidest, bist du nicht durch das UVG, die obligatorische Unfallversicherung, abgesichert. Das bedeutet, dass du einen Spitalaufenthalt infolge dieses Unfalls selbst bezahlen musst, da das KVG die Kosten nicht übernimmt. Du hast auch keinen Anspruch auf Arbeitslosenunterstützung, wenn du diesen Job verlierst: Das RAV erkennt nicht angemeldete Beschäftigungen nicht an, und die Wochen, die du schwarz gearbeitet hast, zählen nicht für die Beitragszeiten. Das System macht keine Ausnahmen, auch wenn dir die Situation vorübergehend oder harmlos erscheint.</p>
<p>Für Ausländer mit Aufenthaltsgenehmigung sind die Folgen schwerwiegender. Eine Verurteilung oder Sanktion wegen Schwarzarbeit oder wegen der Ausübung einer Tätigkeit ohne entsprechende Genehmigung kann sich auf die Verlängerung der Aufenthaltsgenehmigung der Kategorie B auswirken und bei wiederholten Verstößen als nachteiliger Faktor in einem Einbürgerungsverfahren gewertet werden. Dies ist zwar keine automatische oder sofortige Folge, aber es handelt sich um einen Eintrag, der bestehen bleibt. Das Ausländergesetz erlaubt es, Aufenthaltsgenehmigungen nicht zu verlängern, wenn die Person wiederholt gegen die Arbeitsmarktvorschriften verstoßen hat, und Fälle von nicht angemeldeter Arbeit werden registriert.</p>
<p>Der dritte Aspekt, den fast niemand berücksichtigt, ist die AHV. Jeder Monat nicht angemeldeter Arbeit ist ein Monat ohne Beitragszahlungen. In der Schweiz baut sich die Rente Jahr für Jahr auf, und Beitragslücken verringern die Höhe der Altersrente. Wenn du jahrelang teilweise schwarz gearbeitet hast und mit Lücken in deiner Versicherungshistorie das Rentenalter erreichst, fragt das System nicht nach dem Grund: Es berechnet einfach, was du eingezahlt hast, und zahlt dir diesen Betrag aus. Es gibt keine Möglichkeit, nicht versicherte Jahre nachträglich auszugleichen. Für jemanden, der seit 20 oder 30 Jahren in der Schweiz lebt, kann der Unterschied für den Rest seines Lebens mehrere hundert Franken pro Monat betragen.</p>
</div>`
    },

    "fb-daily-20260415-01-ruckgabe-von-einkaufen-in-der-schweiz-was-moglich-ist-und-was-nicht": {
        title: "Rückgabe von Einkäufen in der Schweiz: Was möglich ist und was nicht",
        featuredImage: "media/guides/actualidad-devolver-compras-en-suiza-lo-que-si-y-lo-que.jpg",
        imageAlt: "Rückgabe von Einkäufen in der Schweiz: Was möglich ist und was nicht",
        description: "Rückgabe von Einkäufen in der Schweiz: Was möglich ist und was nicht In der Schweiz gibt es kein allgemeines Rückgaberecht. So einfach ist das. Es ist nicht…",
        keywords: "rückgabe, einkäufen, schweiz, möglich, nicht, gibt, kein, allgemeines",
        keywordsLocalized: true,
        category: "Arbeit und Löhne",
        hub: "trabajo",
        slug: "actualidad-2026-04-15-1-ruckgabe-von-einkaufen-in-der-schweiz-was-moglich-ist-und-was-nicht",
        readingTime: 3,
        dateUpdated: "15 April 2026",
        summary: "Rückgabe von Einkäufen in der Schweiz: Was möglich ist und was nicht In der Schweiz gibt es kein allgemeines Rückgaberecht. So einfach ist das. Es…",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>Rückgabe von Einkäufen in der Schweiz: Was möglich ist und was nicht In der Schweiz gibt es kein allgemeines Rückgaberecht. So einfach ist das. Es ist nicht wie in anderen Ländern, wo man seine Meinung ändern und ein Produkt innerhalb einer bestimmten Frist zurückgeben kann. Wenn man hier auf eigene Initiative etwas kauft – sei es im Laden oder sogar online –, hat man nicht automatisch das Recht, es zurückzugeben, nur weil man es nicht mehr will. Die Rechtsgrundlage ist ziemlich eindeutig. Ein Widerrufsrecht besteht nur in ganz bestimmten, im Obligationenrecht vorgesehenen Fällen, insbesondere in Art. 40a OR ff. Und es hat nichts mit Reue zu tun, sondern mit dem Schutz vor Verkaufsdruck. Dieses 14-tägige Recht gilt nur, wenn man dich aufgesucht hat, um dir etwas zu verkaufen. Zum Beispiel bei Haustürgeschäften, auf der Strasse oder bei Werbeveranstaltungen, bei denen du nichts bestellt hast. Es gilt auch für telefonisch abgeschlossene Verträge, wenn du während eines Anrufs ein Angebot annimmst. In beiden Fällen muss der Vertragswert über 100 CHF liegen. Sind diese Bedingungen erfüllt, kannst du den Vertrag innerhalb von 14 Tagen ohne Strafzahlung widerrufen. Diese Frist beginnt jedoch ab dem Zeitpunkt, an dem du das Angebot annimmst, nicht erst, wenn du die Ware oder Dienstleistung erhältst. Ausserdem muss der Anbieter dich ordnungsgemäss über dieses Recht informiert haben. Tut er dies nicht, ändert sich die Situation. Ausserhalb dieser Fälle besteht kein gesetzliches Recht, etwas nur wegen eines einfachen Sinneswandels zurückzugeben. Wenn du ein Geschäft betrittst, ein Produkt kaufst und es dir dann anders überlegst, bist du allein von den Geschäftsbedingungen des Händlers abhängig. Das Gleiche gilt für Online-Käufe, die du aus eigener Initiative getätigt hast. Wenn Rückgaben akzeptiert werden, dann weil der Händler es will, nicht weil er dazu verpflichtet ist. Das sorgt für ziemliche Verwirrung, da viele Geschäfte in der Schweiz Rückgaben akzeptieren. Aber man muss das richtig verstehen: Es ist kein Recht, sondern eine Geste des Händlers. Jedes Unternehmen legt seine eigenen Regeln fest: Frist, Rückerstattung in bar oder als Gutschein, unbenutztes Produkt, intakte Verpackung usw. Bei Versicherungen oder Dienstleistungen gilt das gleiche Prinzip. Das 14-tägige Widerrufsrecht kann ebenfalls gelten, jedoch nur, wenn der Vertrag unter kommerziellem Druck abgeschlossen wurde. Das heisst, wenn er dir am Telefon oder auf der Strasse verkauft wird, ohne dass du ihn zuvor angefordert hast. In der Praxis bedeutet dies, dass viele Versicherungsverträge nicht unter diese Regelung fallen. Denn normalerweise bist du es, der das Angebot anfordert oder den Prozess initiiert. In diesem Fall besteht kein automatisches Widerrufsrecht mehr und es gelten ausschliesslich die Bedingungen des von dir unterzeichneten Vertrags. Ein anschauliches Beispiel: Du wirst angerufen und stimmst sofort einem Versicherungswechsel zu. Dann hast du 14 Tage Zeit, um davon zurückzutreten. Wenn du jedoch selbst die Versicherung kontaktierst und später unterschreibst, hast du dieses Recht standardmässig nicht. Das Gleiche gilt für ein Fitnessstudio, Internet oder jede andere Dienstleistung. In der Schweiz ist die Logik einfach: Das Gesetz schützt dich, wenn du zum Kauf gedrängt wirst, nicht aber, wenn du dich selbst entscheidest. Deshalb gibt es hier weniger ein Rückgaberecht als vielmehr die Pflicht, vor der Unterzeichnung zu lesen. Denn sobald du aus eigenem Antrieb zugestimmt hast, gibt es kein Zurück mehr, es sei denn, der Vertrag lässt dies zu.</p>
</div>`
    },

    "fb-daily-20260415-02-httpswwwespanolesensuizach": {
        title: "https://www.espanolesensuiza.ch/",
        featuredImage: "media/guides/actualidad-httpswwwespanolesensuizach.jpg",
        imageAlt: "https://www.espanolesensuiza.ch/",
        description: "https://www.espanolesensuiza.ch/ Seit einiger Zeit sehe ich in der Gruppe immer wieder dasselbe. Die gleichen Fragen, die gleichen Zweifel, die gleichen…",
        keywords: "https, espanolesensuiza, seit, einiger, zeit, sehe, gruppe, immer",
        keywordsLocalized: true,
        category: "Formalitäten und Bewilligungen",
        hub: "tramites",
        slug: "actualidad-2026-04-15-2-httpswwwespanolesensuizach",
        readingTime: 2,
        dateUpdated: "15 April 2026",
        summary: "https://www.espanolesensuiza.ch/ Seit einiger Zeit sehe ich in der Gruppe immer wieder dasselbe. Die gleichen Fragen, die gleichen Zweifel, die…",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>https://www.espanolesensuiza.ch/ Seit einiger Zeit sehe ich in der Gruppe immer wieder dasselbe. Die gleichen Fragen, die gleichen Zweifel, die gleichen Antworten … und oft auch die gleichen Fehlinformationen. Das ist niemandes Schuld, es liegt einfach daran, dass in der Schweiz jedes Thema Nuancen, Gesetze und kantonale Unterschiede aufweist, die auf den ersten Blick nicht offensichtlich sind. Deshalb habe ich daran gearbeitet: https://www.espanolesensuiza.ch/ Die Idee ist ganz einfach. Klare Informationen zu bündeln, die auf offiziellen Schweizer Quellen wie ch.ch, SEM oder BFS basieren und auf Spanisch so erklärt werden, dass man sie wirklich versteht. Ohne Schnickschnack, ohne Versprechungen à la „Komm in die Schweiz und werde in zwei Wochen erfolgreich“ und ohne das Erstbeste zu kopieren und einzufügen, was bei Google auftaucht. Es gibt Leitfäden zu Aufenthaltsgenehmigungen, Arbeit, Versicherungen, Steuern, Lebenshaltungskosten, Grenzgängern … und die Idee ist, das Angebot nach und nach weiter auszubauen. Es ist auch so konzipiert, dass es praktisch ist – nicht nur zum Lesen aus Neugier, sondern damit es euch hilft, wenn ihr es wirklich braucht. Nun ist dies aber kein abgeschlossenes Projekt. Im Gegenteil. Ich möchte euer ehrliches Feedback. Wenn etwas unverständlich ist, wenn ihr Fehler seht, wenn wichtige Informationen fehlen, wenn ein Leitfaden nicht das bietet, wonach ihr gesucht habt… sagt es mir. Ohne Filter. Ich ziehe direkte Kritik jetzt vor, als dass sich hundert Leute in Inhalten verlieren, die nicht helfen. Mich interessiert auch, wie ihr es in der Anwendung findet: ob die Informationen leicht zu finden sind, ob die Struktur Sinn ergibt, ob ihr es nützlich findet oder ob es Dinge gibt, die überflüssig sind. Die Idee ist nicht, „noch eine weitere Website“ zu erstellen. Es geht darum, etwas zu schaffen, das denen wirklich nützt, die hier sind oder die darüber nachdenken, hierherzukommen. Wenn ihr also 5 Minuten Zeit habt, schaut es euch an und lasst es mich wissen. Nur so macht das Ganze Sinn.</p>
</div>`
    },

    "fb-daily-20260415-01-einkaufe-in-der-schweiz-zuruckgeben-was-es-gibt-und-was-nicht": {
        title: "Einkäufe in der Schweiz zurückgeben: Was es gibt und was nicht",
        featuredImage: "media/guides/actualidad-devolver-compras-en-suiza-lo-que-si-y-lo-que.jpg",
        imageAlt: "Einkäufe in der Schweiz zurückgeben: Was es gibt und was nicht",
        description: "Rückgabe von Einkäufen in der Schweiz: Was es gibt und was nicht In der Schweiz gibt es kein allgemeines Rückgaberecht. So klar. Es ist nicht wie in anderen…",
        keywords: "einkäufe, schweiz, zurückgeben, gibt, nicht, rückgabe, einkäufen, kein",
        keywordsLocalized: true,
        category: "Arbeit und Löhne",
        hub: "trabajo",
        slug: "actualidad-2026-04-15-1-einkaufe-in-der-schweiz-zuruckgeben-was-es-gibt-und-was-nicht",
        readingTime: 3,
        dateUpdated: "15 April 2026",
        summary: "Rückgabe von Einkäufen in der Schweiz: Was es gibt und was nicht In der Schweiz gibt es kein allgemeines Rückgaberecht. So klar. Es ist nicht wie in…",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>Rückgabe von Einkäufen in der Schweiz: Was es gibt und was nicht In der Schweiz gibt es kein allgemeines Rückgaberecht. So klar. Es ist nicht wie in anderen Ländern, in denen Sie Ihre Meinung ändern und ein Produkt innerhalb einer bestimmten Frist zurückgeben können. Wenn Sie hier etwas auf eigene Faust, im Laden oder sogar online kaufen, haben Sie nicht automatisch das Recht, es zurückzugeben, weil Sie es nicht mehr wollen. Die Rechtsgrundlage ist ziemlich klar. Das Widerrufsrecht besteht nur in ganz bestimmten Situationen, die im Obligationenrecht, insbesondere Art. 40a OR ff., vorgesehen sind. Und das hat nichts mit Reue zu tun, sondern mit dem Schutz vor kommerziellem Druck. Dieses 14-tägige Recht gilt nur, wenn Sie abgeholt wurden, um Ihnen etwas zu verkaufen. Zum Beispiel bei Hausverkäufen, auf der Straße oder bei Werbeveranstaltungen, bei denen Sie nichts bestellt haben. Dies gilt auch für telefonisch abgeschlossene Verträge, wenn Sie während eines Anrufs ein Angebot annehmen. In beiden Fällen muss der Vertrag CHF 100 übersteigen. Wenn diese Bedingungen erfüllt sind, können Sie den Vertrag innerhalb von 14 Tagen ohne Vertragsstrafe kündigen. Diese Frist beginnt jedoch mit der Annahme des Angebots, nicht mit dem Erhalt des Produkts oder der Dienstleistung. Darüber hinaus muss der Lieferant Sie ordnungsgemäß über dieses Recht informiert haben. Wenn Sie das nicht tun, ändert sich die Situation. Außerhalb dieser Fälle besteht kein gesetzliches Rückgaberecht für einen einfachen Sinneswandel. Wenn du in ein Geschäft gehst, ein Produkt kaufst und es dann bereust, bist du allein auf die Handelspolitik des Handels angewiesen. Gleiches gilt für Online-Einkäufe, die auf eigene Initiative getätigt werden. Wenn sie Rücksendungen akzeptieren, liegt das daran, dass sie es wollen, nicht weil sie dazu verpflichtet sind. Das ist ziemlich verwirrend, da viele Geschäfte in der Schweiz Rücksendungen akzeptieren. Aber man muss es gut verstehen: Es ist kein Recht, es ist eine kommerzielle Geste. Jedes Unternehmen legt seine eigenen Regeln fest. Frist, Geld- oder Gutscheinrückerstattung, unbenutztes Produkt, intakte Verpackung usw. Bei Versicherungen oder Dienstleistungen ist das Prinzip dasselbe. Das 14-tägige Widerrufsrecht kann ebenfalls zur Anwendung kommen, jedoch nur, wenn der Vertrag unter kaufmännischem Druck zustande gekommen ist. Das heißt, wenn es Ihnen telefonisch oder auf der Straße verkauft wird, ohne dass Sie es zuvor angefordert haben. In der Praxis bedeutet dies, dass viele Versicherungsverträge nicht in diesen Rahmen fallen. Denn in der Regel sind Sie es, der nach dem Angebot fragt oder den Prozess initiiert. In diesem Fall besteht kein automatisches Widerrufsrecht mehr und es gelten nur noch die Bedingungen des von Ihnen unterzeichneten Vertrages. Klares Beispiel. Sie werden angerufen und stimmen zu, in diesem Moment die Versicherung zu wechseln. Ja, Sie haben 14 Tage Zeit, um sich zurückzuziehen. Wenn Sie jedoch den Versicherer kontaktieren und später unterschreiben, haben Sie dieses Recht standardmäßig nicht. Gleiches gilt für ein Fitnessstudio, das Internet oder andere Dienstleistungen. In der Schweiz ist die Logik einfach: Das Gesetz schützt Sie, wenn Sie zum Kauf gedrängt werden, nicht wenn Sie sich entscheiden. Deshalb gibt es hier mehr als ein Rückgaberecht, was wirklich existiert, ist die Verantwortung, vor der Unterzeichnung zu lesen. Denn sobald du auf eigene Faust ja gesagt hast, gibt es kein Zurück mehr, es sei denn, der Vertrag erlaubt es.</p>
</div>`
    },

    "fb-daily-20260413-01-radfahren-in-der-schweiz-scheint-einfach-zu-sein-bis-sie-die-bussgelde": {
        title: "Radfahren in der Schweiz scheint einfach zu sein... bis Sie die Bußgelder sehen.",
        featuredImage: "media/guides/actualidad-circular-en-bicicleta-en-suiza-parece-sencill.jpg",
        imageAlt: "Radfahren in der Schweiz scheint einfach zu sein... bis Sie die Bußgelder sehen.",
        description: "Radfahren in der Schweiz scheint einfach zu sein... bis Sie die Bußgelder sehen. In der Schweiz ist das Fahrrad kein \"gesetzloses Land\". Sie hat klare Regeln,…",
        keywords: "radfahren, schweiz, scheint, einfach, sein, gelder, sehen, fahrrad",
        keywordsLocalized: true,
        category: "Arbeit und Löhne",
        hub: "trabajo",
        slug: "actualidad-2026-04-13-1-radfahren-in-der-schweiz-scheint-einfach-zu-sein-bis-sie-die-bussgelde",
        readingTime: 1,
        dateUpdated: "13 April 2026",
        summary: "Radfahren in der Schweiz scheint einfach zu sein... bis Sie die Bußgelder sehen. In der Schweiz ist das Fahrrad kein \"gesetzloses Land\". Sie hat…",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>Radfahren in der Schweiz scheint einfach zu sein... bis Sie die Bußgelder sehen. In der Schweiz ist das Fahrrad kein "gesetzloses Land". Sie hat klare Regeln, die die Leute oft nicht kennen, bis sie die Rechnung bekommen. Für den Anfang gelten Fahrräder als Fahrzeuge. Das bedeutet, dass Sie Schilder, Ampeln und Prioritäten wie ein Auto respektieren müssen. Das Überspringen einer roten Ampel ist kein „Unfug“, sondern ein Verstoß. Thema Radwege: Wenn es einen ausgeschilderten Weg gibt, sind Sie verpflichtet, ihn zu benutzen. Es ist nicht optional. Das Fahren auf der Straße "weil ja", wenn Sie einen Radweg neben sich haben, kann zu einer Geldstrafe führen. Ein weiterer Klassiker: Fahren Sie auf dem Bürgersteig. Es ist nur zulässig, wenn ein Zeichen vorhanden ist, das dies autorisiert. Wenn nicht, berühren Sie die Fahrbahn oder den Radweg. Und ja, auch das wird sanktioniert. Lichter: Nachts oder bei schlechter Sicht obligatorisch. Weiß vorne, rot hinten. Es scheint einfach zu sein, aber jeden Winter gibt es Kampagnen, weil viele Leute vom Thema abweichen. Kopfhörer und Handy: Sie sind als solche nicht verboten, aber wenn sie Ihre Reaktionsfähigkeit beeinträchtigen, können Sie mit einer Geldstrafe belegt werden. In der Schweiz ist das Kriterium einfach: Wenn Sie die Sicherheit gefährden, zahlen Sie. Alkohol: Ja, das gilt auch. Ab einem bestimmten Niveau können Sie genauso bestraft werden wie beim Autofahren. Hier gibt es keine offene Bar, weil Sie "Fahrrad fahren". Und Vorsicht vor etwas, was viele nicht wissen: Bei einem Unfall kann die Haftpflichtversicherung ins Spiel kommen. Wenn Sie Schäden verursachen, können Sie am Ende viel mehr als eine Geldstrafe zahlen. Die Schweiz fördert das Radfahren sehr, ist aber kein Themenpark. Es ist ein System, in dem alles geregelt ist. Und wenn ich alles sage... das ist alles.</p>
</div>`
    },

    "fb-daily-20260511-01-haftpflichtversicherung-und-bodeninhaltsversicherung-in-der-schweiz-zw": {
        title: "Haftpflichtversicherung und Bodeninhaltsversicherung in der Schweiz: zwei verschiedene Dinge, die fast immer schlecht gemietet werden",
        featuredImage: "media/guides/actualidad-el-seguro-rc-y-el-seguro-de-contenido-del-pis.jpg",
        imageAlt: "Haftpflichtversicherung und Bodeninhaltsversicherung in der Schweiz: zwei verschiedene Dinge, die fast immer schlecht gemietet werden",
        description: "In der Schweiz gibt es drei Versicherungsschichten, die sich auf einen Mieter auswirken können, und eine Verwechslung kann im am wenigsten erwarteten Moment…",
        keywords: "haftpflichtversicherung, bodeninhaltsversicherung, schweiz, zwei, verschiedene, dinge, fast, immer",
        keywordsLocalized: true,
        category: "Arbeit und Löhne",
        hub: "trabajo",
        slug: "actualidad-2026-05-11-1-haftpflichtversicherung-und-bodeninhaltsversicherung-in-der-schweiz-zw",
        readingTime: 1,
        dateUpdated: "11 Mai 2026",
        summary: "In der Schweiz gibt es drei Versicherungsschichten, die sich auf einen Mieter auswirken können, und eine Verwechslung kann im am wenigsten erwarteten…",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>In der Schweiz gibt es drei Versicherungsschichten, die sich auf einen Mieter auswirken können, und eine Verwechslung kann im am wenigsten erwarteten Moment ein wichtiges Loch hinterlassen. Die erste ist die Gebäudeversicherung, die in den meisten Kantonen obligatorisch ist, aber vom Eigentümer und nicht vom Mieter getragen wird. Es deckt die Wände, die Struktur und die festen Elemente der Wohnung ab. Was sich im Boden befindet, Ihr Sofa, Ihr Computer, Ihre Kleidung, gibt es für diese Versicherung nicht.</p>
<p>Die zweite Schicht ist die Hausratversicherung (assurance ménage oder inventaire du ménage), die Ihre persönlichen Gegenstände vor Feuer, Diebstahl, Wasserschäden oder Naturkatastrophen schützt. Es ist auf nationaler Ebene nicht obligatorisch, aber wenn Ihre Wohnung in Flammen steht oder jemand einbricht und Sie nicht über diese Versicherung verfügen, liegt der Verlust zu hundert Prozent bei Ihnen. Die dritte ist die private Haftpflichtversicherung (responsabilité civile), die Schäden abdeckt, die Sie Dritten zufügen: Sie brechen die Brille eines Nachbarn, verschütten Wasser aus Versehen und verderben den Boden unten oder Ihr Kind schlägt einen Ballon auf etwas Teures. Die Haftpflichtversicherung deckt nicht die Schäden ab, die Sie erleiden, sondern nur die, die Sie verursachen.</p>
<p>Das übliche Problem ist, dass Versicherungsunternehmen sowohl Ménage als auch RC in einem einzigen Vertrag verkaufen, was den Eindruck erweckt, dass sie dasselbe sind. Das sind sie nicht. Und es gibt einen weiteren häufigen Fehler: Viele Mieter glauben, dass das Gebäude „bereits versichert“ ist und dass sie dadurch geschützt werden. Die Gebäudeabdeckung ist für das Gebäude, nicht für das, was Sie darin aufbewahren. Sie können die beiden Deckungen mit verschiedenen Unternehmen haben, wenn es Ihnen finanziell passt, obwohl das kombinierte Angebot in der Regel einen angemessenen Preis hat. Wichtig ist, dass du verstehst, was jede abdeckt, bevor du sie brauchst.</p>
</div>`
    },

    "fb-daily-20260511-02-die-garantie-in-der-schweiz-dauert-gesetzlich-zwei-jahre-aber-die-inan": {
        title: "Die Garantie in der Schweiz dauert gesetzlich zwei Jahre, aber die Inanspruchnahme hat mehr Anforderungen, als Sie sich vorstellen können",
        featuredImage: "media/guides/actualidad-la-garantia-en-suiza-dura-dos-anos-por-ley-pe.jpg",
        imageAlt: "Die Garantie in der Schweiz dauert gesetzlich zwei Jahre, aber die Inanspruchnahme hat mehr Anforderungen, als Sie sich vorstellen können",
        description: "Das schweizerische Gesetz (Obligationenrecht, Art. 210) legt die Frist für die Reklamation von Mängeln an einem gekauften Produkt auf zwei Jahre fest. Bis…",
        keywords: "garantie, schweiz, dauert, gesetzlich, zwei, jahre, aber, inanspruchnahme",
        keywordsLocalized: true,
        category: "Wohnen",
        hub: "vivienda",
        slug: "actualidad-2026-05-11-2-die-garantie-in-der-schweiz-dauert-gesetzlich-zwei-jahre-aber-die-inan",
        readingTime: 1,
        dateUpdated: "11 Mai 2026",
        summary: "Das schweizerische Gesetz (Obligationenrecht, Art. 210) legt die Frist für die Reklamation von Mängeln an einem gekauften Produkt auf zwei Jahre…",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>Das schweizerische Gesetz (Obligationenrecht, Art. 210) legt die Frist für die Reklamation von Mängeln an einem gekauften Produkt auf zwei Jahre fest. Bis dahin hört sich alles gut an. Das Detail, das alles ändert, ist, dass diese Frist nicht bedeutet, dass Sie zwei Jahre Zeit haben, um zu entscheiden, ob Ihnen das Produkt gefällt oder nicht: Es bedeutet, dass Sie zwei Jahre Zeit haben, um einen versteckten Fehler zu entdecken, der zum Zeitpunkt des Kaufs bestand, und den Verkäufer sofort darüber zu informieren. Wenn du es herausfindest und wochenlang auf die Geltendmachung wartest, kannst du dieses Recht verlieren.</p>
<p>Der andere Punkt, den viele nicht kennen, ist, dass die gesetzliche Garantie bereits bestehende Mängel abdeckt, keinen normalen Verschleiß oder Probleme, die sich aus der Verwendung ergeben. Wenn ein Gerät kaputt geht, weil Sie es missbraucht haben, hilft Ihnen die Garantie nicht weiter. Darüber hinaus kann der Verkäufer beim Kauf eines gebrauchten Produkts die Gewährleistungsfrist rechtlich auf ein Jahr verkürzen. Was Sie bei Verkäufen zwischen Unternehmen und Verbraucher nicht tun können, ist, diese Frist vollständig zu beseitigen, obwohl Sie dies mit ausdrücklichen Vertragsklauseln versuchen können, die außerhalb der Allgemeinen Geschäftsbedingungen enthalten sind.</p>
<p>Es ist auch zwischen der gesetzlichen Garantie und der gewerblichen Garantie zu unterscheiden, die von Herstellern oder Geschäften angeboten wird. Letzteres kann großzügiger sein als das Gesetz, aber seine Bedingungen werden vom Verkäufer festgelegt. In der Schweiz gibt es kein automatisches Rückgaberecht, wenn ein Produkt gut funktioniert, Ihnen aber einfach nicht gefällt: Dieses Recht gibt es in keinem Gesetz, es hängt von der Politik des jeweiligen Geschäfts ab. Wer aus Spanien kommt und die beiden Systeme verwechselt, entdeckt dies normalerweise am Rückgabeschalter, mit wenigen Freunden und wenig Handlungsspielraum.</p>
</div>`
    },

    "fb-daily-20260511-03-die-schweizer-e-id-weder-staatliche-uberwachung-noch-dringende-behandl": {
        title: "Die Schweizer E-ID: Weder staatliche Überwachung noch dringende Behandlung, aber es ist ratsam zu verstehen, was es ist, bevor es ankommt",
        featuredImage: "media/guides/actualidad-el-e-id-suizo-ni-vigilancia-estatal-ni-tramit.jpg",
        imageAlt: "Die Schweizer E-ID: Weder staatliche Überwachung noch dringende Behandlung, aber es ist ratsam zu verstehen, was es ist, bevor es ankommt",
        description: "Ab Dezember 2026 wird die Schweiz ihr offizielles elektronisches Identitätssystem einführen. Es heißt E-ID, funktioniert über eine kostenlose App namens swiyu…",
        keywords: "schweizer, weder, staatliche, überwachung, noch, dringende, behandlung, aber",
        keywordsLocalized: true,
        category: "Formalitäten und Bewilligungen",
        hub: "tramites",
        slug: "actualidad-2026-05-11-3-die-schweizer-e-id-weder-staatliche-uberwachung-noch-dringende-behandl",
        readingTime: 1,
        dateUpdated: "11 Mai 2026",
        summary: "Ab Dezember 2026 wird die Schweiz ihr offizielles elektronisches Identitätssystem einführen. Es heißt E-ID, funktioniert über eine kostenlose App…",
        relatedSlugs: [],
        content: `<div class="article-content">
<p>Ab Dezember 2026 wird die Schweiz ihr offizielles elektronisches Identitätssystem einführen. Es heißt E-ID, funktioniert über eine kostenlose App namens swiyu Wallet und ermöglicht die Verwendung des Mobiltelefons als gültiges Ausweisdokument in digitalen und persönlichen Kontexten: um das Alter an einer Supermarktkasse zu überprüfen, ein Online-Bankkonto zu eröffnen, Verwaltungsformalitäten zu erledigen oder die Identität während einer Polizeikontrolle nachzuweisen. Das physische Dokument verschwindet nicht. Die E-ID ist eine freiwillige Alternative, kein obligatorischer Ersatz.</p>
<p>Was in einigen Gruppen bereits im Umlauf ist, ist die Idee, dass es sich um ein zentrales Überwachungs- oder Kontrollsystem handelt. Das technische Design weist in die entgegengesetzte Richtung: Die Daten werden nicht auf einem zentralen Server des Staates gespeichert, sondern auf dem eigenen Telefon des Benutzers. Wenn jemand die E-ID verwendet, kann das Unternehmen oder die Behörde nur die Daten anfordern, die für diesen bestimmten Vorgang unbedingt erforderlich sind, und der Benutzer sieht, was von ihm verlangt wird, und kann dies ablehnen. Es ist das Prinzip der Datenminimierung, das technisch angewendet wird, nicht als einfaches Versprechen auf Papier.</p>
<p>Für Ausländer hat dies eine direkte Auswirkung: Die E-ID ist nicht nur für Schweizer Staatsangehörige. Wer eine gültige Aufenthaltserlaubnis besitzt, kann diese auch beantragen. Dies vereinfacht Formalitäten wie die Identitätsprüfung für Online-Verträge, die Eröffnung von Konten ohne Umzug in ein Büro oder die Unterzeichnung rechtsgültiger digitaler Dokumente. Das System ist noch nicht verfügbar: Der Start ist für Dezember 2026 geplant. Es bleibt noch Zeit, es gut zu verstehen, bevor alle damit beginnen, es für selbstverständlich zu halten.</p>
</div>`
    }
});
